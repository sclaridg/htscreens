...
```{r}
ccl_histologies <- select(ccl_info, source_ccl_name, source_histology, CCLE_ccl_name, Broad_ID) %>% drop_na(source_ccl_name)
...
auc_unmatched_na_histologies <- read.delim("./data_munging/pozdeyev2016/auc_unmatched_na_histologies.csv", sep = ",", row.names = 1)
auc_unmatched_na_histologies$accession_id <- NULL
auc_unmatched_na_histologies$Notes <- NULL
auc_unmatched_na_histologies$Histology <- ifelse(auc_unmatched_na_histologies$Histology == "", NA, as.character(auc_unmatched_na_histologies$Histology))
auc_unmatched_na_histologies$CCLE_ccl_name <- ifelse(auc_unmatched_na_histologies$CCLE_ccl_name  == "", NA, as.character(auc_unmatched_na_histologies$CCLE_ccl_name ))
auc_unmatched_na_histologies$Broad_ID <- ifelse(auc_unmatched_na_histologies$Broad_ID == "", NA, as.character(auc_unmatched_na_histologies$Broad_ID))

ccl_histologies <- subset(ccl_histologies, !(Broad_ID %in% auc_unmatched_na_histologies$Broad_ID))
# Bind DepMap histologies and curated histologies
ccl_histologies <- bind_rows(ccl_histologies, auc_unmatched_na_histologies)
```
...
ccle_poz <- merge(ccl_histologies, ccle_poz, by = "Cell_line", all.y = TRUE)
ccle_poz$InCCLE <- "1"
ccle_poz$InCTRP <- ""
ccle_poz$InGDSC <- ""
...
ctrp_poz <- merge(ccl_histologies, ctrp_poz, by = "Cell_line", all.y = TRUE)
ctrp_poz$InCCLE <- ""
ctrp_poz$InCTRP <- "1"
ctrp_poz$InGDSC <- ""
...
gdsc_poz <- merge(ccl_histologies, gdsc_poz, by = "Cell_line", all.y = TRUE)
gdsc_poz$InCCLE <- ""
gdsc_poz$InCTRP <- ""
gdsc_poz$InGDSC <- "1"
...
auc_unmatched <- bind_rows(ccle_poz, ctrp_poz, gdsc_poz)
auc_unmatched$WasDrugScreened <- 1
...
# auc_unmatched$sample_ID <- with(auc_unmatched, paste(Database, Cell_line, Histology, sep = "_"))
auc_unmatched$Drug_name_Database <- with(auc_unmatched, paste(Drug_name, Database, sep = "_"))

num_auc_per_db_unmatched <- auc_unmatched %>% count(Database)

# Identify cell lines without histology indications. Write to file to edit for merging back to ccl_histologies file
# test <- filter(auc_unmatched, is.na(Histology))[, 1:10]
# auc_unmatched_na_hists <- filter(auc_unmatched, is.na(Histology))$Cell_line
# write.csv(auc_unmatched_na_hists, "./data_munging/pozdeyev2016/auc_unmatched_na_histologies.csv")

# Create merged dataset with sample_ids in the rows and drugs across the columns
auc_unmatched_wide <- dcast(auc_unmatched, Cell_line + Histology + CCLE_ccl_name + Broad_ID + InCCLE + InCTRP + InGDSC + WasDrugScreened ~ Drug_name_Database, value.var = "AUC_IC50", fun.aggregate = mean, fill = 1000000)
auc_unmatched_wide[auc_unmatched_wide == 1000000] <- ""
auc_unmatched_wide <- auc_unmatched_wide %>% group_by(Cell_line, Histology, CCLE_ccl_name, Broad_ID, WasDrugScreened) %>% summarise_all(funs(trimws(paste(., collapse = ""))))
auc_unmatched_wide$InCCLE[auc_unmatched_wide$InCCLE == ""] <- 0
auc_unmatched_wide$InCTRP[auc_unmatched_wide$InCTRP == ""] <- 0
auc_unmatched_wide$InGDSC[auc_unmatched_wide$InGDSC == ""] <- 0
auc_unmatched_wide[auc_unmatched_wide == ""] <- NA
auc_unmatched_wide <- add_column(auc_unmatched_wide, WasCRISPRScreened = NA, .after = "WasDrugScreened")
auc_unmatched_wide <- add_column(auc_unmatched_wide, HasCopyNumberData = NA, .after = "WasCRISPRScreened")
auc_unmatched_wide <- add_column(auc_unmatched_wide, HasGeneExpressionData = NA, .after = "WasCRISPRScreened")
auc_unmatched_wide <- add_column(auc_unmatched_wide, HasMutationData = NA, .after = "WasCRISPRScreened")

ccls_not_drugscreened <- filter(ccl_histologies, Cell_line %in% setdiff(ccl_histologies$Cell_line, auc_unmatched_wide$Cell_line))
...
auc_all_ccls <- bind_rows(auc_unmatched_wide, ccls_not_drugscreened)
auc_all_ccls$HasMutationData <- with(auc_all_ccls, ifelse(Broad_ID %in% maf_df$Broad_ID, 1, 0))
auc_all_ccls$HasCopyNumberData <- with(auc_all_ccls, ifelse(Broad_ID %in% cn_melt$Broad_ID, 1, 0))
auc_all_ccls$HasGeneExpressionData <- with(auc_all_ccls, ifelse(Broad_ID %in% ge_melt$Broad_ID, 1, 0))
auc_all_ccls$WasCRISPRScreened <- with(auc_all_ccls, ifelse(Broad_ID %in% crispr_meta$Broad_ID, 1, 0))
auc_all_ccls$InCCLE <- with(auc_all_ccls, ifelse(WasDrugScreened == 0, 0, InCCLE))
auc_all_ccls$InCTRP <- with(auc_all_ccls, ifelse(WasDrugScreened == 0, 0, InCTRP))
auc_all_ccls$InGDSC <- with(auc_all_ccls, ifelse(WasDrugScreened == 0, 0, InGDSC))
auc_all_ccls[, -c(1:4)] <- apply(auc_all_ccls[, -c(1:4)], MARGIN = 2, FUN = as.numeric)
auc_all_ccls <- subset(auc_all_ccls, !(WasDrugScreened == 0 & WasCRISPRScreened == 0 & HasMutationData == 0 & HasGeneExpressionData == 0 & HasCopyNumberData == 0))
# write.csv.gz(auc_all_ccls, file = "./data_munging/pozdeyev2016/merged_auc_v20190401.4.csv.gz", row.names = FALSE, na = "NA")

# test <- as.data.frame(table(auc_all_ccls$Cell_line))
# test <- filter(auc_all_ccls, Cell_line %in% c("KMH2", "MS1", "TT"))[, 1:12]

# length(unique(sapply(strsplit(as.character(colnames(auc_all_ccls)[13:length(colnames(auc_all_ccls))]), split = "_"), function(x) x[1])))
```

```{r}
auc_all_ccls <- read.delim("./data_munging/pozdeyev2016/merged_auc_v20190401.4.csv.gz", sep = ",")

sum(auc_all_ccls$HasCopyNumberData)
sum(auc_all_ccls$HasMutationData)
sum(auc_all_ccls$HasGeneExpressionData)
sum(auc_all_ccls$WasDrugScreened)

pgx_ccls <- filter(auc_all_ccls, WasDrugScreened == 1)
crispr_ccls <- filter(auc_all_ccls, WasCRISPRScreened == 1)
unscreened_ccls <- filter(auc_all_ccls, WasCRISPRScreened == 0 & WasDrugScreened == 0)

ge_ccls <- filter(auc_all_ccls, HasGeneExpressionData == 1)
cn_ccls <- filter(auc_all_ccls, HasCopyNumberData == 1)
mut_ccls <- filter(auc_all_ccls, HasMutationData == 1)

all_omics_ccls <- intersect(intersect(unique(ge_ccls$Cell_line), unique(cn_ccls$Cell_line)), unique(mut_ccls$Cell_line))
sum(filter(auc_all_ccls, Cell_line %in% all_omics_ccls)$WasDrugScreened)

ccle_ccls <- filter(auc_all_ccls, InCCLE == 1)
ctrp_ccls <- filter(auc_all_ccls, InCTRP == 1)
gdsc_ccls <- filter(auc_all_ccls, InGDSC == 1)

auc_all_ccls %>% count(Histology)
```

```{r}
test <- mut_ccls[, 1:20]
test2 <- test %>% count(Broad_ID)
```
...
# png("./gcn_denise/pozdeyev_unmatched/pozdeyev_venn_all_ccl_unmatched.png", res = 500, units = "in", width = 6, height = 6)
# grid.newpage()
...
png("./gcn_denise/pozdeyev_unmatched/pozdeyev_venn_omics_overlap.png", res = 500, units = "in", width = 6, height = 6)
grid.newpage()
grid.draw(venn_omics_overlap)
dev.off()

num_ccl_per_hist_unmatched_bar <- ggplot(data = unique(select(auc_all_ccls, Cell_line, Histology))) +
...
num_ccl_per_hist_unmatched_bar
# ggsave("./gcn_denise/pozdeyev_unmatched/pozdeyev_ccl_per_histology_unmatched.png", num_ccl_per_hist_unmatched_bar, device = "png", dpi = 450, width = 12, height = 6, units = "in")
```


### Compare to TCGA
Quality check: cluster TCGA samples and cell lines—verify that same tumor types cluster together rather than clustering by cell lines and tumor samples
...
```{r, eval=FALSE}
cn_seg <- read.delim("./data_munging/19Q1/copy_number_20190328.csv.gz", sep = ",")
tcga_pancan <- read.delim("~/Desktop/GDC-PANCAN.masked_cnv.tsv.gz", sep = "\t")

ggplot(cn_seg, aes(x = log2(Segment_Mean/2))) + 
  geom_density()

ggplot(tcga_pancan, aes(x = value)) + 
  geom_density()
```
...
Column descriptions in merged_auc_v20190319.1.csv.gz:
...
```{r, eval=FALSE}
futile.logger::flog.threshold(futile.logger::ERROR, name = "VennDiagramLogger")
venn_all_ccl_unmatched_pozdeyev <- venn.diagram(list("CCLE" = unique(ccle_poz$Cell_line), "CTRP" = unique(ctrp_poz$Cell_line), "GDSC" = unique(gdsc_poz$Cell_line)), resolution = 5000, main = "Overlap of cancer cell lines (CCLs) screened in each dataset", main.fontface = "bold", lty = rep("blank", 3), fill = c("springgreen4", "steelblue4", "turquoise4"), alpha = c(0.5, 0.5, 0.5), cat.pos = c(-30, 30, 180), cat.dist = c(0.01, 0.001, 0.001), cat.cex = 1.5, cex = 1, filename = NULL)
# png("./gcn_denise/pozdeyev_unmatched/pozdeyev_venn_all_ccl_unmatched.png", res = 500, units = "in", width = 6, height = 6)
# grid.newpage()
# grid.draw(venn_all_ccl_unmatched_pozdeyev)
# dev.off()

futile.logger::flog.threshold(futile.logger::ERROR, name = "VennDiagramLogger")
venn_all_drugs_unmatched_pozdeyev <- venn.diagram(list("CCLE" = unique(ccle_poz$Drug_name), "CTRP" = unique(ctrp_poz$Drug_name), "GDSC" = unique(gdsc_poz$Drug_name)), resolution = 5000, main = "Overlap of cancer cell lines (CCLs) screened in each dataset", main.fontface = "bold", lty = rep("blank", 3), fill = c("springgreen4", "steelblue4", "turquoise4"), alpha = c(0.5, 0.5, 0.5), cat.pos = c(-30, 30, 180), cat.dist = c(0.01, 0.001, 0.001), cat.cex = 1.5, cex = 1, filename = NULL)
# png("./gcn_denise/pozdeyev_unmatched/pozdeyev_venn_all_drugs_unmatched.png", res = 500, units = "in", width = 6, height = 6)
# grid.newpage()
# grid.draw(venn_all_drugs_unmatched_pozdeyev)
# dev.off()

num_ccl_per_hist_per_db_unmatched_bar <- ggplot(data = unique(select(auc_unmatched, Database, Cell_line, Histology))) +
  facet_wrap(~ Database) +
  geom_bar(aes(x = Histology, fill = Histology), alpha = 0.7) +
  guides(fill = "none") +
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
num_ccl_per_hist_per_db_unmatched_bar
# ggsave("./gcn_denise/pozdeyev_unmatched/pozdeyev_ccl_per_histology_per_db_unmatched.png", num_ccl_per_hist_per_db_unmatched_bar, device = "png", dpi = 450, width = 12, height = 8, units = "in")

num_ccl_per_hist_unmatched_bar <- ggplot(data = unique(select(auc_unmatched, Cell_line, Histology))) +
  geom_bar(aes(x = Histology, fill = Histology), alpha = 0.7) +
  guides(fill = "none") +
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
num_ccl_per_hist_unmatched_bar
# ggsave("./gcn_denise/pozdeyev_unmatched/pozdeyev_ccl_per_histology_unmatched.png", num_ccl_per_hist_unmatched_bar, device = "png", dpi = 450, width = 12, height = 8, units = "in")

aucic50_per_db_unmatched <- ggplot(data = auc_unmatched) +
...
aucic50_per_db_unmatched
# ggsave("./gcn_denise/pozdeyev_unmatched/pozdeyev_aucic50_per_db_unmatched.png", aucic50_per_db_unmatched, device = "png", dpi = 450, width = 12, height = 6, units = "in")
```
...
There were `r length(setdiff(unique(ccl_histologies$Cell_line), unique(auc_matched $Cell_line)))` cell lines not represented in the Pozdeyev et al. harmonized dataset.
