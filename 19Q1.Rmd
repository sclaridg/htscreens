...
g2p <- add_column(g2p, Variant_Type = NA, .after = "Alt")
g2p$Variant_Type <-
  ...
  ifelse(g2p$Ref_Length < g2p$Alt_Length, "INS", NA))))))
...
g2p$biomarker_type_curated <-
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z0-9]+-[A-Z0-9]+^[>|)]")), "fusion",
  ...
  ifelse(!is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "biallelic inactivation")), "biallelic inactivation",
  ifelse(!is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "amplification")), "amplification",
  ifelse(!is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "mutant|mutation")), "mutation",
  ...
  ifelse(g2p$biomarker_type == "inframe_deletion", "In_Frame_Del",
  ...
  ifelse(g2p$Feature.Names %in% c("EGFR E746_A750del", "EGFR exon 19 deletions", "Exon 19 deletion/insertion", "Exon 19 deletion"), "In_Frame_Ins",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z][0-9]+([X]|[*])")), "Nonsense_Mutation",
  ifelse(g2p$Feature.Names == "KRAS G12fs*3" | g2p$Feature.Names == "EGFR E746fs*15", "deletion",
  ifelse(!is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "amplification")), "amplification",
  ifelse(g2p$biomarker_type == "microsatellite", "microsatellite",
  ifelse(g2p$biomarker_type == "positive" | !is.na(str_match(string = g2p$Feature.Names, pattern = "positive")), "positive",
  ifelse(g2p$biomarker_type == "rearrange", "rearrangement",
  ifelse(g2p$biomarker_type == "intron variant", "Intron",
  ifelse(!is.na(str_match(string = tolower(g2p$biomarker_type), pattern = "mut")), "mutation", g2p$biomarker_type_curated)))))))))))))))))))))))
