...
# g2p_json <- "./../htscreens_giant_files/g2p_all.json"
# g2p_all <- fromJSON(sprintf("[%s]", paste(readLines(g2p_json), collapse = ",")), flatten = TRUE)
# saveRDS(g2p_all, "./../htscreens_giant_files/g2p_all_flattened_json.rds", compress = "xz")
...
g2p[g2p == -1] <- NA
g2p$Gene <- ifelse(is.na(g2p$Gene) & g2p$Drug == "daunorubicin", "MLL", as.character(g2p$Gene))
...
biomarker_type_curated <-
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "splice")) | !is.na(str_match(string = g2p$biomarker_type, pattern = "splice")), "Splice_Site",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "Amplification|amplification")), "amplification",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "^PDGFRA [(]")) | !is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "mutant|mutation|mut")), "mutation",
  ifelse(!is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "fusion")) | !is.na(str_match(string = tolower(g2p$biomarker_type), pattern = "fusion")), "fusion", 
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z0-9|A-Z]+-[A-Z0-9|A-Z]+")) & is.na(str_match(string = g2p$Feature.Names, pattern = "[0-9]+-[0-9]+")), "fusion",
  ifelse(g2p$Variant_Type == "DEL" & g2p$Variant_Length %% 3 == 0, "In_Frame_Del",
  ifelse(g2p$Variant_Type == "DEL" & g2p$Variant_Length %% 3 != 0, "Frame_Shift_Del",
  ifelse(g2p$Variant_Type == "INS" & g2p$Variant_Length %% 3 == 0, "In_Frame_Ins",
  ifelse(g2p$Variant_Type == "INS" & g2p$Variant_Length %% 3 != 0, "Frame_Shift_Ins",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "EGFR E746_A750del")), "In_Frame_Del",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "EGFR H773>PNPY")), "In_Frame_Ins",
  ifelse(g2p$Feature.Names == "KRAS G12G" | g2p$Feature.Names == "KRAS G13G", "Silent",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z][0-9]+[A-WYZ]")) | !is.na(str_match(string = g2p$biomarker_type, pattern = "missense")), "Missense_Mutation",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z][0-9]+([X]|[*])")), "Nonsense_Mutation",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "positive")), "positive",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "expression")), "expression",
  ifelse(g2p$biomarker_type == "rearrange", "rearrangement",
  ifelse(g2p$biomarker_type == "intron variant", "Intron",
  ifelse(g2p$biomarker_type == "inframe_deletion" | !is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "inframe deletion")), "In_Frame_Del",
  ifelse(g2p$biomarker_type == "inframe_insertion" | !is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "inframe insertion")), "In_Frame_Ins",
  ifelse(!is.na(str_match(string = tolower(g2p$Feature.Names), pattern = "biallelic inactivation")), "biallelic inactivation", NA)))))))))))))))))))))
...
```{r g2p}
g2p <- read.delim("./data_munging/g2p_db_edited.csv", sep = ",", header = TRUE)
g2p$biomarker_type_curated <- ifelse(is.na(g2p$biomarker_type_curated), "None", g2p$biomarker_type_curated)
...
```{r fusion_calls_19Q2, eval=FALSE}
...
colnames(fusions) <- c("Broad_ID", "FusionPair")
```
...
19Q1, CCLE RNAseq gene expression data:
...
MergeOmicsG2P <- function(database, maf_df, ge_df, cn_df, outfile) {
  ...
  if(database %in% c("CCLE", "CTRP", "GDSC")) {
    ...
    data <- merge(data, maf_df, by = c("Gene", "Broad_ID"), all.x = TRUE)
  ...
  # Merge GE data
  data <- merge(data, ge_df, by = c("Gene", "Broad_ID"), all.x = TRUE)
  print("Merged gene expression data.")
  
  # Merge CN data
  data <- merge(data, cn_df, by = c("Gene", "Broad_ID"), all.x = TRUE)
...
ccle <- MergeOmicsG2P(database = "CCLE", maf_df = maf_df, ge_df = ge_melt, cn_df = cn_melt, outfile = "./data_munging/rds/ccle_data_19Q1.rds")
# ctrp <- MergeOmicsG2P(database = "CTRP", maf_df = maf_df, ge_df = ge_melt, cn_df = cn_melt, outfile = "./../htscreens_giant_files/ctrp_data_19Q1.rds")
# gdsc <- MergeOmicsG2P(database = "GDSC", maf_df = maf_df, ge_df = ge_melt, cn_df = cn_melt, outfile = "./data_munging/rds/gdsc_data_19Q1.rds")


# Omics metadata
# maf_df_ptmuts <- unique(filter(maf_df, Gene %in% g2p_genes & PointMutation == 1)
# ge_filt <- filter(ge_melt, Gene %in% g2p_genes)
# cn_filt <- filter(cn_melt, Gene %in% g2p_genes)
# ccle_ptmuts <- MergeOmicsG2P(database = "CCLE", maf_df = maf_df_ptmuts, ge_df = ge_filt, cn_df = cn_filt, outfile = "./data_munging/rds/ccle_data_19Q1_g2pgenes_ptmuts.rds")
# ctrp_ptmuts <- MergeOmicsG2P(database = "CTRP", maf_df = maf_df_ptmuts, ge_df = ge_filt, cn_df = cn_filt, outfile = "./../htscreens_giant_files/ctrp_data_19Q1_g2pgenes_ptmuts.rds")
# gdsc_ptmuts <- MergeOmicsG2P(database = "GDSC", maf_df = maf_df_ptmuts, ge_df = ge_filt, cn_df = cn_filt, outfile = "./data_munging/rds/gdsc_data_19Q1_g2pgenes_ptmuts.rds")
```

```{r}
ccle_missense <- filter(ccle, Variant_Classification == "Missense_Mutation")
ccle_missense_grid <- expand.grid("Gene" = unique(ccle_missense$Gene), "Broad_ID" = unique(ccle_missense$Broad_ID))
ccle_missense_grid$Variant_Classification <- NA
ccle_missense <- merge(ccle_missense, ccle_missense_grid, by = c("Gene", "Broad_ID", "Variant_Classification"), all = TRUE)
```


```{r load_drug_rds, eval=TRUE}
ccle_ptmuts <- readRDS("./data_munging/rds/ccle_data_19Q1_g2pgenes_ptmuts.rds") %>% drop_na(Drug)
ctrp_ptmuts <- readRDS("./../htscreens_giant_files/ctrp_data_19Q1_g2pgenes_ptmuts.rds") %>% drop_na(Drug)
gdsc_ptmuts <- readRDS("./data_munging/rds/gdsc_data_19Q1_g2pgenes_ptmuts.rds") %>% drop_na(Drug)
...
test_crispr <- unique(select(crispr_melt, Broad_ID, Histology)) %>% count(Histology)
...
crispr_data$Mutation_Status <- factor(crispr_data$Mutation_Status)
...
```{r}
ccle_auc_grid <- dcast(filter(ccle_ptmuts, Drug %in% g2p_drugs), Drug ~ Broad_ID, value.var = "Score", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Drug)
ctrp_auc_grid <- dcast(filter(ctrp_ptmuts, Drug %in% g2p_drugs), Drug ~ Broad_ID, value.var = "Score", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Drug)
gdsc_auc_grid <- dcast(filter(gdsc_ptmuts, Drug %in% g2p_drugs), Drug ~ Broad_ID, value.var = "Score", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Drug)
...
# crispr_signif_all_gene <- WilcoxonTestDF(dataframe = crispr_ptmuts, grouping = "Gene", outfile = "./data_munging/rds/crispr_signif_gene.rds")
crispr_ptmuts_filt <- filter(crispr_ptmuts, Gene %in% g2p_genes)
crispr_signif_gene <- WilcoxonTestDF(dataframe = crispr_ptmuts_filt, grouping = "Gene", outfile = "./data_munging/rds/crispr_signif_g2p_gene.rds")
crispr_signif_lineage <- WilcoxonTestDF(dataframe = crispr_ptmuts_filt, grouping = c("Gene", "Primary.Disease"), outfile = "./data_munging/rds/crispr_signif_g2p_lineage.rds")
...
crispr_ptmuts_filt <- filter(crispr_ptmuts, Gene %in% g2p_genes)
crispr_ptmuts_filt$Gene <- factor(crispr_ptmuts_filt$Gene, levels = g2p_genes)
crispr_ptmuts_filt$Mutation_Status <- as.character(crispr_ptmuts_filt$Mutation_Status)

crispr_gene_mut_plot <- ggplot(data = crispr_ptmuts_filt) +
...
crispr_gene_lin_plot <- ggplot(data = crispr_ptmuts_filt) +
...
crispr_spear_ge <- transform(crispr_ptmuts_filt %>% group_by(Gene) %>% summarize(p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value, p_log10 = -log10(p), corr = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate, Metric = "Gene Expression"), Gene = reorder(Gene, corr))
crispr_spear_cn <- transform(crispr_ptmuts_filt %>% group_by(Gene) %>% summarize(p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value, p_log10 = -log10(p), corr = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate, Metric = "Copy Number"), Gene = reorder(Gene, corr))
```
...
spearman_KRAS <- rbind(filter(crispr_ptmuts_filt, Gene == "KRAS") %>% summarize(p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value, corr = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate, Metric = "Gene Expression"), filter(crispr_ptmuts_filt, Gene == "KRAS") %>% summarize(p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value, corr = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate, Metric = "Copy Number"))

crispr_ge_spearman_corr_inset <- ggplot(filter(crispr_ptmuts_filt, Gene == "KRAS"), aes(x = TPM, y = Score)) +
  geom_point(pch = 21, fill = "cadetblue3", alpha = 0.6) +
  labs(x = expression("KRAS Gene Expression"~(log[2](TPM))), y = "CERES Score", subtitle = paste0("Spearman correlation coefficient = ", round(spearman_KRAS$corr[1], 2), " (p = ", formatC(as.numeric(spearman_KRAS$p[1]), format = "e", digits = 1), ")"))

crispr_cn_spearman_corr_inset <- ggplot(filter(crispr_ptmuts_filt, Gene == "KRAS"), aes(x = CN, y = Score)) +
...
ctrp_ge_ht2 <- MakeCorrHeatmap(drug_cor_res = ctrp_cor, row_names = FALSE, col_title = "CTRP", color_range = c(-0.5, 0, 0.5), legend_title = "Gene expr. Spearman correlation", metric = "GE")
...
```{r drug_KS_tests, eval=FALSE, warning=FALSE, fig.height=6, fig.width=12}
...
g2p_corr_4plot <- filter(g2p_dgl_corr, G2P_Drug_Gene == "Yes")
