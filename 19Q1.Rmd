...
```{r}
g2p_all <- read.delim("./data_munging/g2p_full_level_A_dataset_NEW_DEC_2_2018_withGroupedLineages.csv", sep = "\t", header = TRUE)
g2p_all[, 1:2] <- NULL
g2p_all$Drug <- tolower(g2p_all$Drug)
g2p_all[g2p_all == -1] <- NA
g2p_all$Ref[g2p_all$Ref == "None"] <- NA
g2p_all$Alt[g2p_all$Alt == "None"] <- NA
g2p_all <- subset(g2p_all, grepl("^CID", g2p_all$DrugID))
g2p <- merge(g2p_all, lineage_converter, by = "grouped_general_doid", all.x = TRUE)
...
g2p_nonCID <- subset(g2p_all, !grepl("^CID", g2p_all$DrugID))
...
g2p_dg_counts <- g2p_all %>% count(Drug, , Gene) %>% filter(!is.na(Drug), !is.na(Gene))
g2p_dg_counts <- dcast(g2p_dg_counts, Gene ~ Drug, value.var = "n", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Gene)
g2p_dg_counts <- g2p_dg_counts[order(rowSums(-g2p_dg_counts)), order(colSums(-g2p_dg_counts))]
g2p_dg_counts[g2p_dg_counts == 0] <- NA

# Define color scale
col_fun <- colorRamp2(c(0, 103), c("mistyrose", "purple4"))
# Legend properties list
ht_legend_list <- list(legend_direction = "horizontal", title_position = "lefttop", legend_width = unit(6, "in"), border = "black")
ht <- Heatmap(g2p_dg_counts, 
              name = "Frequency",
              col = col_fun,
              na_col = "white",
              rect_gp = gpar(col = "gray88"),
...
**NEED TO FILTER FOR ONLY G2P DRUG-GENE COMBINATIONS PRIOR TO KS TEST**
