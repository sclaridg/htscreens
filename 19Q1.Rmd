...
g2p <- add_column(g2p, biomarker_type_curated = as.character(894), .after = "biomarker_type")
g2p$biomarker_type_curated <-
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = ",")), NA,
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "del")), "deletion",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "ins|dup")), "insertion",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "oncogenic mutation|mutant")), "mutation",
  ifelse(g2p$Feature.Names == "KRAS G12fs*3", "deletion",
  ifelse(g2p$biomarker_type == "intron variant", "intron variant",
  ifelse(g2p$biomarker_type == "microsatellite", "microsatellite",
  ifelse(g2p$biomarker_type == "positive", "positive",
  ifelse(g2p$biomarker_type == "rearrange", "rearrangement",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "fusion|FUSION|Fusion")), "fusion", 
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z0-9]-[A-Z0-9]")), "fusion",
  ifelse(g2p$biomarker_type == "synonymous", "synonymous",
  ifelse(g2p$biomarker_type == "biallelic inactivation", "biallelic inactivation",       
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z][0-9]+[A-WYZ]")), "missense",
  ifelse(!is.na(str_match(string = g2p$Feature.Names, pattern = "[A-Z][0-9]+([X]|[*])")), "nonsense", NA)))))))))))))))
...
!is.na(str_match(string = "ALK L1198F", pattern = "[A-Z][0-9]+[A-WYZ]"))
