...
```{r}
ccl_info <- read.delim("./data_munging/19Q1/DepMap-2019Q1-celllines_20181115.csv", sep = ",", header = TRUE, na.strings = c("", NA))
...
# From figshare
crispr_meta <- read.delim("./data_munging/19Q1/sample_info_19Q1_crispr_20181115.csv", sep = ",", header = TRUE, na.strings = c("", NA))
...
```{r, eval=FALSE}
maf_raw <- read.delim("./data_munging/19Q1/depmap_19Q1_mutation_calls_20181114.csv.gz", header = TRUE, sep = ",", row.names = 1)
...
DepMap WES CN Data:
```{r, eval=FALSE}
cn <- read.delim("./data_munging/19Q1/public_19Q1_gene_cn_20181115.csv.gz", sep = ",", check.names = FALSE, header = TRUE)
...
```{r, eval=FALSE}
ge_19Q1 <- read.delim("./../htscreens_giant_files/CCLE_depMap_19Q1_TPM_v2_20181115.csv.gz", header = TRUE, sep = ",", check.names = FALSE)
# Remove Entrez gene IDs from colnames
# Format: Hugo_Symbol (Entrez ID)
colnames(ge_19Q1) <- gsub(" .*", "", colnames(ge_19Q1))
colnames(ge_19Q1)[1] <- "DepMap_ID"
# Melt
ge_melt_19Q1_all <- melt(ge_19Q1, id.vars = "DepMap_ID", measure.vars = colnames(ge_19Q1)[2:ncol(ge_19Q1)], variable.name = "Hugo_Symbol", value.name = "TPM")
ge_melt_19Q1 <- unique(ge_melt_19Q1)
