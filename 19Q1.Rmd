...
auc_unmatched_wide <- dcast(auc_unmatched, Cell_line + Histology + CCLE_ccl_name + Broad_ID + InCCLE + InCTRP + InGDSC ~ Drug_name_Database, value.var = "AUC_IC50", fun.aggregate = mean, fill = 1000000)
auc_unmatched_wide[auc_unmatched_wide == 1000000] <- ""
auc_unmatched_wide <- auc_unmatched_wide %>% group_by(Cell_line, Histology, CCLE_ccl_name, Broad_ID) %>% summarise_all(funs(trimws(paste(., collapse = ''))))
auc_unmatched_wide$InCCLE[auc_unmatched_wide == ""] <- 0
auc_unmatched_wide$InCTRP[auc_unmatched_wide == ""] <- 0
auc_unmatched_wide$InGDSC[auc_unmatched_wide == ""] <- 0
auc_unmatched_wide[auc_unmatched_wide == ""] <- NA

# write.csv.gz(auc_unmatched_wide, file = "./data_munging/pozdeyev2016/merged_auc_vCHANGEDATE.3.csv.gz", row.names = FALSE, na = "NA")
```

```{r}
# test <- auc_unmatched_wide_test$InCCLE
# rm_numerics <- function(x) {
#   col_new <- c()
#   for(i in 1:length(x))
#     print(i)
#     x_unlist <- unlist(x[i])
#     print(x_unlist)
#     x_new <- ifelse(is.na(x_unlist[1]), 1000000, x_unlist[1])
#     print(x_new)
#     col_new <- c(col_new, x_new)
#     print(col_new)
#   return(col_new)
# }
# test2 <- rm_numerics(test)
```
...
- `Database`: the dataset from which the data comes from (either CCLE, CTRP, or GDSC)
...
- `sample_ID`: custom identifiers for this dataset that are concatenations of the `Database`, `Cell_line`, and `Histology`, separated by underscores
- `CCLE_ccl_name`: the cell line name using CCLE nomenclature, which is a concatenation of the cell line name with the CCLE-annotated histology in all caps, separated by underscores
- `Broad_ID`: the custom identifiers (formatted as ACH-######) used by the Broad's Cancer Dependency Map project to ease cell line name harmonization efforts 
- All other columns: compound and drug names that were screened in at least one CCL in at least one database
    - NA values indictate that compound wasn't screened in the respective CCL in the respective source dataset
