...
auc_unmatched$Database_Drug_name <- with(auc_unmatched, paste(Database, Drug_name, sep = "_"))
...
auc_unmatched_wide <- dcast(auc_unmatched, Cell_line + Histology + CCLE_ccl_name + Broad_ID + InCCLE + InCTRP + InGDSC ~ Database_Drug_name, value.var = "AUC_IC50", fun.aggregate = mean, fill = 1000000)
auc_unmatched_wide[, 8:ncol(auc_unmatched_wide)] <- auc_unmatched_wide[, 8:ncol(auc_unmatched_wide)] %>% replace_with_na_all(condition = ~.x == 1000000)
# auc_unmatched_wide <- aggregate(. ~ colnames(auc_unmatched_wide)[c(1:4)], data = auc_unmatched_wide, FUN = na.omit, na.action = "na.pass")
