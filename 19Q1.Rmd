...
# Accout for cell line aliases and missing histologies
...
auc_unmatched_wide <- dcast(auc_unmatched, Database + Cell_line + Histology + sample_ID + CCLE_ccl_name + Broad_ID ~ Drug_name, value.var = "AUC_IC50", fun.aggregate = mean, fill = 1000000)
auc_unmatched_wide[, 7:ncol(auc_unmatched_wide)] <- auc_unmatched_wide[, 7:ncol(auc_unmatched_wide)] %>% replace_with_na_all(condition = ~.x == 1000000)
# write.csv.gz(auc_unmatched_wide, file = "./data_munging/pozdeyev2016/merged_auc_v20190319.2.csv.gz", row.names = FALSE, na = "NA")
```
