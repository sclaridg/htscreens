...
MergeOmicsG2P <- function(database, outfile) {
  ...
  data$Keep_PtMut_Tests <- with(data, ifelse(Reference_Allele == 1 | is.na(Reference_Allele), TRUE, ifelse(Reference_Allele == 0 & Tumor_Seq_Allele1 == 1, TRUE, FALSE)))
...
FilterForPointMuts <- function(dataframe, outfile) {
  ...
  ptmuts <- filter(dataframe, Keep_PtMut_Tests == TRUE)
  # Remove columns that lead to duplicate mutation entries
  ptmuts <- subset(ptmuts, select = -c(Variant_Classification, Reference_Allele, Tumor_Seq_Allele1, Keep_PtMut_Tests))
...
ctrp <- readRDS("./data_munging/rds/ctrp_data_19Q1_g2pgenes.rds")
...
ctrp_ptmuts <- readRDS("./data_munging/rds/ctrp_data_19Q1_g2pgenes_ptmuts.rds")
...
crispr_melt <- melt(crispr, id.vars = "DepMap_ID", measure.vars = colnames(crispr)[2:ncol(crispr)], variable.name = "Gene", value.name = "Score")
...
crispr_muts <- merge(crispr_melt, maf_filt_crispr, by = c("Gene", "CCLE_Name", "DepMap_ID"), all.x = TRUE)
...
# Copy number
crispr_data <- merge(crispr_data, cn_melt, by = c("Gene", "DepMap_ID"), all.x = TRUE)
# Gene expression (RPKM)
ge_crispr_18Q3 <- filter(ge_melt_18Q3, Gene %in% as.character(unique(crispr_melt$Gene)))
# ge_crispr_19Q1 <- filter(ge_melt_19Q1, Gene %in% as.character(unique(crispr_melt$Gene)))
crispr_data <- merge(crispr_data, ge_crispr_18Q3, by = c("Gene", "CCLE_Name", "DepMap_ID"), all.x = TRUE)
# crispr_data <- merge(crispr_data, ge_crispr_19Q1, by = c("Gene", "DepMap_ID"), all.x = TRUE)
...
crispr_data_ptmuts <- filter(crispr_data, Gene %in% g2p_genes)
