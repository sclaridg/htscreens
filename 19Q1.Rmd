...
MergeOmicsG2P <- function(database, outfile) {
  ...
  if(database == "CRISPR") {
    ...
    data <- melt(crispr, id.vars = "Broad_ID", measure.vars = colnames(crispr)[2:ncol(crispr)], variable.name = "Gene", value.name = "Score")
...
```{r, eval=TRUE}
crispr_all <- readRDS("./../htscreens_giant_files/crispr_data_19Q1.rds")

# Filter G2P CRISPR for point mutations
crispr_ptmuts <- filter(crispr_data, PointMutation == 1)
crispr_ptmuts <- subset(crispr_data_ptmuts, select = -c(PointMutation))
# Filter CRISPR for G2P genes in G2P
crispr_data_g2p <- filter(crispr_ptmuts, Gene %in% g2p_genes)
crispr_data_g2p <- unique(crispr_data_g2p)
crispr_data_g2p$Gene <- factor(crispr_data_g2p$Gene)
crispr_data_g2p$Mutation_Status <- factor(crispr_data_g2p$Mutation_Status)
crispr_data_g2p$Primary.Disease <- factor(crispr_data_g2p$Primary.Disease)
```

Test merging and filtering functions on CRISPR data:
```{r, eval=FALSE}
crispr <- MergeOmicsG2P(database = "CRISPR", outfile = "./../htscreens_giant_files/crispr_data_19Q1_g2pgenes.rds")
crispr_ptmuts <- FilterForPointMuts(dataframe = crispr, outfile = "./../htscreens_giant_files/crispr_data_19Q1_g2pgenes_ptmuts.rds")
```
