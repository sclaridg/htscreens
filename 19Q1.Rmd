...
pgx_ccle_ge <- merge(select(ccl_info, source, source_ccl_name, source_histology, sample_id, Broad_ID), ge_19Q1, by = "Broad_ID", all = TRUE)
...
ccle_filt <- merge(select(ccl_info, source, source_ccl_name, source_histology, sample_id), ccle_filt, by = c("source_ccl_name", "source_histology"), all = TRUE)
ccle_filt$compound_name <- toupper(ccle_filt$Compound)
colnames(ccle_filt)[colnames(ccle_filt) == "ActArea"] <- "AUC"
ccle_auc_long <- select(ccle_filt, source, source_ccl_name, source_histology, sample_id, compound_name, AUC)
...
ge_merged <- merge(select(ccl_info, Broad_ID, source_histology, source_ccl_name), ge_melt, by = "Broad_ID", all = TRUE)
```
