...
saveRDS(crispr_data, "./data_munging/rds/crispr_data_19Q1.rds", compress = "xz")
...
crispr_ptmuts <- readRDS("./data_munging/rds/crispr_data_19Q1_g2pgenes_ptmuts.rds")
```
...
venn.diagram(x = list(ccle_ccls, ctrp_ccls, gdsc_ccls, crispr_ccls),
             filename = "./plots/manuscript/venn_ccls.png",
...
venn.diagram(x = list(ccle_drugs, ctrp_drugs, gdsc_drugs),
             filename = "./plots/manuscript/venn_drugs.png",
             output = TRUE, imagetype = "png",
             height = 2000, width = 2000, resolution = 500,
             lwd = 1, cex = 1.4,
...
png(filename = "./plots/manuscript/histology_distributions.png", width = 8, height = 9, units = "in", res = 500)
...
WilcoxonTestDF <- function(dataframe, grouping, outfile) {
  require(ggpubr)
  if(grouping == "Drug_Gene_Lin") {
    # Filter out DGLs with too few samples in at least one mutation status category
    count <- ctrp_ptmuts %>% count(Drug_Gene_Lin, Mutation_Status)
    count2 <- count %>% count(Drug_Gene_Lin)
    count3 <- merge(count, count2, by = "Drug_Gene_Lin", suffixes = c("_sub3", "_1mut")) %>% filter(n_1mut == 2)
    count3$n_sub3 <- ifelse(count3$n_sub3 >= 3, TRUE, FALSE)
    count4 <- count3 %>% filter(n_sub3) %>% count(Drug_Gene_Lin) %>% filter(n == 2)
    dataframe <- filter(dataframe, Drug_Gene_Lin %in% unique(count4$Drug_Gene_Lin))
    dataframe$Drug_Gene_Lin <- factor(dataframe$Drug_Gene_Lin)
  }
...
ccle_signif_g2p_druggene <- WilcoxonTestDF(dataframe = filter(ccle_ptmuts, Drug %in% g2p_drugs & Gene %in% g2p_genes), grouping = "Drug_Gene", outfile = "./data_munging/rds/ccle_signif_g2p_druggene.rds")
ccle_signif_lineage <- WilcoxonTestDF(dataframe = ccle_ptmuts, grouping = "Drug_Gene_Lin", outfile = "./data_munging/rds/ccle_signif_g2p_lineage.rds")
...
ctrp_signif_g2p_druggene <- WilcoxonTestDF(dataframe = filter(ctrp_ptmuts, Drug %in% g2p_drugs & Gene %in% g2p_genes), grouping = "Drug_Gene", outfile = "./data_munging/rds/ctrp_signif_g2p_druggene.rds")
ctrp_signif_lineage <- WilcoxonTestDF(dataframe = ctrp_ptmuts, grouping = "Drug_Gene_Lin", outfile = "./data_munging/rds/ctrp_signif_g2p_lineage.rds")
...
gdsc_signif_g2p_druggene <- WilcoxonTestDF(dataframe = filter(gdsc_ptmuts, Drug %in% g2p_drugs & Gene %in% g2p_genes), grouping = "Drug_Gene", outfile = "./data_munging/rds/gdsc_signif_g2p_druggene.rds")
gdsc_signif_lineage <- WilcoxonTestDF(dataframe = gdsc_ptmuts, grouping = "Drug_Gene_Lin", outfile = "./data_munging/rds/gdsc_signif_g2p_lineage.rds")
```
...
crispr_gene_mut_plot <- ggplot(data = crispr_ptmuts) +
  ...
  geom_text(data = crispr_signif_gene, aes(x = 1.5, y = 1.75, label = p.signif.adj)) +
...
```{r crispr_mut_lin_plot, fig.height=12, fig.width=8}
...
crispr_gene_lin_plot <- ggplot(data = crispr_ptmuts) +
  facet_wrap(~ Primary.Disease, nrow = 7, scales = "free_x") +
  scale_color_manual(name = "Mutation Status", labels = c("Wildtype", "Mutant"), values = c("1" = "darkorchid", "0" = "thistle")) +
  geom_point(aes(x = Gene, y = Score, color = Mutation_Status), alpha = 0.5, size = 0.6) +
  geom_text(data = crispr_signif_lineage, aes(x = Gene, y = 1.75, label = p.signif.adj)) +
  geom_hline(yintercept = 0, lty = 2, color = "darkgray") +
  theme(legend.position = "top", legend.direction = "horizontal", axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.4, size = 10)) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  ylab("CERES Score") +
  scale_x_discrete(name = "Mutation Status", labels = c("0" = "WT", "1" = "Mut"))
crispr_gene_lin_plot

# ggsave("./plots/manuscript/crispr_gene_lineage_plot.png", crispr_gene_lin_plot, width = 16, height = 20, units = "in")
```
...
```{r drug_spear_calc, warning=FALSE}
g2p_druggene_tally <- g2p %>% group_by(Drug_Gene) %>% tally() %>% drop_na()

# CCLE
ccle_cor_res <- ccle_ptmuts %>% group_by(Drug_Gene) %>%summarize(
...
# CTRP
ctrp_cor_res <- ctrp_ptmuts %>% group_by(Drug_Gene) %>%summarize(
                GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate,
                GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value,
                CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate,
                CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value)
...
# GDSC
gdsc_cor_res <- gdsc_ptmuts %>% group_by(Drug_Gene) %>%summarize(
                GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate,
                GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value,
                CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate,
                CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value)
