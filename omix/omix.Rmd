...
DepMapTCGAMakeWideDF <- function(melted_df, output_type, data_source, exclusions) {
  ...
  colnames(df) <- c("sample_ID", "TCGA_classification", "Gene", "Value")
...
DepMapTCGACorrMatrix <- function(DepMap_wide_df, TCGA_wide_df, corr_method) {
  ...
  corr <- wide %>% as.matrix %>% cor(method = corr_method) %>% as.data.frame %>% rownames_to_column(var = "var1") %>% gather(var2, value, -var1)
  ...
  corr_wide <- dcast(corr, DepMap ~ TCGA, value.var = "r")
  rownames(corr_wide) <- corr_wide$DepMap
  corr_wide$DepMap <- NULL
  return(corr_wide)
}
...
DepMapTCGACorrHeatmap <- function(corr_wide_df, out_filepath, legend_title, color_breaks) {
  ...
  # Get column order to have symmetrically ordered axes
...
DepMapTCGAPropsScatterplots <- function(DepMap_melted_df, TCGA_melted_df, out_filepath) {
  ...
  DepMap_prop <- DepMap_melted_df %>% group_by(TCGA_classification, Gene) %>% summarise(Proportion = sum(Value) / n())
  TCGA_prop <- TCGA_melted_df %>% group_by(TCGA_classification, Gene) %>% summarise(Proportion = sum(Value) / n())
  ...
  props_scatters <- ggplot(data = props, aes(x = Proportion_TCGA, y = Proportion_DepMap)) +
    facet_wrap(~ TCGA_classification, nrow = 4) +
  ...
  # Save to file
  ggsave(out_filepath, props_scatters, device = "png", dpi = 450, width = 12, height = 12, units = "in")
...
# Make wide
ge_depmap_wide <- DepMapTCGAMakeWideDF(df = ge_depmap_melt, output_type = "average", data_source = "DepMap")
...
# Make wide
ge_tcga_wide <- DepMapTCGAMakeWideDF(df = ge_tcga_melt, output_type = "average", data_source = "TCGA")

# Correlate all against all
ge_corr_wide_pearson <- DepMapTCGACorrMatrix(DepMap_wide_df = ge_depmap_wide, TCGA_wide_df = ge_tcga_wide, corr_method = "pearson")
ge_corr_wide_spearman <- DepMapTCGACorrMatrix(DepMap_wide_df = ge_depmap_wide, TCGA_wide_df = ge_tcga_wide, corr_method = "spearman")
```
...
ge_ht_s <- DepMapTCGACorrHeatmap(corr_wide_df = ge_corr_wide_spearman, out_filepath = "./figures/ge_depmap_tcga_heatmap_spearman.png", legend_title = "Expression Spearman correlation", color_breaks = c(-0.5, 0, 0.5, 1))
# draw(ge_ht_s, heatmap_legend_side = "top")
```
...
# Make wide
mut_depmap_wide <- DepMapTCGAMakeWideDF(df = mut_depmap_melt, output_type = "proportion", data_source = "DepMap", exclusions = c("TP53"))
...
# Make wide
mut_tcga_wide <- DepMapTCGAMakeWideDF(df = mut_tcga_melt, output_type = "proportion", data_source = "TCGA", exclusions = c("TP53"))

# Correlate all against all
mut_corr_wide_pearson <- DepMapTCGACorrMatrix(DepMap_wide_df = mut_depmap_wide, TCGA_wide_df = mut_tcga_wide, corr_method = "pearson")
mut_corr_wide_spearman <- DepMapTCGACorrMatrix(DepMap_wide_df = mut_depmap_wide, TCGA_wide_df = mut_tcga_wide, corr_method = "spearman")
```
...
mut_ht_p <- DepMapTCGACorrHeatmap(corr_wide_df = mut_corr_wide_pearson, out_filepath = "./figures/mut_depmap_tcga_heatmap_pearson_252.png", legend_title = "Mutation Pearson correlation", color_breaks = c(0, 0.3, 0.6, 0.9))
# draw(mut_ht_p, heatmap_legend_side = "top")

mut_ht_s <- DepMapTCGACorrHeatmap(corr_wide_df = mut_corr_wide_spearman, out_filepath = "./figures/mut_depmap_tcga_heatmap_spearman_252.png", legend_title = "Mutation Spearman correlation", color_breaks = c(0, 0.3, 0.5, 0.7))
# draw(mut_ht_s, heatmap_legend_side = "top")
...
# Make wide
cn_depmap_wide <- DepMapTCGAMakeWideDF(df = cn_depmap_melt, output_type = "average", data_source = "DepMap")
...
# Make wide
cn_tcga_wide <- DepMapTCGAMakeWideDF(df = cn_tcga_melt, output_type = "proportion", data_source = "TCGA")

# Correlate all against all
cn_corr_wide_pearson <- DepMapTCGACorrMatrix(DepMap_wide_df = cn_depmap_wide, TCGA_wide_df = cn_tcga_wide, corr_method = "pearson")
cn_corr_wide_spearman <- DepMapTCGACorrMatrix(DepMap_wide_df = cn_depmap_wide, TCGA_wide_df = cn_tcga_wide, corr_method = "spearman")
```
...
Make plots:
```{r cn_plots, eval=TRUE, fig.height=4, fig.width=3.75}
cn_ht_p <- DepMapTCGACorrHeatmap(corr_wide_df = cn_corr_wide_pearson, out_filepath = "./figures/cn_depmap_tcga_heatmap_pearson_thresholdedloss.png", legend_title = "Copy number Pearson correlation", color_breaks = c(0, 0.2, 0.4, 0.6))
# draw(cn_ht_p, heatmap_legend_side = "top")

cn_ht_s <- DepMapTCGACorrHeatmap(corr_wide_df = cn_corr_wide_spearman, out_filepath = "./figures/cn_depmap_tcga_heatmap_spearman_thresholdedloss.png", legend_title = "Copy number Spearman correlation", color_breaks = c(0, 0.2, 0.4, 0.6))
# draw(cn_ht_s, heatmap_legend_side = "top")
