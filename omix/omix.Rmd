...
**DO NOT RUN UNTIL ADDED TCGA TO CN AND GE DATASETS**
```{r cn_heatmap_data, eval=FALSE}
# TCGA copy number
...
cn_corr <- cn_wide %>% as.matrix %>% cor(method = "spearman") %>% as.data.frame %>% rownames_to_column(var = "var1") %>% gather(var2, value, -var1)
# Filter self-comparisons and eliminate duplicate correlations
...
# DepMap copy number (19Q1)
# Filter for genes and TCGA classifications in TCGA CN dataset
cn_filt <- filter(cn_melt, Hugo_Symbol %in% unique(cn_tcga_melt$Gene) & TCGA_classification %in% unique(cn_tcga_melt$TCGA_classification))
colnames(cn_filt)[colnames(cn_filt) == "Hugo_Symbol"] <- "Gene"
# Assign CN levels
cn_19Q1$CN <- with(cn_19Q1, ifelse(CN > 0.3, 1, ifelse(CN < -0.3, -1, 0)))
# Average CN levels per gene per TCGA classification
cn_19Q1_avg <- cn_19Q1 %>% group_by(TCGA_classification, Gene) %>% summarise(Mean_CN = mean(CN))
# Make wide
cn_19Q1_wide <- dcast(data = cn_19Q1_avg, formula = Gene ~ TCGA_classification, fun.aggregate = mean)
rownames(cn_19Q1_wide) <- cn_19Q1_wide$Gene
cn_19Q1_wide$Gene <- NULL
colnames(cn_19Q1_wide) <- paste(colnames(cn_19Q1_wide), "DepMap", sep = "_")

# Correlate all against all (19Q1)
cn_wide_19Q1 <- bind_cols(cn_19Q1_wide, cn_tcga_wide)
cn_corr_19Q1 <- cn_wide_19Q1 %>% as.matrix %>% cor(method = "spearman") %>% as.data.frame %>% rownames_to_column(var = "var1") %>% gather(var2, value, -var1)
# Filter self-comparisons and eliminate duplicate correlations
cn_corr_19Q1 <- filter(cn_corr_19Q1, !(grepl("DepMap", var1) & grepl("DepMap", var2)) & !(grepl("TCGA", var1) & grepl("TCGA", var2)) & grepl("TCGA", var2))
# Data management
colnames(cn_corr_19Q1) <- c("DepMap", "TCGA", "r")
cn_corr_19Q1 <- data.frame(apply(cn_corr_19Q1, 2, function(x) gsub("_TCGA|_DepMap","", x)))
cn_corr_19Q1$r <- as.numeric(as.character(cn_corr_19Q1$r))
# Set up coprrelation dataframe for plotting
cn_corr_19Q1_wide <- dcast(cn_corr_19Q1, DepMap ~ TCGA)
rownames(cn_corr_19Q1_wide) <- cn_corr_19Q1_wide$DepMap
cn_corr_19Q1_wide$DepMap <- NULL
```

Pseudo-figure caption: This is comparison of thresholded copy number profiles between DepMap cancer cell lines and TCGA tumor samples, both classified by TCGA project. Levels are -1 for copuy number loss, 0 for neutral, and 1 for gain. In each dataset and for each TCGA classification, these numeric copy number levels across all genes shared between the two datasets were averaged, and the datasets were correlated against each other. Pearson correlations (r) between TCGA and DepMap copy number feature sets are shown in the heatmap.
...
cn_ht <- Heatmap(cn_corr_wide, 
                 ...
                 column_title = "TCGA tumor samples", 
                 column_title_side = "bottom")
# png(filename = "./figures/cn_depmap_tcga_heatmap_spearman.png", width = 7.5, height = 8, units = "in", res = 500)
draw(cn_ht, heatmap_legend_side = "top")
# dev.off()

# 19Q1
cn_19Q1_ht4order <- Heatmap(cn_corr_19Q1_wide, cluster_rows = FALSE)

cn_19Q1_ht <- Heatmap(cn_corr_19Q1_wide, 
                 name = "Expression correlation",
                 col = col_fun, 
                 rect_gp = gpar(col = "black"),
                 heatmap_legend_param = cn_ht_legend_list,
                 row_order = rev(column_order(cn_19Q1_ht4order)), 
                 show_column_dend = FALSE, 
                 cluster_rows = FALSE, 
                 row_names_side = "left",
                 row_title = "DepMap cancer cell lines",
                 column_title = "TCGA tumor samples", 
                 column_title_side = "bottom")
# png(filename = "./figures/cn_depmap_19Q1_tcga_heatmap_spearman.png", width = 7.5, height = 8, units = "in", res = 500)
draw(cn_19Q1_ht, heatmap_legend_side = "top")
# dev.off()
```
