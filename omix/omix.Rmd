...
# Average scaled TPM per gene per TCGA classification
cn_depmap_avg <- cn_depmap_melt %>% group_by(TCGA_classification, Gene) %>% summarise(Mean_CN = mean(CN_scaled))
# Make wide
...
# Average scaled TPM per gene per TCGA classification
cn_tcga_avg <- cn_tcga_melt %>% group_by(TCGA_classification, Gene) %>% summarise(Mean_CN = mean(CN_scaled))
# Make wide
...
# cn_corr <- filter(cn_corr, DepMap %in% tcga_classifications_16 & TCGA %in% tcga_classifications_16)
# Set up coprrelation dataframe for plotting
...
Thresholded CN estimate data wrangling and processing:
```{r cn_heatmap_data_thresholded, eval=FALSE, warning=FALSE}
# TCGA copy number
...
col_fun <- colorRamp2(c(-0.5, 0, 0.25, 0.75), c("black", "red3", "yellow", "white"))
...
cn_ht <- Heatmap(cn_corr_wide, 
                 ...
                 column_title = "TCGA tumor samples", 
                 column_title_side = "bottom")
# png(filename = "./figures/cn_depmap_tcga_heatmap_pearson_continuous.png", width = 7.5, height = 8, units = "in", res = 500)
...
# Calce proportion mutated per gene per TCGA classification
...
mut_tcga_wide <- mut_tcga_wide[mut_tcga_wide$Gene != "TP53", ]
...
mut_depmap_wide <- mut_depmap_wide[mut_depmap_wide$Gene != "TP53", ]
...
mut_ht <- Heatmap(mut_corr_wide,
                 ...
                 column_title = "TCGA tumor samples", 
                 column_title_side = "bottom")
# png(filename = "./figures/mut_depmap_tcga_heatmap_spearman_252.png", width = 7.5, height = 8, units = "in", res = 500)
...
```{r cn_scatters_data, eval=FALSE}
# For both datasets: CN_Status encoded as 1 is non-neutral (-1 or +1) and 0 is neutral

# TCGA
# Calc proportion of non-neutral CN per gene per TCGA classification
cn_tcga_prop <- cn_tcga_melt
cn_tcga_prop$CN_Status <- with(cn_tcga_prop, ifelse(CN != 0, 1, CN))
cn_tcga_prop <- cn_tcga_prop %>% group_by(TCGA_classification, Gene) %>% summarise(Proportion_NonNeutral_CN = sum(CN_Status) / n(), N = n())

# DepMap
# Calc proportion of non-neutral CN per gene per TCGA classification
cn_depmap_prop <- cn_depmap_melt
cn_depmap_prop$CN_Status <- with(cn_depmap_prop, ifelse(CN != 0, 1, CN))
cn_depmap_prop <- cn_depmap_prop %>% group_by(TCGA_classification, Gene) %>% summarise(Proportion_NonNeutral_CN = sum(CN_Status) / n(), N = n())

# Merge non-neutral CN proportion dataframes for plotting
cn_props <- merge(cn_depmap_prop, cn_tcga_prop, by = c("TCGA_classification", "Gene"), all = TRUE, suffixes = c("_DepMap","_TCGA"))
```

Pseudo-caption: This is a comparison of proportions of non-neutral copy number samples between cancer cell lines and TCGA samples in our 16 TCGA classification categories. Each point corresponds to a single gene in the associated cancer type. For these plots, thresholded copy number levels of -1 (loss) and +1 (gain) were recoded as "non-neutral."

```{r cn_scatters, fig.height=8, fig.width=8, eval=FALSE}
cn_props_scatters <- ggplot(data = cn_props, aes(x = Proportion_NonNeutral_CN_TCGA, y = Proportion_NonNeutral_CN_DepMap)) +
  ...
  # geom_text_repel(data = subset(cn_props, Proportion_NonNeutral_CN_TCGA > 0.4 & Proportion_NonNeutral_CN_DepMap > 0.6), aes(x = Proportion_NonNeutral_CN_DepMap, y = Proportion_NonNeutral_CN_DepMap, label = Gene), size = 3) +
  labs(x = "TCGA non-neutral CN frequency", y = "DepMap non-neutral CN frequency", caption = paste0(formatC(x = length(unique(cn_props$Gene)), format = "d", big.mark = ","), " genes compared."))

ggsave("./figures/cn_depmap_tcga_scatter_pearson.png", cn_props_scatters, device = "png", dpi = 450, width = 12, height = 12, units = "in")
```

### Mut

```{r mut_scatters_data, eval=FALSE}
# Merge mutation proportion dataframes for plotting
mut_props <- merge(mut_depmap_prop, mut_tcga_prop, by = c("TCGA_classification", "Gene"), all = TRUE, suffixes = c("_DepMap","_TCGA"))
```

Pseudo-caption: This is a comparison of proportions of mutated samples between cancer cell lines and TCGA samples in our 16 TCGA classification categories. Each point corresponds to a single gene in the associated cancer type.

```{r mut_scatters, fig.height=8, fig.width=8, eval=FALSE}
mut_props_scatters <- ggplot(data = mut_props, aes(x = Proportion_Mut_TCGA, y = Proportion_Mut_DepMap)) +
  facet_wrap(~ TCGA_classification, nrow = 4) +
  geom_point(alpha = 0.3, color = "darkcyan") +
  scale_y_continuous(breaks = seq(0, 1, by = 0.2), labels = seq(0, 1, by = 0.2)) +
  scale_x_continuous(breaks = seq(0, 1, by = 0.2), labels = seq(0, 1, by = 0.2)) +
  coord_cartesian(ylim = c(0, 1), xlim = c(0, 1)) +
  geom_smooth(method = "lm", se = FALSE, lwd = 0.5, color = "gainsboro") +
  stat_cor(method = "pearson", label.x = 0.85, label.y = 0.15, label.sep = "\n") +
  geom_text_repel(data = subset(mut_props, Proportion_Mut_TCGA > 0.3 & Proportion_Mut_DepMap > 0.3), aes(x = Proportion_Mut_TCGA, y = Proportion_Mut_DepMap, label = Gene), size = 3) +
  labs(x = "TCGA mutation frequency", y = "DepMap mutation frequency", caption = paste0(formatC(x = length(unique(mut_props$Gene)), format = "d", big.mark = ","), " genes compared."))

ggsave("./figures/mut_depmap_tcga_scatter_pearson.png", mut_props_scatters, device = "png", dpi = 450, width = 12, height = 12, units = "in")
```
