...
saveRDS(ge_melt, "./data_munging/rds/ge_melt_18Q3.rds", compress = "xz")
```

```{r}
ge_melt <- readRDS("./data_munging/rds/ge_melt_18Q3.rds")
```
...
saveRDS(shrna_data, "./data_munging/rds/shrna_data_18Q3.rds", compress = "xz")
```

```{r}
shrna_data <- readRDS("./data_munging/rds/shrna_data_18Q3.rds")
shrna_ccl <- data.frame("Broad_ID" = shrna_data$Broad_ID)
```

```{r, eval=FALSE}
write(union(as.character(distinct(crispr_data[which(is.na(crispr_data$lineage_name)), c("CCLE_Name", "lineage_name")])$CCLE_Name), as.character(distinct(shrna_data[which(is.na(shrna_data$lineage_name)), c("CCLE_Name", "lineage_name")])$CCLE_Name)), file = "./../../../Desktop/na_ccl_in_converter.txt")
```
...
saveRDS(shrna_signif_lineage, "./data_munging/rds/shrna_signif_lineage.rds")
```

```{r, eval=FALSE}
shrna_signif_lineage <- readRDS("./data_munging/rds/shrna_signif_lineage.rds")
```
