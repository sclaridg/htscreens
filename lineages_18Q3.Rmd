...
## D. Charytonowicz ell line converter

This comprehensive cancer cell line information curated by Daniel Charytonowicz.
```{r}
ccl_converter <- read.delim("./data_munging/cell_line_database_completed_extra_grouped_doids_FINAL.csv", sep = "\t", header = TRUE)
...
```{r}
ccl_converter_test <- read.delim("./data_munging/cell_line_database_FINAL_MASTER.tsv", sep = "\t", header = TRUE)
colnames(ccl_converter_test)[colnames(ccl_converter_test) == "ccle_cell_line_name"] <- "CCLE_Name"
colnames(ccl_converter_test)[colnames(ccl_converter_test) == "broad_id"] <- "Broad_ID"
```
...
old_dan_test <- select(ccl_converter_test, CCLE_Name, Broad_ID, cell_line_name)
old_dan_test <- old_dan_test[-which(old_dan_test$CCLE_Name == ""), ]
colnames(old_dan_test)[colnames(old_dan_test) == "Broad_ID"] <- "Broad_ID_Old"
ccl_test <- select(ccl_info, CCLE_Name, Broad_ID)

test <- merge(ccl_test, dan_test, by = "CCLE_Name", all = TRUE, suffixes = c("_DepMap", "_Daniel"))
test <- merge(test, old_dan_test, by = "CCLE_Name", all = TRUE)
test$BroadIDs_match_Daniel <- ifelse(as.character(test$Broad_ID_DepMap) == as.character(test$Broad_ID_Daniel), TRUE, FALSE)
test$BroadIDs_match_Old <- ifelse(as.character(test$Broad_ID_DepMap) == as.character(test$Broad_ID_Old), TRUE, FALSE)

test_false_Daniel <- filter(test, BroadIDs_match_Daniel == FALSE)
test_false_Old <- filter(test, BroadIDs_match_Old == FALSE)
# write.table(test_false, file = "~/Desktop/cell_line_database_problems.csv", quote = FALSE, sep = ",")
```
...
maf_df$Mutation_Status <- ifelse(maf_df$isDeleterious == TRUE, "Mutant", "Other")
maf_df$Mutation_Status_Nonsilent <- ifelse(maf_df$Variant_Classification == "Silent", "Other", "Mutant")
maf_df <- unique(subset(maf_df, select = c("Hugo_Symbol", "CCLE_Name", "Broad_ID", "Mutation_Status", "Mutation_Status_Nonsilent")))
...
write.table(crispr_signif, file = "~/Desktop/crispr_signif_gene.csv", quote = FALSE, sep = ",", row.names = FALSE)
```
...
saveRDS(crispr_signif_nonsilent, "./data_munging/rds/crispr_signif_nonsilent.rds")

write.table(crispr_signif_nonsilent, file = "~/Desktop/crispr_signif_nonsilent_gene.csv", quote = FALSE, sep = ",", row.names = FALSE)
```

```{r}
crispr_signif_nonsilent <- readRDS("./data_munging/rds/crispr_signif_nonsilent.rds")
...
```{r, eval=TRUE}
crispr_signif_lineage <- compare_means(Score ~ Mutation_Status, group.by = c("Hugo_Symbol", "group_general_lineage_name"), data = crispr_data, method = "wilcox.test", p.adjust.method = "BH")
...
write.table(crispr_signif_lineage, file = "~/Desktop/crispr_signif_lineage.csv", quote = FALSE, sep = ",", row.names = FALSE)
```
...
write.table(crispr_signif_tissue, file = "~/Desktop/crispr_signif_tissue.csv", quote = FALSE, sep = ",", row.names = FALSE)
```
