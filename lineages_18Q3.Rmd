...
# Select columns
maf_df <- subset(maf_raw, select = c("Hugo_Symbol", "CCLE_Name", "Broad_ID", "Variant_Classification", "isDeleterious", "Reference_Allele", "Tumor_Seq_Allele1"))
...
ccle_melt <- melt(ccle, id.vars = c("CID", "Drug"), measure.vars = colnames(ccle)[3:ncol(ccle)], variable.name = "accession_id", value.name = "AUC_CCLE")
...
ctrp_melt <- melt(ctrp, id.vars = c("CID", "Drug"), measure.vars = colnames(ctrp)[3:ncol(ctrp)], variable.name = "accession_id", value.name = "AUC_CTRP")
...
gdsc_melt <- melt(gdsc, id.vars = c("CID", "Drug"), measure.vars = colnames(gdsc)[3:ncol(gdsc)], variable.name = "accession_id", value.name = "AUC_GDSC")

# Merge all data
drug_data <- merge(merge(ccle_melt, ctrp_melt, by = c("accession_id", "CID", "Drug"), all = TRUE), gdsc_melt, by = c("accession_id", "CID", "Drug"), all = TRUE)
drug_data <- merge(drug_data, ccl_converter, by = "accession_id", all.x = TRUE)
drug_data <- merge(drug_data, maf_df, by = c("CCLE_Name", "Broad_ID"))
drug_data$Hugo_Symbol <- factor(drug_muts$Hugo_Symbol)

saveRDS(drug_data, "./../crispr_lineages_giant_files/drug_data_18Q3.rds", compress = "xz")
```
...
### Grouped by gene and lineage: Non-silent mutant vs. Other
...
crispr_signif_genes_lvlA <- as.character(filter(crispr_signif_lvlA, !is.na(p.signif.adj))$Hugo_Symbol)

knitr::kable(filter(crispr_signif_lvlA, p < 0.05)[, c("Hugo_Symbol", "p", "p.adj", "p.format", "p.signif", "p.signif.adj")], caption = "Wilcoxon test results for Level A point mutations, p < 0.01 (BH-adjusted p-values: * p <= 0.05, ** p <= 0.01, *** p <= 0.001, **** p <= 0.0001)") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```
...
genes_lvlA_filt <- c("ABL1", "ALK", "AR", "ATM", "BRAF", "BRCA1", "BRCA2", "CDKN2A", "DNMT3A", "DPYD", "EGFR", "ERBB2", "ERBB3", "FGFR2", "FLT3", "JAK2", "KIT", "KRAS", "MET", "NOTCH1", "NOTCH2", "NRAS", "PDGFRA", "PDGFRB", "PIK3CA", "PML", "PTCH1", "RET", "ROS1", "SF3B1", "SMO", "TP53", "TSC1")
...
crispr_ptmuts_lvlA_filt_plot <- ggplot(data = crispr_ptmuts_lvlA_filt, aes(x = Hugo_Symbol, y = Score)) +
  ...
  guides(color = FALSE) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.direction = "horizontal", legend.justification = c(0.5, 0), legend.position = c(0.092, 0.8), legend.box.margin = margin(c(10, 10, 10, 10))) +
  labs(fill = "Mutation Status", y = "CERES Score", x = "Gene")
crispr_ptmuts_lvlA_filt_plot
# ggsave("./plots_18Q3/manuscript/crispr_ptmuts_lvlA_filt_plot.png", crispr_ptmuts_lvlA_filt_plot, device = "png", width = 20, height = 6, units = "in")

crispr_ptmuts_lvlA_filt_plot_faceted <- ggplot(data = crispr_ptmuts_lvlA_filt, aes(x = Mutation_Status_Nonsilent, y = Score)) +
  facet_wrap(~ Hugo_Symbol, drop = FALSE, ncol = length(unique(genes_lvlA_filt))) +
  geom_hline(yintercept = 0, lty = 2, color = "darkgray") +
  geom_boxplot(mapping = aes(fill = Mutation_Status_Nonsilent), position = position_dodge(0.85)) +
  coord_cartesian(ylim = c(-3, 5)) +
  scale_fill_manual(values = crispr_ptmuts_lvlA_filt_color) +
  guides(color = FALSE) +
  geom_text(data = label_text, mapping = aes(x = 1.5, y = 5, label = p.signif.adj)) +
...
# ggsave("./plots_18Q3/manuscript/crispr_ptmuts_lvlA_filt_plot_faceted.png", crispr_ptmuts_lvlA_filt_plot_faceted, device = "png", width = 20, height = 5, units = "in")

crispr_ptmuts_lvlA_filt_plot_faceted_truncated <- ggplot(data = crispr_ptmuts_lvlA_filt, aes(x = Mutation_Status_Nonsilent, y = Score)) +
  facet_wrap(~ Hugo_Symbol, drop = FALSE, ncol = length(unique(genes_lvlA_filt))) +
  geom_hline(yintercept = 0, lty = 2, color = "darkgray") +
  coord_cartesian(ylim = c(-2.5, 2.5)) +
  geom_boxplot(aes(fill = Mutation_Status_Nonsilent), position = position_dodge(0.85)) +
  scale_fill_manual(values = crispr_ptmuts_lvlA_filt_color) +
  geom_text(data = label_text, mapping = aes(x = 1.5, y = 2.5, label = p.signif.adj)) +
  guides(color = FALSE) +
  theme(legend.direction = "horizontal", legend.justification = c(0.5, 0), legend.position = c(0.5, 0), legend.box.margin = margin(c(10, 10, 10, 10)), axis.ticks.x = element_blank(), axis.text.x = element_blank(), axis.title.x = element_blank()) +
  labs(fill = "Mutation Status", y = "CERES Score", x = "Mutation Status")
crispr_ptmuts_lvlA_filt_plot_faceted_truncated
# ggsave("./plots_18Q3/manuscript/crispr_ptmuts_lvlA_filt_plot_faceted_truncated.png", crispr_ptmuts_lvlA_filt_plot_faceted_truncated, device = "png", width = 20, height = 5, units = "in")
```
...
lvlA_signif_genes_plot <- ggplot(data = lvlA_genes_signif, aes(x = Mutation_Status_Nonsilent, y = Score)) +
  facet_wrap(~ Hugo_Symbol, nrow = 1, scales = "free_y") +
  ...
  theme(legend.position = "top", axis.text.x = element_text(angle = 65, hjust = 1, vjust = 1, size = 10)) +
  labs(y = "CERES Score",
       x = "Lineage",
       color = "Mutation Status",
       title = "CERES scores for significant Level A G2P genes",
       subtitle = "BH-adjusted p-values: * p <= 0.05, ** p <= 0.01, *** p <= 0.001, **** p <= 0.0001\nGenes are sorted by increasing p-value.")
lvlA_signif_genes_plot

# ggsave("./plots_18Q3/manuscript/crispr_g2p_lvlA_signif_genes_plot.png", lvlA_signif_genes_plot, width = 12, height = 5, units = "in")
```
...
```{r}
test <- '{"feature_names": "ALK positive", "tags": "test", "genes": "ALK"}'
