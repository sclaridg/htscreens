...
crispr_ptmuts_lvlA_plot_faceted <- ggplot(data = crispr_ptmuts_lvlA, aes(x = Mutation_Status_Nonsilent, y = Score)) +
  ...
  scale_fill_manual(values = crispr_ptmuts_lvlA_color) +
  guides(color = FALSE) +
  theme(legend.direction = "horizontal", legend.justification = c(0.5, 0), legend.position = c(0.092, 0.8), legend.box.margin = margin(c(10, 10, 10, 10)), axis.ticks.x = element_blank(), axis.text.x = element_blank(), axis.title.x = element_blank()) +
...
# label_text <- crispr_signif_nonsilent$p.signif.adj
# names(label_text) <- crispr_signif_nonsilent$Hugo_Symbol
# label_text <- label_text[genes_lvlA]
# 
# crispr_ptmuts_lvlA_plot_faceted_truncated <- ggplot(data = crispr_ptmuts_lvlA, aes(x = Mutation_Status_Nonsilent, y = Score, fill = Mutation_Status_Nonsilent)) +
#   facet_wrap(~ Hugo_Symbol, ncol = length(unique(crispr_ptmuts_lvlA$Hugo_Symbol))) +
#   geom_hline(yintercept = 0, lty = 2, color = "darkgray") +
#   coord_cartesian(ylim = c(-2, 2)) +
#   geom_boxplot(position = position_dodge(0.85)) +
#   scale_fill_manual(values = crispr_ptmuts_lvlA_color) +
#   geom_text(data = label_text, mapping = aes(label = p.signif.adj)) +
#   guides(color = FALSE) +
#   theme(legend.direction = "horizontal", legend.justification = c(0.5, 0), legend.position = c(0.092, 0.8), legend.box.margin = margin(c(10, 10, 10, 10)), axis.ticks.x = element_blank(), axis.text.x = element_blank(), axis.title.x = element_blank()) +
#   labs(fill = "Mutation Status", y = "CERES Score", x = "Mutation Status")
...
```{r}
g2p <- read.delim("./data_munging/g2p_full_level_A_dataset.tsv", sep = "\t", header = TRUE, encoding="UTF-8")
