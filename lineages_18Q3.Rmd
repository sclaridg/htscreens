...
ggplot(data = maf_annot, aes(x = primary_tissue, y = n)) +
  ...
  labs(title = "Distribution of mutations across cell lines and primary tissues", subtitle = "Each data point represents a single cell line.", x = "Primary Tissue", y = "Number of Mutations")
```

## Dependency probabilities

```{r fig.height=3, fig.width=6}
ggplot(data = crispr_data, aes(x = Dep_Prob)) +
...
```{r}
dep95 <- filter(crispr_data, Dep_Prob >= 0.95)
```


```{r fig.height=6, fig.width=8}
ggplot(data = dep95, aes(x = Dep_Prob)) +
  geom_histogram(mapping = aes(fill = primary_tissue), breaks = seq(0.95, 1, by = 0.005), color = "black", alpha = 0.7) +
