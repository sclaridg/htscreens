...
g2p$Direction_Curated <- ifelse(!is.na(str_match(string = tolower(g2p$Direction), pattern = "resist")[,1]), "resistance", ifelse(!is.na(str_match(string = tolower(g2p$Direction), pattern = "sensitive|sensitivity|responsive")[,1]), "sensitivity", "none"))
...
maf_changes$Keep <- ifelse(!is.na(str_match(string = maf_changes$Protein_Change, pattern = "[A-Z][0-9]+([A-Z]|[*])")[,1]), "Yes", "No")
maf_changes_df <- filter(maf_changes, Keep == "Yes")
maf_changes_df$Keep <- NULL
maf_changes_df$Gene_Protein_Change <- with(maf_changes_df, paste0(Gene, ":", Protein_Change))
saveRDS(maf_changes_df, "./data_munging/19Q2/maf_changes_df_19Q2.rds", compress = "xz")
...
g2p_mut_4merge$Direction_Numeric <- as.numeric(g2p_mut_4merge$Direction_Curated)
...
# ccle_signif_g2p_dg <- merge(ccle_signif_g2p_dg, g2p_mut_4merge, by = "Drug_Gene", all.x = TRUE)
# ccle_signif_g2p_dg$Direction_Numeric <- ifelse(is.na(ccle_signif_g2p_dg$Direction_Numeric), 4, as.numeric(ccle_signif_g2p_dg$Direction_Numeric))
...
# ctrp_signif_g2p_dg <- merge(ctrp_signif_g2p_dg, g2p_mut_4merge, by = "Drug_Gene", all.x = TRUE)
...
# gdsc_signif_g2p_dg <- merge(gdsc_signif_g2p_dg, g2p_mut_4merge, by = "Drug_Gene", all.x = TRUE)
