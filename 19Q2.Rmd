...
write.csv(ccl_meta, "./data_munging/19Q2/sample_info_edit_20190708.csv")
```
...
g2p_prot$Drug_Gene <- with(g2p_prot, paste0(Drug, "_", Gene))
...
```{r}
test <- g2p %>% drop_na(Drug_Gene, Direction_Curated) %>% count(Drug_Gene, Direction_Curated) %>% count(Drug_Gene) %>% filter(n > 1)
test_g2p <- filter(g2p, Drug_Gene %in% as.character(test$Drug_Gene))
```
...
write.csv.gz(fusions, "./data_munging/19Q2/CCLE_fusions_edit_20190708.csv.gz")
```
...
crispr_fus <- merge(merge(crispr, crispr_fus_grid, by = "DepMap_ID", all = TRUE), fusions_filt, by = c("DepMap_ID", "CCLE_name", "CCLE_histology", "Cell_line", "FusionPair"), all = TRUE)
crispr_fus$Fusion_Status <- ifelse(!is.na(crispr_fus$LeftGene), "Yes", "No")
crispr_fus$Drug_FusionPair <- ifelse(is.na(crispr_fus$Drug) | is.na(crispr_fus$FusionPair), NA, paste0(crispr_fus$Drug, "_", crispr_fus$FusionPair))
crispr_fus$G2P_Drug_FusionPair <- ifelse(crispr_fus$Drug_FusionPair %in% unique(as.character(g2p_fus$Drug_FusionPair)), "Yes", "No")
...
colnames(crispr_fus)[colnames(crispr_fus) == "AUC_IC50"] <- "Score"
...
# CCLE
crispr_prot_grid <- expand.grid("DepMap_ID" = unique(crispr$DepMap_ID), "Gene_Protein_Change" = unique(maf_changes_filt$Gene_Protein_Change))
crispr_prot <- merge(crispr, crispr_prot_grid, by = "DepMap_ID", all = TRUE)
...
MakeWilcoxonHeatmap <- function(signif_df, feature, row_names, col_title, color, legend_title, legend_breaks, row_title_txt, radius, na_color) {
  ...
  ht <- Heatmap(signif_grid,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  if(!is.na(str_match(string = tolower(feature), pattern = "prot"))) {
                    grid.text(signif_grid_annot1[i, j], x = x - width*0.20, y = y + height*0.07, gp = gpar(fontsize = 17))
                  }
