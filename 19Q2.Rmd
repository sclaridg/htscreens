...
crispr_ccls <- unique(crispr_ccls$DepMap_ID)
...
**RUN THIS WHEN CRISPR IS DONE MERGING (5/17)**
```{r histology_dists, fig.height=4.5, fig.width=4, eval=TRUE}
...
# CCLE
ccle_cor_res <- ccle %>% group_by(Drug, Gene, Drug_Gene) %>% summarize(
...
# CTRP
ctrp_cor_res <- ctrp %>% group_by(Drug, Gene, Drug_Gene) %>% summarize(
                GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate,
                GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value,
                CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate,
                CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value)
...
# GDSC
gdsc_cor_res <- gdsc %>% group_by(Drug, Gene, Drug_Gene) %>% summarize(
                GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate,
                GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value,
                CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate,
                CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value)
