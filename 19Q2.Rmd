...
ccle_signif_prot <- merge(ccle_signif_prot, unique(select(ccle_prot, Drug_Gene_Protein_Change, Direction_Curated)), by = "Drug_Gene_Protein_Change", all.x = TRUE)

ctrp_signif_prot <- readRDS("./data_munging/rds/ctrp_signif_protein_changes.rds")
ctrp_signif_prot <- merge(ctrp_signif_prot, unique(select(ctrp_prot, Drug_Gene_Protein_Change, Direction_Curated)), by = "Drug_Gene_Protein_Change", all.x = TRUE)

gdsc_signif_prot <- readRDS("./data_munging/rds/gdsc_signif_protein_changes.rds")
gdsc_signif_prot <- merge(gdsc_signif_prot, unique(select(gdsc_prot, Drug_Gene_Protein_Change, Direction_Curated)), by = "Drug_Gene_Protein_Change", all.x = TRUE)
```
...
signif_grid_annot <- dcast(signif_df, Gene_Protein_Change ~ Drug, value.var = "Direction_Curated", fill = 0)
%>% `rownames<-`(.[,1]) %>% select(-Gene_Protein_Change)
signif_grid_annot <- signif_grid_annot[, intersect(g2p_drug_order, colnames(signif_grid))]
signif_grid_annot <- signif_grid_annot[complete.cases(signif_grid_annot), ]
signif_grid_annot[signif_grid_annot == "No"] <- ""
signif_grid_annot[signif_grid_annot == "Yes"] <- "O"

test <- ctrp_signif_prot %>% count(Gene_Protein_Change, Drug)
```
...
ht_ctrp <- MakeWilcoxonHeatmap(signif_df = ctrp_signif_prot, feature = "protein changes", legend_title = "-log10(p-value)", legend_breaks = seq(0, 18, by = 3), row_names = TRUE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Genes", radius = 0.015)
