...
MergeOmicsG2P <- function(database, dataframe, maf_df, ge_df, cn_df, gene_list, outfile) {
  ...
  if(database == "CRISPR") {
    ...
    data <- select(dataframe, matches(paste(c("DepMap_ID", gene_list), collapse = "|")))
    data <- melt(dataframe, id.vars = "DepMap_ID", measure.vars = colnames(data)[2:ncol(data)], variable.name = "Drug", value.name = "Score")
    grid <- expand.grid("DepMap_ID" = unique(data$DepMap_ID), "Gene" = gene_list)
    data <- merge(data, grid, by = "DepMap_ID", all = TRUE)
    # Merge cell line metadata
    data <- merge(data, ccl_meta, by = "DepMap_ID", all.y = TRUE)
    data$Drug <- paste0("CRISPR_", data$Drug)
    data$Drug_Gene <- paste0(data$Drug, "_", data$Gene)
...
# test <- colnames(crispr_all)[colnames(crispr_all) %in% g2p_genes]
...
**10/15 MERGED CRISPR -- NEED TO TEST**
...
crispr_fus$Drug <- crispr_fus$Drug_Gene
...
crispr_all <- select(crispr_all, matches(paste(c("DepMap_ID", g2p_genes), collapse = "|")))
...
maf_changes_df <- readRDS("./data_munging/19Q2/maf_changes_df_19Q2.rds")
maf_changes_filt <- filter(maf_changes_df, Gene_Protein_Change %in% unique(as.character(g2p_prot$Gene_Protein_Change))) %>% select(-Gene)
...
crispr_prot <- merge(crispr_melt, crispr_prot_grid, by = "DepMap_ID", all = TRUE)
...
crispr_prot <- select(crispr_prot, -Protein_Change)
crispr_prot$Drug <- paste0("CRISPR_", crispr_prot$Gene)
...
saveRDS(crispr_prot, "./data_munging/19Q2/crispr_protein_changes.rds", compress = "xz")
```
...
crispr_signif_g2p_g <- add_column(crispr_signif_g2p_g, Drug = "CRISPR", .after = "Gene")
...
crispr_signif_lin <- add_column(crispr_signif_lin, Drug = "CRISPR", .after = "Gene")
...
crispr_prot <- unfactorize(crispr_prot)
crispr_signif_prot <- WilcoxonTestDF(dataframe = crispr_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change"), g2p_list = unique(as.character(g2p_prot$Drug_Gene_Protein_Change[!is.na(g2p_prot$Drug_Gene_Protein_Change)])), type = "protein_change", outfile = "./data_munging/rds/crispr_signif_protein_changes.rds")
# 10/9: Ran above line and got this error: Error in wilcox.test.default(xi, xj, paired = paired, ...) : not enough (finite) 'x' observations
crispr_signif_prot_lin <- WilcoxonTestDF(dataframe = crispr_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change", "Drug_Gene_Protein_Change_Lineage", "disease"), g2p_list = unique(as.character(g2p_prot$Drug_Gene_Protein_Change_Lineage[!is.na(g2p_prot$Drug_Gene_Protein_Change_Lineage)])), type = "protein_change_lineage", outfile = "./data_munging/rds/crispr_signif_protein_changes_lineage.rds")
...
```{r}
dataframe <- crispr_fus
grouping <- c("FusionPair", "Drug", "Drug_FusionPair")
...
crispr_signif_fus <- WilcoxonTestDF(dataframe = crispr_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair"), g2p_list = unique(as.character(g2p_fus$Drug_FusionPair[!is.na(g2p_fus$Drug_FusionPair)])), type = "fusion", outfile = "./data_munging/rds/crispr_signif_fusions.rds")
crispr_signif_fus_lin <- WilcoxonTestDF(dataframe = crispr_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair", "Drug_Fusion_Lineage"), g2p_list = unique(as.character(g2p_fus$Drug_Fusion_Lineage[!is.na(g2p_fus$Drug_Fusion_Lineage)])), type = "fusion_lineage", outfile = "./data_munging/rds/crispr_signif_fusions_lineage.rds")
...
**10/15 NEED TO DO CRISPR FUS BOXPLOTS**

```{r fusion_boxplots, fig.height=5, fig.width=12, eval=FALSE}
# CRISPR
# LOOK AT FUSION PREVALENCE
# crispr_fus_filt <- filter(crispr_fus, FusionPair %in% c("ABL1-BCR", "BCR-ABL1"))
# 
# crispr_fus_boxplot <- ggplot(data = crispr_fus_filt) +
#   facet_grid(FusionPair ~ Drug) +
...
#   geom_text(data = crispr_signif_fus_filt, aes(x = 1.5, y = 1.1, label = p.signif)) +
  ...
  ht <- Heatmap(signif_grid,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  ...
                  if(color == "darkgreen") {
                    ...
                    else { radius <- width*0.20 }
                  }
                  if(color == "turquoise4") {
                    if(!is.na(str_match(string = tolower(feature), pattern = "fus|prot"))) {
                      radius <- height*0.18
                    }
                    else { radius <- width*0.20 }
                  }
                  
                  grid.text(signif_grid_annot1[i, j], x = x + width*0.25, y = y + height*0.07, gp = gpar(fontsize = 17))
...
png(filename = "./plots/manuscript/all_mut_heatmap.png", width = 14, height = 9.5, units = "in", res = 500)
draw(ht_ccle2 + ht_ctrp2 + ht_gdsc2 + ht_crispr2, gap = unit(0.25, "in"), heatmap_legend_side = "top")
dev.off()
```

**10/15 NEED TO FIGURE OUT CRISPR HT BELOW**

Ran line 1565:
```
Error in seq_len(n%%length(x)) : 
  argument must be coercible to non-negative integer
```

```{r prot_heatmaps, eval=FALSE, fig.height=10, fig.width=3}
ht_crispr<- MakeWilcoxonHeatmap(signif_df = crispr_signif_prot_4plot, feature = "protein changes", legend_title = "-log10(p-value)", legend_breaks = seq(0, 15, by = 3), row_names = TRUE, color = "darkgreen", col_title = "Drugs", row_title_txt = "Gene:Protein Change")
...
crispr_spear_cn <- add_column(crispr_spear_cn, Drug = "CRISPR", .after = "Gene")
...
crispr_spear_ge <- add_column(crispr_spear_ge, Drug = "CRISPR", .after = "Gene")
...
```{r load_spear}
g2p_amp_crispr_4merge <- select(g2p_amp, Gene, Direction_Curated)
...
MakeCorrHeatmap <- function(cor_res, row_names, col_title, color_range, legend_title, metric, radius, crispr) {
  ...
  if(crispr == TRUE) {
    annot_grid <- dcast(cor_res, Gene ~ Drug, value.var = "Direction_Curated", fill = 0)
  ...
  if(crispr == FALSE) {
    annot_grid <- dcast(cor_res, Gene ~ Drug, value.var = "Direction_Curated", fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Gene)
...
# CRISPR
crispr_spear_cn <- unfactorize(filter(crispr_spear_cn, Gene %in% unique(g2p_amp$Gene)))
crispr_cn_ht <- MakeCorrHeatmap(drug_cor_res = crispr_spear_cn, row_names = TRUE, col_title = "CCLE", color_range = c(-0.2, 0, 0.2), radius = 0.1, legend_title = "CCLE Spearman correlation", metric = "CN")
