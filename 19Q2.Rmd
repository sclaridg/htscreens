...
```{r prot_wilcoxon_tests, eval=TRUE}
# CCLE
ccle_prot <- unfactorize(ccle_prot)
...
```{r fus_wilcoxon_tests, eval=TRUE}
...
crispr_signif_fus <- WilcoxonTestDF(dataframe = crispr_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair", "Drug_Gene"), type = "fusion", outfile = "./data_munging/rds/crispr_signif_fusions.rds")
crispr_signif_fus_lin <- WilcoxonTestDF(dataframe = crispr_fus, grouping = c("Drug_Gene", "FusionPair", "Drug", "Drug_FusionPair", "Drug_Fusion_Lineage"), type = "fusion_lineage", outfile = "./data_munging/rds/crispr_signif_fusions_lineage.rds")
```
...
```{r}
MakeDrugscreenSpearmanCorrDF <- function(dataset, df, metric, lineage) {
  df_filt <- filter(df, Drug %in% g2p_drugs & Gene %in% g2p_genes)
  df_filt$Gene <- factor(df_filt$Gene, ordered = FALSE)
  df_filt <- unfactorize(df_filt)

  if(metric == "GE") {
    if(lineage) {
      res <- df_filt %>% group_by(Drug, Gene, disease, Drug_Gene_Lin) %>% drop_na(TPM) %>%
    ...
    if(!lineage) {
      res <- df_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(TPM) %>%
        summarize(GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$estimate,
                  GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$p.value, 
                  GE_p_log10 = -log10(GE_p))
  ...
  if(metric == "CN") {
    if(lineage) {
      res <- df_filt %>% group_by(Drug, Gene, disease, Drug_Gene_Lin) %>% drop_na(CN) %>%
    ...
    if(!lineage) {
      res <- df_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(CN) %>%
        summarize(CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$estimate,
                  CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$p.value, 
                  CN_p_log10 = -log10(GE_p))
      res$InG2P <- ifelse(res$Drug_Gene %in% unique(as.character(g2p_expr$Drug_Gene[!is.na(g2p_expr$Drug_Gene)])),   "Yes", "No")
    }
  }
  res$Dataset <- dataset
...
# CRISPR
crispr_spear_cn <- crispr %>% group_by(Gene, disease) %>% drop_na(CN) %>%
...
crispr_spear_cn <- add_column(crispr_spear_cn, Drug_Gene_Lin = paste0(Drug_Gene, "_", disease), .before = "Drug")
saveRDS(crispr_spear_cn, "./data_munging/rds/crispr_cn_cor_lineage.rds")

crispr_spear_ge <- crispr %>% group_by(Gene, disease) %>% drop_na(TPM) %>%
...
crispr_spear_ge <- add_column(crispr_spear_ge, Drug_Gene_Lin = paste0(Drug_Gene, "_", disease), .before = "Drug")
saveRDS(crispr_spear_ge, "./data_munging/rds/crispr_ge_cor_lineage.rds")
...
cn_cor_lin <- rbind(ccle_cn_res_lin, ctrp_cn_res_lin, gdsc_cn_res_lin)
saveRDS(cn_cor_lin, "./data_munging/rds/drug_cn_cor_lineage.rds")
...
ge_cor_lin <- rbind(ccle_ge_res_lin, ctrp_ge_res_lin, gdsc_ge_res_lin)
saveRDS(ge_cor_lin, "./data_munging/rds/drug_ge_cor_lineage.rds")
```
