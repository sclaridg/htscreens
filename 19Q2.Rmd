...
g2p <- add_column(g2p, Mutation_Curated = NA, .after = "Mutation")
g2p$Mutation_Curated <- gsub("u'|'| |,|\\[|\\]", "", g2p$Mutation)
g2p$Mutation_Curated <- ifelse(g2p$Mutation_Curated == "", str_match(string = g2p$Feature.Names, pattern = "^[A-Z][0-9]+([A-Z]|[*])| [A-Z][0-9]+([A-Z]|[*])|:[A-Z][0-9]+([A-Z]|[*])")[,1], as.character(g2p$Mutation_Curated))
g2p$Mutation_Curated <- gsub(":| |c\\..*", "", g2p$Mutation_Curated)
g2p$Mutation_Curated <- ifelse(!is.na(str_match(string = tolower(g2p$Mutation_Curated), pattern = "exon|expression|frameshift|fusion|methylation|mutation")[,1]), NA, as.character(g2p$Mutation_Curated))
g2p$Mutation_Curated <- gsub("^([0-9]+)([A-Z])$", "\\2\\1\\2", g2p$Mutation_Curated)
g2p$Mutation_Curated <- gsub("^([A-Z])([0-9]+)$", "\\1\\2\\1", g2p$Mutation_Curated)
g2p <- add_column(g2p, Gene_Protein_Change = NA, .after = "Mutation_Curated")
g2p$Gene_Protein_Change <- ifelse(!is.na(g2p$Gene) & !is.na(g2p$Mutation_Curated), paste0(as.character(g2p$Gene), ":", as.character(g2p$Mutation_Curated)), NA)
...
Merge protein changes: **NEED TO MAKE G2P_prot DATAFRAME (6/4/19)**
...
ccle_prot_grid <- expand.grid("DepMap_ID" = unique(ccle_all$DepMap_ID), "Protein_Change" = unique(maf_changes_df$Protein_Change))
ccle_prot <- merge(merge(ccle_all, ccle_prot_grid, by = "DepMap_ID", all = TRUE), maf_changes_df, by = c("DepMap_ID", "CCLE_name", "CCLE_histology", "Cell_line", "Protein_Change"), all = TRUE)
ccle_prot$Protein_Change_Status <- ifelse(!is.na(ccle_prot$Protein_Change), "Yes", "No")
ccle_prot$Drug_Protein_Change <- ifelse(is.na(ccle_prot$Drug) | is.na(ccle_prot$Protein_Change), NA, paste0(ccle_prot$Drug, "_", ccle_prot$Protein_Change))
# ccle_prot$G2P_Drug_Protein_Change <- ifelse(ccle_prot$Drug_Protein_Change %in% unique(as.character(g2p_prot$Drug_Protein_Change)), "Yes", "No")
# ccle_prot$G2P_Protein_Change <- ifelse(ccle_prot$Protein_Change %in% unique(as.character(g2p_prot$Protein_Change)), "Yes", "No")
