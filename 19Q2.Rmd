...
g2p$Direction_Curated <- ifelse(!is.na(str_match(string = tolower(g2p$Direction), pattern = "resist")), "resistance", ifelse(!is.na(str_match(string = tolower(g2p$Direction), pattern = "sensitive|sensitivity|responsive")), "sensitivity", "none"))
...
```{r}
cgi <- read.delim("./../../../Desktop/cgi_biomarkers_latest/cgi_biomarkers_per_variant.tsv", sep = "\t", header = TRUE)
```
...
```{r wilcoxon_heatmap_function_EDIT, eval=FALSE}
MakeWilcoxonHeatmap <- function(signif_df, feature, row_names, col_title, color, legend_title, legend_breaks, row_title_txt, radius) {
  if(!is.na(str_match(string = tolower(feature), pattern = "mut"))) {
    ...
    signif_grid_annot <- dcast(signif_df, Gene ~ Drug, value.var = "Direction_Curated", fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Gene)
  ...
  if(!is.na(str_match(string = tolower(feature), pattern = "fus"))) {
    ...
    signif_grid_annot <- dcast(signif_df, FusionPair ~ Drug, value.var = "Direction_Curated", fill = 0) %>% `rownames<-`(.[,1]) %>% select(-FusionPair)
  ...
  ht <- Heatmap(signif_grid,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  ...
                  if(signif_grid_annot[i, j] == "none") {
                    grid.circle(x = x, y = y, r = radius, gp = gpar(fill = "transparent", col = "black"))
                  }
                  if(signif_grid_annot[i, j] == "sensitivity") {
                    grid.polygon(x = c(0, 0.5, 1), y = c(0, 1, 0), default.units = "npc", gp = gpar(fill = "transparent", col = "black"))
                  }
                  if(signif_grid_annot[i, j] == "resistance") {
                    grid.polygon(x = c(0.1, 0.1, 0.9, 0.9), y = c(0.1, 0.9, 0.9, 0.1), default.units = "npc", gp = gpar(fill = "transparent", col = "black"))
                  }
                }
                )
  return(ht)
...
grid.newpage()
grid.polygon(x = c(0.1, 0.1, 0.9, 0.9), y = c(0.1, 0.9, 0.9, 0.1), default.units = "npc",
             gp = gpar(), draw = TRUE, vp = NULL)

signif_df <- filter(ccle_signif_fus, Drug %in% g2p_drugs)
signif_grid <- dcast(signif_df, FusionPair ~ Drug, value.var = "p_log10", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-FusionPair)
signif_grid <- signif_grid[g2p_gene_order, intersect(g2p_drug_order, colnames(signif_grid))]
signif_grid <- signif_grid[complete.cases(signif_grid), ]
signif_grid[signif_grid < -log10(0.05)] <- NA
