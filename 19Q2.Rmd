...
MakeWilcoxonHeatmap <- function(signif_df, feature, row_names, col_title, color, legend_title, legend_breaks, row_title_txt, na_color) {
  ...
  if(!is.na(str_match(string = tolower(feature), pattern = "crispr")) & is.na(str_match(string = tolower(feature), pattern = "prot"))) {
  ...
  ht <- Heatmap(signif_grid,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  if(color == "darkorchid4") {
                    if(is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*0.18 }
                    if(!is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*0.16 }
                  }
                  if(color == "steelblue4") {
                    if(is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*0.12 }
                    if(!is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*0.16 }
                  }
                  if(color == "darkgreen") {
                    if(!is.na(str_match(string = tolower(feature), pattern = "fus"))) {
...
**10/23 STOPPED HERE: Was fixing annotation dot sizes on heatmaps**
