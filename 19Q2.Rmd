...
g2p_gl <- as.character(unique(g2p$Drug_Gene[!is.na(g2p$Gene_Lin)]))
g2p_drugs <- unique(sapply(strsplit(g2p_dgl, "_", fixed = TRUE), function(x) x[1]))
g2p_genes <- unique(sapply(strsplit(g2p_dgl, "_", fixed = TRUE), function(x) x[2]))
g2p_lins <- unique(sapply(strsplit(g2p_dgl, "_", fixed = TRUE), function(x) x[3]))
```
