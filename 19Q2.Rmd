...
write.csv(ccl_meta, "./data_munging/19Q2/sample_info_edit_20190718.csv")
```

```{r}
ccl_meta <- read.delim("./data_munging/19Q2/sample_info_edit_20190717.csv", sep = ",", header = TRUE, row.names = 1)
...
write.csv.gz(fusions, "./data_munging/19Q2/CCLE_fusions_edit_20190718.csv.gz")
```

```{r}
fusions <- read.delim("./data_munging/19Q2/CCLE_fusions_edit_20190718.csv.gz", header = TRUE, sep = ",")
```
...
crispr <- MergeOmicsG2P(database = "CRISPR", dataframe = crispr_all, maf_df = maf_df, ge_df = ge_melt, cn_df = cn_melt, gene_list = g2p_genes, outfile = "./../htscreens_giant_files/crispr_data_19Q2.rds")
# write.csv.gz(crispr %>% drop_na(Score), "./../htscreens_giant_files/crispr_data_19Q2.csv.gz")
```

```{r read_crispr_data}
crispr <- readRDS("./../htscreens_giant_files/crispr_data_19Q2.rds")
```

```{r merge_crispr_fus, eval=FALSE}
fusions_filt <- filter(fusions, FusionPair %in% unique(as.character(g2p_fus$FusionPair)))
crispr_fus_grid <- expand.grid("DepMap_ID" = unique(crispr$DepMap_ID), "FusionPair" = unique(fusions_filt$FusionPair))
crispr_fus <- merge(merge(crispr, crispr_fus_grid, by = "DepMap_ID", all = TRUE), fusions_filt, by = c("DepMap_ID", "Cell_line", "FusionPair", "disease"), all = TRUE)
crispr_fus$Fusion_Status <- ifelse(!is.na(crispr_fus$LeftGene), "Yes", "No")
crispr_fus$Drug <- "CRISPR"
...
saveRDS(crispr_fus, "./data_munging/rds/crispr_fusions.rds")
```
...
saveRDS(crispr_prot, "./data_munging/rds/crispr_protein_changes.rds")
```

```{r, eval = FALSE}
crispr_fus <- readRDS("./data_munging/rds/crispr_fusions.rds")
crispr_prot <- readRDS("./data_munging/rds/crispr_protein_changes.rds")
```
...
genes <- union(unique(ccle_signif_dgl$Gene), union(unique(ctrp_signif_dgl$Gene), unique(gdsc_signif_dgl$Gene)))
diseases <- union(unique(ccle_signif_dgl$disease), union(unique(ctrp_signif_dgl$disease), unique(gdsc_signif_dgl$disease)))

ccle_signif_dgl_4plot <- merge(merge(ccle_signif_dgl, unique(select(ccle_filt, Drug, Gene, disease)), by = c("Drug", "Gene", "disease"), all.x = TRUE), expand.grid("Drug" = unique(ccle_signif_dgl$Drug), "Gene" = genes, "disease" = diseases), by = c("Drug", "Gene", "disease"), all = TRUE)
ccle_signif_dgl_4plot$Database <- "CCLE"

ctrp_signif_dgl_4plot <- merge(merge(ctrp_signif_dgl, unique(select(ctrp_filt, Drug, Gene, disease)), by = c("Drug", "Gene", "disease"), all.x = TRUE), expand.grid("Drug" = unique(ctrp_signif_dgl$Drug), "Gene" = genes, "disease" = diseases), by = c("Drug", "Gene", "disease"), all = TRUE)
ctrp_signif_dgl_4plot$Database <- "CTRP"

gdsc_signif_dgl_4plot <- merge(merge(gdsc_signif_dgl, unique(select(gdsc_filt, Drug, Gene, disease)), by = c("Drug", "Gene", "disease"), all.x = TRUE), expand.grid("Drug" = unique(gdsc_signif_dgl$Drug), "Gene" = genes, "disease" = diseases), by = c("Drug", "Gene", "disease"), all = TRUE)
gdsc_signif_dgl_4plot$Database <- "GDSC"
```
...
# all_directions <- unique(rbind(select(ccle_prot, Gene_Protein_Change, Drug, Direction_Curated), select(ctrp_prot, Gene_Protein_Change, Drug, Direction_Curated), select(gdsc_prot, Gene_Protein_Change, Drug, Direction_Curated)))
...
MakeWilcoxonHeatmap <- function(signif_df, feature, row_names, col_title, color, legend_title, legend_breaks, row_title_txt, radius, na_color) {
  if(missing(na_color)) {
    na_color <- "white"
  }
...
ht_ccle2 <- MakeWilcoxonHeatmap(signif_df = ccle_signif_prot_4plot, feature = "protein changes", legend_title = "CCLE -log10(p-value)                                                              ", legend_breaks = seq(0, 15, by = 3), row_names = TRUE, color = "darkgreen", col_title = "", row_title_txt = "Gene:Protein Change", radius = 0.03, na_color = "black")
ht_ctrp2 <- MakeWilcoxonHeatmap(signif_df = ctrp_signif_prot_4plot, feature = "protein changes", legend_title = "CTRP -log10(p-value)                                                                               ", legend_breaks = seq(0, 15, by = 3), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Gene:Protein Change", radius = 0.015, na_color = "black")
ht_gdsc2 <- MakeWilcoxonHeatmap(signif_df = gdsc_signif_prot_4plot, feature = "protein changes", legend_title = "GDSC -log10(p-value)", legend_breaks = seq(0, 8, by = 2), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Gene:Protein Change", radius = 0.015, na_color = "black")
...
```{r}
MakeWilcoxonLineageHeatmap <- function(all_lin_df, lineage) {
  use_data <- filter(all_lin_df, Gene %in% unique(g2p_mut$Gene) & disease == lineage)
  ccle_df <- filter(use_data, Database == "CCLE")
  ctrp_df <- filter(use_data, Database == "CTRP")
  gdsc_df <- filter(use_data, Database == "GDSC")
  
  ht_ccle <- MakeWilcoxonHeatmap(signif_df = ccle_df, feature = "mutation", legend_title = "CCLE -log10(p-value)                                                                     ", legend_breaks = seq(0, 5, by = 1), row_names = TRUE, color = "darkgreen", col_title = "", row_title_txt = "Genes", radius = 0.03)
  
  ht_ctrp <- MakeWilcoxonHeatmap(signif_df = ctrp_df, feature = "mutation", legend_title = "CTRP -log10(p-value)                                                                    ", legend_breaks = seq(0, 12, by = 2), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Genes", radius = 0.008)
  
  ht_gdsc <- MakeWilcoxonHeatmap(signif_df = gdsc_df, feature = "mutation", legend_title = "GDSC -log10(p-value)", legend_breaks = seq(0, 8, by = 2), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Genes", radius = 0.015)
  
  ht <- ht_ccle + ht_ctrp + ht_gdsc
  return(ht)
}

SaveWilcoxonLineageHeatmap <- function(ht, outfile) {
  png(filename = outfile, width = 13, height = 9.5, units = "in", res = 500)
  draw(ht, gap = unit(0.25, "in"), heatmap_legend_side = "top")
  dev.off()
}
```

```{r, eval=FALSE}
diseases <- union(unique(ccle_signif_dgl_4plot$disease), union(unique(ctrp_signif_dgl_4plot$disease), unique(gdsc_signif_dgl_4plot$disease)))

drugscreens_all_signif_mut <- rbind(ccle_signif_dgl_4plot, ctrp_signif_dgl_4plot, gdsc_signif_dgl_4plot)

lin_hts <- lapply(diseases, MakeWilcoxonLineageHeatmap, all_lin_df = drugscreens_all_signif_mut)
lin_ht_paths <- paste0(lins, "_mut_heatmap.png")
pwalk(list(lin_ht_paths, lin_hts), SaveWilcoxonLineageHeatmap, outfile = "./plots/manuscript/lineage_heatmaps")
```
