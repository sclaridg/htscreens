...
colnames(ccl_converter)[colnames(ccl_converter) == "DepMap_ID"] <- "DepMap_ID"
...
```{r}
ccle_auc_grid <- dcast(filter(ccle_ptmuts, Drug %in% g2p_drugs), Drug ~ DepMap_ID, value.var = "Score", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Drug)
ctrp_auc_grid <- dcast(filter(ctrp_ptmuts, Drug %in% g2p_drugs), Drug ~ DepMap_ID, value.var = "Score", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Drug)
gdsc_auc_grid <- dcast(filter(gdsc_ptmuts, Drug %in% g2p_drugs), Drug ~ DepMap_ID, value.var = "Score", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Drug)
...
# ctrp_signif_gene <- WilcoxonTestDF(dataframe = ctrp, grouping = c("Drug", "Gene", "Drug_Gene"), outfile = "./data_munging/rds/ctrp_signif_g2p_gene.rds")
# ctrp_signif_g2p_druggene <- WilcoxonTestDF(dataframe = filter(ctrp, Drug %in% g2p_drugs & Gene %in% g2p_genes), grouping = c("Drug", "Gene", "Drug_Gene"), outfile = "./data_munging/rds/ctrp_signif_g2p_druggene.rds")
# ctrp_signif_lineage <- WilcoxonTestDF(dataframe = ctrp_filt, grouping = c("Drug", "Gene", "Primary.Disease", "Drug_Gene_Lin"), outfile = "./data_munging/rds/ctrp_signif_g2p_lineage.rds")
# 
# gdsc_filt <- filter(gdsc, Drug %in% g2p_drugs)
# gdsc_filt$Gene <- factor(gdsc_filt$Gene, ordered = FALSE)
# gdsc_filt <- unfactorize(gdsc_filt)
# gdsc_signif_gene <- WilcoxonTestDF(dataframe = gdsc, grouping = c("Drug", "Gene", "Drug_Gene"), outfile = "./data_munging/rds/gdsc_signif_g2p_gene.rds")
# gdsc_signsif_g2p_druggene <- WilcoxonTestDF(dataframe = filter(gdsc, Drug %in% g2p_drugs & Gene %in% g2p_genes), grouping = c("Drug", "Gene", "Drug_Gene"), outfile = "./data_munging/rds/gdsc_signif_g2p_druggene.rds")
# gdsc_signif_lineage <- WilcoxonTestDF(dataframe = gdsc_filt, grouping = c("Drug", "Gene", "Primary.Disease", "Drug_Gene_Lin"), outfile = "./data_munging/rds/gdsc_signif_g2p_lineage.rds")
```
