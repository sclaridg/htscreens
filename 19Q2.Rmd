...
```{r Figure3, fig.height=12, fig.width=12, eval=FALSE}
# A: Heatmap
ht_ccle2 <- MakeWilcoxonHeatmap(signif_df = filter(ccle_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "mutation", legend_title = "CCLE -log10(p-value)          ", legend_breaks = seq(0, 12, by = 3), row_names = FALSE, color = "darkgreen", col_title = "", row_title_txt = "Genes", radius = 0.023)
ht_ctrp2 <- MakeWilcoxonHeatmap(signif_df = filter(ctrp_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "mutation", legend_title = "CTRP -log10(p-value)          ", legend_breaks = seq(0, 18, by = 3), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Genes", radius = 0.007)
ht_gdsc2 <- MakeWilcoxonHeatmap(signif_df = filter(gdsc_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "mutation", legend_title = "GDSC -log10(p-value)          ", legend_breaks = seq(0, 5, by = 1), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Genes", radius = 0.011)
...
# B: CRISPR mutation status (agnostic)
crispr_mut_tb <- tableGrob(crispr_signif_g2p_g %>% filter(Gene %in% unique(g2p_mut$Gene), p < 0.05) %>% select(Gene, p.format, `n WT/mut`), rows = NULL)
crispr_mut_title <- textGrob("CRISPR", gp = gpar(fontsize = 20))
padding <- unit(0.5, "line")
crispr_mut_tb <- gtable_add_rows(crispr_mut_tb, heights = grobHeight(crispr_mut_title) + padding, pos = 0)
crispr_mut_tb <- gtable_add_grob(crispr_mut_tb, list(crispr_mut_title), t = 1, l = 1, r = ncol(crispr_mut_tb))

# C: Drug screen mutation status (agnostic)
...
mut_signif_tb <- tableGrob(rbind(ccle_mut_df, ctrp_mut_df, gdsc_mut_df), rows = NULL)
...
# D: CRISPR mutation status (lineage)
crispr_mut_lin_tb <- tableGrob(crispr_signif_lineage %>% filter(Gene %in% unique(g2p_mut$Gene), p < 0.05) %>% select(Gene, disease, p.format, `n WT/mut`), rows = NULL)
crispr_mut_lin_tb <- gtable_add_rows(crispr_mut_lin_tb, heights = grobHeight(crispr_mut_title) + padding, pos = 0)
crispr_mut_lin_tb <- gtable_add_grob(crispr_mut_lin_tb, list(crispr_mut_title), t = 1, l = 1, r = ncol(crispr_mut_lin_tb))

# E: Drug screen mutation status (lineage)
ccle_mut_lin_df <- ccle_signif_g2p_dgl %>% filter(InG2P == "Yes", p < 0.05) %>% select(Drug, Gene, disease, p.format, `n WT/mut`)
ccle_mut_lin_df <- add_column(ccle_mut_lin_df, .before = "Drug", Dataset = c("CCLE", rep("", nrow(ccle_mut_lin_df) - 1)))
ctrp_mut_lin_df <- ctrp_signif_g2p_dgl %>% filter(InG2P == "Yes", p < 0.05) %>% select(Drug, Gene, disease, p.format, `n WT/mut`)
ctrp_mut_lin_df <- add_column(ctrp_mut_lin_df, .before = "Drug", Dataset = c("CTRP", rep("", nrow(ctrp_mut_lin_df) - 1)))
gdsc_mut_lin_df <- gdsc_signif_g2p_dgl %>% filter(InG2P == "Yes", p < 0.05) %>% select(Drug, Gene, disease, p.format, `n WT/mut`)
gdsc_mut_lin_df <- add_column(gdsc_mut_lin_df, .before = "Drug", Dataset = c("GDSC", rep("", nrow(gdsc_mut_lin_df) - 1)))
mut_lin_signif_tb <- tableGrob(rbind(ccle_mut_lin_df, ctrp_mut_lin_df, gdsc_mut_lin_df), rows = NULL)
...
crispr_mut_lin_filt <- filter(crispr, Drug_Gene_Lin %in% c("CRISPR_BRAF_skin", "CRISPR_KRAS_lung"))
crispr_mut_lin_filt_plot <- ggplot(data = crispr_mut_lin_filt, mapping = aes(x = as.character(Mutation), y = Score)) +
  ...
  geom_boxplot(outlier.shape = NA, width = 0.07) +
  stat_compare_means(method = "wilcox", comparisons = list(c("0", "1"))) +
  labs(title = "CRISPR, skin", x = "Mutation Status")

# Make figure
A <- textGrob("A", hjust = 0, vjust = 1, gp = gpar(fontsize = 20, fontface = "bold"))
space <- textGrob("")
B <- textGrob("B", hjust = 0, vjust = 1, gp = gpar(fontsize = 20, fontface = "bold"))
C <- textGrob("C", hjust = 0, vjust = 1, gp = gpar(fontsize = 20, fontface = "bold"))
D <- textGrob("D", hjust = 0, vjust = 1, gp = gpar(fontsize = 20, fontface = "bold"))
E <- textGrob("E", hjust = 0, vjust = 1, gp = gpar(fontsize = 20, fontface = "bold"))
f <- textGrob("F", hjust = 0, vjust = 1, gp = gpar(fontsize = 20, fontface = "bold"))
...
png(filename = "./plots/manuscript/figure3.png", width = 25, height = 25, units = "in", res = 500)
grid.arrange(A, space,
             ht_mut_grob,
             B, C, D, E,
             crispr_mut_tb, mut_signif_tb, crispr_mut_lin_tb, mut_lin_signif_tb,
             f,
             arrangeGrob(ggplotGrob(crispr_mut_filt_plot),
                         ggplotGrob(ctrp_mut_filt_plot),
                         ggplotGrob(crispr_mut_lin_filt_plot),
                         nrow = 1),
             layout_matrix = lay)
dev.off()
```
