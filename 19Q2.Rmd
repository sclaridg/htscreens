...
MakeSpearmanCorrDF <- function(dataset, df, metric, lineage) {
  ...
  if(metric == "GE") {
    if(lineage) {
      df_count <- df %>% drop_na(TPM) %>% count(Drug, Gene, disease, Drug_Gene_Lin) %>% filter(n >= 3)
      df <- filter(df, Drug_Gene_Lin %in% df_count$Drug_Gene_Lin)
      res <- df %>% group_by(Drug, Gene, disease, Drug_Gene_Lin) %>%
  ...
  if(metric == "CN") {
    if(lineage) {
      df_count <- df %>% drop_na(CN) %>% count(Drug, Gene, disease, Drug_Gene_Lin) %>% filter(n >= 3)
      df <- filter(df, Drug_Gene_Lin %in% df_count$Drug_Gene_Lin)
      res <- df %>% group_by(Drug, Gene, disease, Drug_Gene_Lin) %>%
...
cn_cor <- rbind(crispr_spear_cn, ccle_cn_res, ctrp_cn_res, gdsc_cn_res)
saveRDS(cn_cor, "./data_munging/rds/cn_cor.rds")
...
saveRDS(ge_cor, "./data_munging/rds/ge_cor.rds")
```
