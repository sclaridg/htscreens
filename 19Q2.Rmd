...
crispr <- readRDS("./data_munging/19Q2/crispr_data_19Q2.rds") %>% drop_na(Gene)
crispr$Gene_Lin <- with(crispr, paste0(Gene, "_", disease))
crispr_unique <- unique(crispr)
saveRDS(crispr_unique, file = "./data_munging/19Q2/crispr_data_19Q2_unique.rds", compress = "xz")

ccle_unique <- unique(ccle)
saveRDS(ccle_unique, file = "./data_munging/19Q2/ccle_data_19Q2_unique.rds", compress = "xz")
gdsc_unique <- unique(gdsc)
saveRDS(gdsc_unique, file = "./data_munging/19Q2/gdsc_data_19Q2_unique.rds", compress = "xz")
ctrp_filt <- filter(ctrp, Drug %in% g2p_drugs)
ctrp_unique <- unique(ctrp_filt)
saveRDS(ctrp_unique, file = "./data_munging/19Q2/ctrp_data_19Q2_g2pdrugs_unique.rds", compress = "xz")
```

```{r}
crispr <- readRDS("./data_munging/19Q2/crispr_data_19Q2_unique.rds")
ccle <- readRDS("./data_munging/19Q2/ccle_data_19Q2_unique.rds")
ctrp <- readRDS("./data_munging/19Q2/gdsc_data_19Q2_unique.rds")
gdsc <- readRDS("./data_munging/19Q2/ctrp_data_19Q2_g2pdrugs_unique.rds")
```
...
g2p_list = unique(as.character(g2p_prot$Gene_Protein_Change[!is.na(g2p_prot$Gene_Protein_Change)]))
outfile = "./data_munging/rds/crispr_signif_protein_changes.rds"
...
g2p_munge <- select(g2p, disease)
g2p_munge <- g2p_munge %>% count(disease)
g2p_munge$Perc <- g2p_munge$n / sum(g2p_munge$n) * 100
g2p_munge <- g2p_munge[order(g2p_munge$Perc),]
g2p_munge$disease <- factor(g2p_munge$disease, levels = g2p_munge$disease)
ggplot(data = g2p_munge) +
  geom_bar(mapping = aes(x = disease, y = Perc), stat = "identity") +
  geom_text(aes(x = disease, y = Perc + 3, label = round(Perc, 1))) +
...
maf_g2p_summ_gene <- maf_g2p %>% group_by(Gene) %>% count(Mutation) %>% filter (Mutation == 1) %>% merge(unique(select(maf_g2p, DepMap_ID, Gene)) %>% count(Gene), by = "Gene", suffixes = c("_mut", "_cell_lines"), all = TRUE)
maf_g2p_summ_gene$PercCellLinesMutated <- maf_g2p_summ_gene$n_mut/maf_g2p_summ_gene$n_cell_lines * 100

maf_g2p_summ <- maf_g2p %>% group_by(disease) %>% count(Mutation) %>% filter (Mutation == 1) %>% merge(unique(select(maf_g2p, DepMap_ID, disease)) %>% count(disease), by = "disease", suffixes = c("_mut", "_cell_lines"), all = TRUE)
maf_g2p_summ$MutPerCCL <- maf_g2p_summ$n_mut/maf_g2p_summ$n_cell_lines

sum(maf_g2p_summ$n_cell_lines)
777/1588
sum(maf_g2p_summ$n_mut[!is.na(maf_g2p_summ$n_mut)])
2067/3151
```
...
```{r}
crispr_g2p_count <- filter(crispr_signif_g2p_g, Gene %in% unique(g2p_mut$Gene), InG2P == "Yes")
ccle_g2p_count <- filter(ccle_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene), InG2P == "Yes")
ctrp_g2p_count <- filter(ctrp_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene), InG2P == "Yes")
gdsc_g2p_count <- filter(gdsc_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene), InG2P == "Yes")
```

```{r mut_status_fig, fig.height=8, fig.width=15, eval=FALSE}
ht_ccle2 <- MakeWilcoxonHeatmap(signif_df = filter(ccle_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "mutation", legend_title = "CCLE -log10(p-value)", legend_breaks = seq(0, 12, by = 3), row_names = FALSE, color = "darkgreen", col_title = "", row_title_txt = "Genes", radius = 0.023)
ht_ctrp2 <- MakeWilcoxonHeatmap(signif_df = filter(ctrp_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "mutation", legend_title = "CTRP -log10(p-value)", legend_breaks = seq(0, 18, by = 3), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Genes", radius = 0.008)
ht_gdsc2 <- MakeWilcoxonHeatmap(signif_df = filter(gdsc_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "mutation", legend_title = "GDSC -log10(p-value)", legend_breaks = seq(0, 5, by = 1), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Genes", radius = 0.011)
ht_crispr2 <- MakeWilcoxonHeatmap(signif_df = filter(crispr_signif_g2p_g, Gene %in% unique(g2p_mut$Gene) & Mutation_1_count >= 3), feature = "crispr", legend_title = "CRISPR -log10(p-value)", legend_breaks = seq(0, 45, by = 9), row_names = TRUE, color = "darkorchid4", col_title = "", row_title_txt = "Genes", radius = 0.15)
...
crispr_signif_g2p_dg_summ <- filter(crispr_signif_g2p_g, InG2P == "Yes") 
...
crispr_mut_filt <- filter(crispr, Gene %in% filter(crispr_signif_g2p_dg_summ, p < 0.05)$Gene)
crispr_mut_filt_plot <- ggplot(data = crispr_mut_filt, aes(x = as.character(Mutation), y = Score) )+
  facet_wrap(~ Gene, ncol = 5) +
  geom_violin(aes(fill = as.character(Mutation))) +
  ...
  scale_fill_manual(name = "Mutation Status", labels = c("Wildtype", "Mutant"), values = c("1" = "darkorchid", "0" = "thistle")) +
  stat_compare_means(method = "wilcox", aes(label = paste0("p = ", ..p.format..)), label.x = 1.35, label.y = 1.1) +
  labs(title = "CRISPR", x = "Mutation Status", y = "CERES Score") +
...
ccle_mut_filt_plot <- ggplot(data = ccle_mut_filt, mapping = aes(x = as.character(Mutation), y = Score)) +
  facet_wrap(~ Drug_Gene, ncol = 5) +
  geom_violin(aes(fill = as.character(Mutation))) +
  geom_boxplot(outlier.shape = NA, width = 0.07) +
  scale_x_discrete(labels = c("0" = "Wildtype", "1" = "Mutant")) +
  scale_fill_manual(name = "Mutation Status", labels = c("Wildtype", "Mutant"), values = c("1" = "springgreen4", "0" = "palegreen3")) +
...
ctrp_mut_filt_plot <- ggplot(data = ctrp_mut_filt, mapping = aes(x = as.character(Mutation), y = Score)) +
  facet_wrap(~ Drug_Gene, ncol = 7) +
  geom_violin(aes(fill = as.character(Mutation))) +
  geom_boxplot(outlier.shape = NA, width = 0.07,) +
  scale_x_discrete(labels = c("0" = "Wildtype", "1" = "Mutant")) +
  scale_fill_manual(name = "Mutation Status", labels = c("Wildtype", "Mutant"), values = c("1" = "steelblue4", "0" = "slategray3")) +
  scale_y_continuous(breaks = seq(0, 1.00, by = 0.25), labels = c("0.00", "0.25", "0.50", "0.75", "1.00")) +
  coord_cartesian(ylim = c(0, 1.10)) +
  stat_compare_means(method = "wilcox", aes(label = paste0("p = ", ..p.format..)), label.x = 1.35, label.y = 1.1) +
...
gdsc_mut_filt_plot <- ggplot(data = gdsc_mut_filt, mapping = aes(x = as.character(Mutation), y = Score)) +
  ...
  geom_boxplot(outlier.shape = NA, width = 0.07, ) +
  scale_x_discrete(labels = c("0" = "Wildtype", "1" = "Mutant")) +
  scale_fill_manual(name = "Mutation Status", labels = c("Wildtype", "Mutant"), values = c("1" = "turquoise4", "0" = "paleturquoise3")) +
  scale_y_continuous(breaks = seq(0, 1.00, by = 0.25), labels = c("0.00", "0.25", "0.50", "0.75", "1.00")) +
  coord_cartesian(ylim = c(0, 1.10)) +
  stat_compare_means(method = "wilcox", aes(label = paste0("p = ", ..p.format..)), label.x = 1.35, label.y = 1.1) +
  labs(title = "GDSC", x = "Mutation Status", y = "AUC") +
  theme(legend.position = "none", axis.title.x = element_blank(),
        panel.background = element_rect(fill = "lightcyan", colour = "lightcyan", size = 0.5, linetype = "solid"),
        panel.grid.major = element_line(size = 0.5, linetype = 'solid', colour = "white"), 
        panel.grid.minor = element_line(size = 0.25, linetype = 'solid', colour = "white"))
...
gs_mut <- gList(arrangeGrob(ht_mut_grob, left = textGrob("A", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(crispr_mut_filt_plot), left = textGrob("B", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(ccle_mut_filt_plot), left = textGrob("C", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(ctrp_mut_filt_plot), left = textGrob("D", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(gdsc_mut_filt_plot), left = textGrob("E", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))))

png(filename = "./plots/manuscript_final/mutation_pancancer_small.png", width = 15, height = 17, units = "in", res = 100)
grid.arrange(layout_matrix = lay, grobs = gs_mut)
dev.off()

png(filename = "./plots/manuscript_final/mutation_pancancer.png", width = 15, height = 17, units = "in", res = 500)
grid.arrange(layout_matrix = lay, grobs = gs_mut)
dev.off()
```
...
crispr_cn_cor_4plot <- unfactorize(filter(cn_cor, Dataset == "CRISPR" & Gene %in% unique(g2p_amp$Gene)))
ccle_cn_cor_4plot <- unfactorize(filter(cn_cor, Dataset == "CCLE" & Gene %in% unique(g2p_amp$Gene)))
ctrp_cn_cor_4plot <- unfactorize(filter(cn_cor, Dataset == "CTRP" & Gene %in% unique(g2p_amp$Gene)))
gdsc_cn_cor_4plot <- unfactorize(filter(cn_cor, Dataset == "GDSC" & Gene %in% unique(g2p_amp$Gene)))
crispr_cn_ht2 <- MakeCorrHeatmap(cor_res = crispr_cn_cor_4plot, row_names = TRUE, heatmap_legend = TRUE, col_title = "CRISPR", color_range = c(-0.25, 0, 0.25), radius = 0.15, legend_title = "Copy num. Spearman correlation", metric = "CN")
ccle_cn_ht2 <- MakeCorrHeatmap(cor_res = ccle_cn_cor_4plot, row_names = FALSE, heatmap_legend = FALSE, col_title = "CCLE", color_range = c(-0.25, 0, 0.25), radius = 0.025, legend_title = "Copy num. Spearman correlation", metric = "CN")
ctrp_cn_ht2 <- MakeCorrHeatmap(cor_res = ctrp_cn_cor_4plot, row_names = FALSE, heatmap_legend = FALSE, col_title = "CTRP", color_range = c(-0.25, 0, 0.25), radius = 0.025, legend_title = "Copy num. Spearman correlation", metric = "CN")
gdsc_cn_ht2 <- MakeCorrHeatmap(cor_res = gdsc_cn_cor_4plot, row_names = FALSE, heatmap_legend = FALSE, col_title = "GDSC", color_range = c(-0.25, 0, 0.25), radius = 0.025, legend_title = "Copy num. Spearman correlation", metric = "CN")
...
ht_cn_grob <- grid.grabExpr(draw(ht_cn_list, gap = unit(0.25, "in"), heatmap_legend_side = "top", annotation_legend_list = lgd, annotation_legend_side = "top", merge_legend = TRUE))

cn_cor$p.format <- with(cn_cor, ifelse(CN_p < 0.001, "p < 0.001", paste0("p = ", round(CN_p, digits = 3))))
crispr_cn_cor <- unfactorize(filter(cn_cor, Dataset == "CRISPR" & InG2P == "Yes"))
...
crispr_cn_cor_filt <- filter(crispr_cn_cor, CN_p < 0.05)
crispr_cn_filt <- filter(crispr, Gene %in% crispr_cn_cor_filt$Gene)
crispr_text <- data.frame(label = paste0("r = ", round(crispr_cn_cor_filt$CN_r, digits = 3), ",\n", crispr_cn_cor_filt$p.format), Gene = crispr_cn_cor_filt$Gene)
crispr_cn_filt_plot <- ggplot(data = crispr_cn_filt, aes(x = CN, y = Score) )+
  facet_wrap(~ Gene) +
  geom_point(fill = "darkorchid", pch = 21, size = 1, alpha = 0.7) +
  geom_text(data = crispr_text,
            mapping = aes(x = +Inf, y = +Inf, label = label,
            hjust = 1.1, vjust = 1.2)) +
  scale_y_continuous(breaks = seq(-3, 1, by = 0.5), labels = c("-3.0", "-2.5", "-2.0", "-1.5", "-1.0", "-0.5", "0.0", "0.5", "1.0")) +
  labs(title = "CRISPR", x = "Copy number (log2)", y = "CERES Score") +
...
ccle_cn_filt_plot <- ggplot(data = ccle_cn_filt, aes(x = CN, y = Score) )+
  ...
  geom_point(fill = "springgreen4", pch = 21, size = 1, alpha = 0.7) +
...
#   facet_wrap(~ Drug_Gene) +
#   geom_point(fill = "steelblue4", pch = 21, size = 1, alpha = 0.7) +
...
gdsc_cn_filt <- filter(gdsc, Drug_Gene %in% gdsc_cn_cor_filt$Drug_Gene)
gdsc_text <- data.frame(label = paste0("r = ", round(gdsc_cn_cor_filt$CN_r, digits = 3), ",\n", gdsc_cn_cor_filt$p.format), Drug_Gene = gdsc_cn_cor_filt$Drug_Gene)
gdsc_cn_filt_plot <- ggplot(data = gdsc_cn_filt, aes(x = CN, y = Score) )+
  facet_wrap(~ Drug_Gene) +
  geom_point(fill = "turquoise4", pch = 21, size = 1, alpha = 0.7) +
...
crispr_ge_cor_4plot <- unfactorize(filter(ge_cor, Dataset == "CRISPR" & Gene %in% unique(g2p_expr$Gene)))
ccle_ge_cor_4plot <- unfactorize(filter(ge_cor, Dataset == "CCLE" & Gene %in% unique(g2p_expr$Gene)))
ctrp_ge_cor_4plot <- unfactorize(filter(ge_cor, Dataset == "CTRP" & Gene %in% unique(g2p_expr$Gene)))
gdsc_ge_cor_4plot <- unfactorize(filter(ge_cor, Dataset == "GDSC" & Gene %in% unique(g2p_expr$Gene)))
crispr_ge_ht2 <- MakeCorrHeatmap(cor_res = crispr_ge_cor_4plot, row_names = TRUE, heatmap_legend = TRUE, col_title = "CRISPR", color_range = c(-0.5, 0, 0.5), radius = 0.155, legend_title = "Gene expr. Spearman correlation", metric = "GE")
ccle_ge_ht2 <- MakeCorrHeatmap(cor_res = ccle_ge_cor_4plot, row_names = FALSE, heatmap_legend = FALSE, col_title = "CCLE", color_range = c(-0.5, 0, 0.5), radius = 0.07, legend_title = "Gene expr. Spearman correlation", metric = "GE")
ctrp_ge_ht2 <- MakeCorrHeatmap(cor_res = ctrp_ge_cor_4plot, row_names = FALSE, heatmap_legend = FALSE, col_title = "CTRP", color_range = c(-0.5, 0, 0.5), radius = 0.07, legend_title = "Gene expr. Spearman correlation", metric = "GE")
gdsc_ge_ht2 <- MakeCorrHeatmap(cor_res = gdsc_ge_cor_4plot, row_names = FALSE, heatmap_legend = FALSE, col_title = "GDSC", color_range = c(-0.5, 0, 0.5), radius = 0.07, legend_title = "Gene expr. Spearman correlation", metric = "GE")
...
crispr_ge_cor <- unfactorize(filter(ge_cor, Dataset == "CRISPR" & InG2P == "Yes"))
...
crispr_ge_cor_filt <- filter(crispr_ge_cor, GE_p < 0.05)
crispr_ge_filt <- filter(crispr, Gene %in% crispr_ge_cor_filt$Gene)
crispr_text <- data.frame(label = paste0("r = ", round(crispr_ge_cor_filt$GE_r, digits = 3), ",\n", crispr_ge_cor_filt$p.format), Gene = crispr_ge_cor_filt$Gene)
crispr_ge_filt_plot <- ggplot(data = crispr_ge_filt, aes(x = TPM, y = Score) )+
  facet_wrap(~ Gene) +
  geom_point(fill = "darkorchid", pch = 21, size = 1, alpha = 0.7) +
  geom_text(data = crispr_text,
            mapping = aes(x = +Inf, y = +Inf, label = label,
            hjust = 1.1, vjust = 1.2)) +
  scale_y_continuous(breaks = seq(-3, 1, by = 0.5), labels = c("-3.0", "-2.5", "-2.0", "-1.5", "-1.0", "-0.5", "0.0", "0.5", "1.0")) +
  labs(title = "CRISPR", x = "Gene expression [log2(TPM)]", y = "CERES Score") +
  theme(legend.position = "none",
        panel.background = element_rect(fill = "snow1", colour = "snow1", size = 0.5, linetype = "solid"),
        panel.grid.major = element_line(size = 0.5, linetype = 'solid', colour = "white"), 
        panel.grid.minor = element_line(size = 0.25, linetype = 'solid', colour = "white"))
...
gs_cn_ge <- gList(arrangeGrob(ht_cn_grob, left = textGrob("A", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(crispr_cn_filt_plot), left = textGrob("B", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(ccle_cn_filt_plot), left = textGrob("C", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(gdsc_cn_filt_plot), left = textGrob("D", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ht_ge_grob, left = textGrob("E", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(crispr_ge_filt_plot), left = textGrob("F", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(ccle_ge_filt_plot), left = textGrob("G", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(ctrp_ge_filt_plot), left = textGrob("H", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))),
             arrangeGrob(ggplotGrob(gdsc_ge_filt_plot), left = textGrob("I", x = unit(1, "npc"), y = unit(.97, "npc"), gp = gpar(fontsize = 20, fontface = "bold"))))

png(filename = "./plots/manuscript_final/cn_ge_pancancer_small.png", width = 20, height = 15, units = "in", res = 100)
grid.arrange(layout_matrix = hlay, grobs = gs_cn_ge[select_grobs(hlay)])
dev.off()

png(filename = "./plots/manuscript_final/cn_ge_pancancer.png", width = 20, height = 15, units = "in", res = 500)
grid.arrange(layout_matrix = hlay, grobs = gs_cn_ge[select_grobs(hlay)])
dev.off()
```
...
Mutation:
```{r}
crispr_mut_lin <- filter(crispr_signif_lin, InG2P == "Yes")
...
```{r}
crispr_test <- filter(crispr, Gene_Lin == "PTCH1_skin")
ggplot(crispr_test, aes(x = as.character(Mutation), y = Score)) +
  geom_boxplot() +
  geom_point() +
  stat_compare_means(method = "wilcox", aes(label = paste0("p = ", ..p.format..)), label.x = 1.35)
```
