...
g2p_lineages$disease <- with(g2p_lineages, ifelse(!is.na(str_match(string = tolower(Phenotype.Family), pattern = "astrocytoma|central nervous system|glioma")[,1]) | !is.na(str_match(string = tolower(Phenotype.Description), pattern = "astrocytoma|glioblastoma|craniopharyngioma|medulloblastoma")[,1]), "central_nervous_system",
                                           ...
                                           ifelse(!is.na(str_match(string = tolower(Phenotype.Family), pattern = "lymphoma|bone marrow|hematologic|immune system|leukemia")[,1]) | !is.na(str_match(string = tolower(Phenotype.Description), pattern = "leukemia|lymphoproliferative|myelodisplasic|systemic mastocytosis|hyper eosinophilic|waldenström macroglobulinemia|langerhans")[,1]), "haematopoietic_and_lymphoid",
...
g2p <- unique(g2p)
write.csv(g2p, "./data_munging/g2p_db_edited_20190718.csv")
```
...
saveRDS(crispr_prot, "./../htscreens_giant_files/crispr_protein_changes.rds", compress = "xz")
```
...
WilcoxonTestDF <- function(dataframe, grouping, g2p_list, type, outfile) {
  ...
  if(tolower(type) == "drug_gene_lineage") {
    ...
    if(is.na(str_match(pattern = "crispr", string = outfile))) {
  ...
  if(tolower(type) == "fusion") {
    ...
    count_n <- dataframe %>% count(Drug_FusionPair, Fusion_Status) %>% filter(n >= 3)
    count_category <- count_n %>% count(Drug_FusionPair) %>% filter(n == 2)
  ...
  if(tolower(type) == "fusion_lineage") {
    ...
    count_n <- dataframe %>% count(Drug_Fusion_Lineage, Fusion_Status) %>% filter(n >= 3)
    count_category <- count_n %>% count(Drug_Fusion_Lineage) %>% filter(n == 2)
    keep <- intersect(count_n$Drug_Fusion_Lineage, count_category$Drug_Fusion_Lineage)
    dataframe <- filter(dataframe, Drug_Fusion_Lineage %in% keep)
    dataframe$Drug_Fusion_Lineage <- as.character(dataframe$Drug_Fusion_Lineage)
    signif <- compare_means(Score ~ Fusion_Status, group.by = grouping, data = dataframe, paired = FALSE, method = "wilcox.test", p.adjust.method = "BH")
    signif <- adj_signif(signif, alpha = 0.05)
    signif <- signif[order(signif$p),]
    signif$InG2P <- ifelse(signif$Drug_Fusion_Lineage %in% g2p_list, "Yes", "No")
  }
  if(tolower(type) == "protein_change") {
    ...
    count_n <- dataframe %>% count(Drug_Gene_Protein_Change, Change_Status) %>% filter(n >= 3)
    count_category <- count_n %>% count(Drug_Gene_Protein_Change) %>% filter(n == 2)
  ...
  if(tolower(type) == "protein_change_lineage") {
    ...
    count_n <- dataframe %>% count(Drug_Gene_Protein_Change_Lineage, Change_Status) %>% filter(n >= 3)
    count_category <- count_n %>% count(Drug_Gene_Protein_Change_Lineage) %>% filter(n == 2)
    keep <- intersect(count_n$Drug_Gene_Protein_Change_Lineage, count_category$Drug_Gene_Protein_Change_Lineage)
    dataframe <- filter(dataframe, Drug_Gene_Protein_Change_Lineage %in% keep)
    dataframe$Drug_Gene_Protein_Change_Lineage <- as.character(dataframe$Drug_Gene_Protein_Change_Lineage)
    signif <- compare_means(Score ~ Change_Status, group.by = grouping, data = dataframe, paired = FALSE, method = "wilcox.test", p.adjust.method = "BH")
    signif <- adj_signif(signif, alpha = 0.05)
    signif <- signif[order(signif$p),]
    signif$InG2P <- ifelse(signif$Drug_Gene_Protein_Change_Lineage %in% g2p_list, "Yes", "No")
  }
...
crispr_signif_lineage <- WilcoxonTestDF(dataframe = crispr, grouping =  c("Drug", "Gene", "disease", "Drug_Gene_Lin"), type = "drug_gene_lineage", outfile = "./data_munging/rds/crispr_signif_g2p_gl.rds")
...
crispr_prot <- unfactorize(crispr_prot)
crispr_signif_prot <- WilcoxonTestDF(dataframe = crispr_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change"), g2p_list = unique(as.character(g2p_prot$Gene_Protein_Change[!is.na(g2p_prot$Gene_Protein_Change)])), type = "protein_change", outfile = "./data_munging/rds/crispr_signif_protein_changes.rds")
crispr_signif_prot_lin <- WilcoxonTestDF(dataframe = crispr_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change", "Drug_Gene_Protein_Change_Lineage", "disease"), g2p_list = unique(as.character(g2p_prot$Gene_Protein_Change_Lineage[!is.na(g2p_prot$Gene_Protein_Change_Lineage)])), type = "protein_change_lineage", outfile = "./data_munging/rds/crispr_signif_protein_changes_lineage.rds")
...
ccle_signif_prot_lin <- WilcoxonTestDF(dataframe = ccle_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change", "Drug_Gene_Protein_Change_Lineage", "disease"), g2p_list = unique(as.character(g2p_prot$Drug_Gene_Protein_Change_Lineage[!is.na(g2p_prot$Drug_Gene_Protein_Change_Lineage)])), type = "protein_change_lineage", outfile = "./data_munging/rds/ccle_signif_protein_changes_lineage.rds")
...
ctrp_signif_prot_lin <- WilcoxonTestDF(dataframe = ctrp_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change", "Drug_Gene_Protein_Change_Lineage", "disease"), g2p_list = unique(as.character(g2p_prot$Drug_Gene_Protein_Change_Lineage[!is.na(g2p_prot$Drug_Gene_Protein_Change_Lineage)])), type = "protein_change_lineage", outfile = "./data_munging/rds/ctrp_signif_protein_changes_lineage.rds")
...
gdsc_signif_prot_lin <- WilcoxonTestDF(dataframe = gdsc_prot, grouping = c("Gene_Protein_Change", "Drug", "Drug_Gene_Protein_Change", "Drug_Gene_Protein_Change_Lineage", "disease"), g2p_list = unique(as.character(g2p_prot$Drug_Gene_Protein_Change_Lineage[!is.na(g2p_prot$Drug_Gene_Protein_Change_Lineage)])), type = "protein_change_lineage", outfile = "./data_munging/rds/gdsc_signif_protein_changes_lineage.rds")
```
...
crispr_signif_fus <- WilcoxonTestDF(dataframe = crispr_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair", "Drug_Gene"), g2p_list = unique(as.character(g2p_fus$Drug_FusionPair[!is.na(g2p_fus$Drug_FusionPair)])), type = "fusion", outfile = "./data_munging/rds/crispr_signif_fusions.rds")
crispr_signif_fus_lin <- WilcoxonTestDF(dataframe = crispr_fus, grouping = c("Drug_Gene", "FusionPair", "Drug", "Drug_FusionPair", "Drug_Fusion_Lineage"), g2p_list = unique(as.character(g2p_fus$Drug_Fusion_Lineage[!is.na(g2p_fus$Drug_Fusion_Lineage)])), type = "fusion_lineage", outfile = "./data_munging/rds/crispr_signif_fusions_lineage.rds")
...
ccle_signif_fus_lin <- WilcoxonTestDF(dataframe = ccle_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair", "Drug_Fusion_Lineage"), g2p_list = unique(as.character(g2p_fus$Drug_Fusion_Lineage[!is.na(g2p_fus$Drug_Fusion_Lineage)])), type = "fusion_lineage", outfile = "./data_munging/rds/ccle_signif_fusions_lineage.rds")
...
ctrp_signif_fus_lin<- WilcoxonTestDF(dataframe = ctrp_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair", "Drug_Fusion_Lineage"), g2p_list = unique(as.character(g2p_fus$Drug_Fusion_Lineage[!is.na(g2p_fus$Drug_Fusion_Lineage)])), type = "fusion_lineage", outfile = "./data_munging/rds/ctrp_signif_fusions_lineage.rds")
...
gdsc_signif_fus_lin <- WilcoxonTestDF(dataframe = gdsc_fus, grouping = c("FusionPair", "Drug", "Drug_FusionPair", "Drug_Fusion_Lineage"), g2p_list = unique(as.character(g2p_fus$Drug_Fusion_Lineage[!is.na(g2p_fus$Drug_Fusion_Lineage)])), type = "fusion_lineage", outfile = "./data_munging/rds/gdsc_signif_fusions_lineage.rds")
```
...
**10/29 NEED TO FIX CIRCLE ANNOTS ON FUSION/STEELBLUE4 HEATMAP**

```{r wilcoxon_heatmap_function}
MakeWilcoxonHeatmap <- function(signif_df, feature, row_names, col_title, color, legend_title, legend_breaks, row_title_txt, na_color) {
  if(missing(na_color)) {
    na_color <- "white"
  }
  ...
  ht <- Heatmap(signif_grid,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  if(color == "darkgreen") {
                    if(!is.na(str_match(string = tolower(feature), pattern = "fus"))) { radius <- height*0.16 }
                    if(!is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*0.3 }
                    else { radius <- width*0.17 }
                  }
                  if(color == "steelblue4") {
                    if(!is.na(str_match(string = tolower(feature), pattern = "fus"))) { radius <- height*0.5 }
                    if(!is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*0.16 }
                    else { radius <- width*0.18 }
                  }
                  if(color == "turquoise4") {
                    if(!is.na(str_match(string = tolower(feature), pattern = "fus|prot"))) { radius <- height*0.16 }
                    else { radius <- width*0.18 }
                  }
                  if(color == "darkorchid4") {
                    if(is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*3.3 }
                    if(!is.na(str_match(string = tolower(feature), pattern = "prot"))) { radius <- height*2 }
                  }
...
# ht_ctrp <- MakeWilcoxonHeatmap(signif_df = filter(ctrp_signif_fus, Drug %in% g2p_drugs), feature = "fusion", legend_title = "-log10(p-value)", legend_breaks = seq(0, 10, by = 2), row_names = TRUE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Fusion Pairs")
...
# dev.off()
# 
# ht_ctrp <- MakeWilcoxonHeatmap(signif_df = ctrp_signif_prot_4plot, feature = "protein changes", legend_title = "-log10(p-value)", legend_breaks = seq(0, 15, by = 3), row_names = TRUE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Gene:Protein Change")
...
ht_crispr <- MakeWilcoxonHeatmap(signif_df = filter(crispr_signif_g2p_g, Gene %in% unique(g2p_mut$Gene)), feature = "crispr", legend_title = "-log10(p-value)", legend_breaks = seq(0, 45, by = 9), row_names = TRUE, color = "darkorchid4", col_title = "", row_title_txt = "Genes")
...
ht_ccle <- MakeWilcoxonHeatmap(signif_df = filter(ccle_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene)), feature = "mutation", legend_title = "-log10(p-value)", legend_breaks = seq(0, 12, by = 3), row_names = TRUE, color = "darkgreen", col_title = "Drugs", row_title_txt = "Genes")
...
ht_ctrp <- MakeWilcoxonHeatmap(signif_df = filter(ctrp_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene)), feature = "mutation", legend_title = "-log10(p-value)", legend_breaks = seq(0, 18, by = 3), row_names = TRUE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Genes")
png(filename = "./plots/manuscript/ctrp_mut_heatmap.png", width = 9.5, height = 10, units = "in", res = 500)
draw(ht_ctrp, heatmap_legend_side = "top")
dev.off()

ht_gdsc <- MakeWilcoxonHeatmap(signif_df = filter(gdsc_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene)), feature = "mutation", legend_title = "-log10(p-value)", legend_breaks = seq(0, 5, by = 1), row_names = TRUE, color = "turquoise4", col_title = "Drugs", row_title_txt = "Genes")
...
ht_ccle2 <- MakeWilcoxonHeatmap(signif_df = filter(ccle_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene)), feature = "mutation", legend_title = "CCLE -log10(p-value)          ", legend_breaks = seq(0, 12, by = 3), row_names = TRUE, color = "darkgreen", col_title = "", row_title_txt = "Genes")
ht_ctrp2 <- MakeWilcoxonHeatmap(signif_df = filter(ctrp_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene)), feature = "mutation", legend_title = "CTRP -log10(p-value)          ", legend_breaks = seq(0, 18, by = 3), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Genes")
ht_gdsc2 <- MakeWilcoxonHeatmap(signif_df = filter(gdsc_signif_g2p_dg, Gene %in% unique(g2p_mut$Gene)), feature = "mutation", legend_title = "GDSC -log10(p-value)          ", legend_breaks = seq(0, 5, by = 1), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Genes")
ht_crispr2 <- MakeWilcoxonHeatmap(signif_df = filter(crispr_signif_g2p_g, Gene %in% unique(g2p_mut$Gene)), feature = "crispr", legend_title = "CRISPR -log10(p-value)", legend_breaks = seq(0, 45, by = 9), row_names = FALSE, color = "darkorchid4", col_title = "", row_title_txt = "Genes")
...
png(filename = "./plots/manuscript/all_mut_heatmap.png", width = 14, height = 8, units = "in", res = 500)
draw(ht_ccle2 + ht_ctrp2 + ht_gdsc2 + ht_crispr2, gap = unit(0.25, "in"), heatmap_legend_side = "top")
dev.off()
```

```{r prot_heatmaps, eval=FALSE, fig.height=10, fig.width=3}
ht_crispr <- MakeWilcoxonHeatmap(signif_df = crispr_signif_prot_4plot, feature = "protein changes CRISPR", legend_title = "-log10(p-value)", legend_breaks = seq(0, 25, by = 5), row_names = TRUE, color = "darkorchid4", col_title = "", row_title_txt = "Gene:Protein Change")
png(filename = "./plots/manuscript/crispr_prot_heatmap.png", width = 3, height = 5.5, units = "in", res = 500)
draw(ht_crispr, heatmap_legend_side = "top")
dev.off()

ht_ccle <- MakeWilcoxonHeatmap(signif_df = ccle_signif_prot_4plot, feature = "protein changes", legend_title = "-log10(p-value)", legend_breaks = seq(0, 15, by = 3), row_names = TRUE, color = "darkgreen", col_title = "Drugs", row_title_txt = "Gene:Protein Change")
...
ht_gdsc <- MakeWilcoxonHeatmap(signif_df = gdsc_signif_prot_4plot, feature = "protein changes", legend_title = "-log10(p-value)", legend_breaks = seq(0, 8, by = 2), row_names = TRUE, color = "turquoise4", col_title = "Drugs", row_title_txt = "Gene:Protein Change")
png(filename = "./plots/manuscript/gdsc_prot_heatmap.png", width = 5, height = 5.5, units = "in", res = 500)
draw(ht_gdsc, heatmap_legend_side = "top")
dev.off()

ht_ccle2 <- MakeWilcoxonHeatmap(signif_df = ccle_signif_prot_4plot, feature = "protein changes", legend_title = "CCLE -log10(p-value)          ", legend_breaks = seq(0, 15, by = 3), row_names = TRUE, color = "darkgreen", col_title = "", row_title_txt = "Gene:Protein Change", na_color = "black")
ht_ctrp2 <- MakeWilcoxonHeatmap(signif_df = ctrp_signif_prot_4plot, feature = "protein changes", legend_title = "CTRP -log10(p-value)          ", legend_breaks = seq(0, 15, by = 3), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Gene:Protein Change", na_color = "black")
ht_gdsc2 <- MakeWilcoxonHeatmap(signif_df = gdsc_signif_prot_4plot, feature = "protein changes", legend_title = "GDSC -log10(p-value)          ", legend_breaks = seq(0, 8, by = 2), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Gene:Protein Change", na_color = "black")
ht_crispr2 <- MakeWilcoxonHeatmap(signif_df = crispr_signif_prot_4plot, feature = "protein changes CRISPR", legend_title = "CRISPR -log10(p-value)", legend_breaks = seq(0, 25, by = 5), row_names = FALSE, color = "darkorchid4", col_title = "", row_title_txt = "Gene:Protein Change", na_color = "black")
...
**10/25 Need to fix InG2P annots on CTRP data**

```{r fus_heatmaps, eval=FALSE}
ht_crispr <- MakeWilcoxonHeatmap(signif_df = filter(crispr_signif_fus, Drug_Gene == "CRISPR_ABL1" & FusionPair %in% c("BCR-ABL1", "ABL1-BCR")), feature = "fusion crispr", legend_title = "-log10(p-value)",  legend_breaks = seq(0, 2, by = 0.5), row_names = TRUE, color = "darkorchid4", col_title = "Drugs", row_title_txt = "Fusion Pairs")
png(filename = "./plots/manuscript/crispr_fus_heatmap.png", width = 3, height = 3, units = "in", res = 500)
draw(ht_crispr, heatmap_legend_side = "top")
dev.off()

ht_ccle <- MakeWilcoxonHeatmap(signif_df = filter(ccle_signif_fus, Drug %in% g2p_drugs), feature = "fusion", legend_title = "-log10(p-value)",  legend_breaks = seq(0, 4, by = 1), row_names = TRUE, color = "darkgreen", col_title = "Drugs", row_title_txt = "Fusion Pairs")
...
ht_gdsc <- MakeWilcoxonHeatmap(signif_df = filter(gdsc_signif_fus, Drug %in% g2p_drugs), feature = "fusion", legend_title = "-log10(p-value)", legend_breaks = seq(0, 5, by = 1), row_names = TRUE, color = "turquoise4", col_title = "Drugs", row_title_txt = "Fusion Pairs")
png(filename = "./plots/manuscript/gdsc_fus_heatmap.png", width = 9, height = 3, units = "in", res = 500)
draw(ht_gdsc, heatmap_legend_side = "top")
dev.off()

ht_ccle2 <- MakeWilcoxonHeatmap(signif_df = filter(ccle_signif_fus, Drug %in% g2p_drugs), feature = "fusion", legend_title = "CCLE -log10(p-value)          ", legend_breaks = seq(0, 4, by = 1), row_names = TRUE, color = "darkgreen", col_title = "", row_title_txt = "Fusion Pairs")
ht_ctrp2 <- MakeWilcoxonHeatmap(signif_df = filter(ctrp_signif_fus, Drug %in% g2p_drugs), feature = "fusion", legend_title = "CTRP -log10(p-value)          ", legend_breaks = seq(0, 10, by = 2), row_names = FALSE, color = "steelblue4", col_title = "Drugs", row_title_txt = "Fusion Pairs")
ht_gdsc2 <- MakeWilcoxonHeatmap(signif_df = filter(gdsc_signif_fus, Drug %in% g2p_drugs), feature = "fusion", legend_title = "GDSC -log10(p-value)          ", legend_breaks = seq(0, 5, by = 1), row_names = FALSE, color = "turquoise4", col_title = "", row_title_txt = "Fusion Pairs")
ht_crispr2 <- MakeWilcoxonHeatmap(signif_df = filter(crispr_signif_fus, Drug_Gene == "CRISPR_ABL1" & FusionPair %in% c("BCR-ABL1", "ABL1-BCR")), feature = "fusion crispr", legend_title = "CRISPR -log10(p-value)",  legend_breaks = seq(0, 2, by = 0.5), row_names = FALSE, color = "darkorchid4", col_title = "", row_title_txt = "Fusion Pairs")
...
**10/23 MADE PLOTS UP TO HERE**

### Lineage

**10/23 NEED TO RUN LINEAGE PLOTS**
...
# CRISPR
crispr_spear_cn <- crispr %>% group_by(Gene) %>% drop_na(CN) %>%
  summarize(CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$estimate,
            CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "complete.obs")$p.value,
            CN_p_log10 = -log10(CN_p),
            InG2P = "No",
            Dataset = "CRISPR",
            Metric = "Copy Number")
crispr_spear_cn <- transform(crispr_spear_cn, Gene = reorder(Gene, CN_r))
crispr_spear_cn <- add_column(crispr_spear_cn, Drug = "CRISPR", .before = "Gene")
Drug_Gene <- with(crispr_spear_cn, paste0(Drug, "_", Gene))
crispr_spear_cn <- add_column(crispr_spear_cn, Drug_Gene = Drug_Gene, .before = "Drug")
saveRDS(crispr_spear_cn, "./data_munging/rds/crispr_cn_cor.rds")

crispr_spear_ge <- crispr %>% group_by(Gene) %>% drop_na(TPM) %>%
  summarize(GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$estimate,
            GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "complete.obs")$p.value,
            GE_p_log10 = -log10(GE_p),
            InG2P = "No",
            Dataset = "CRISPR",
            Metric = "Gene Expression")
crispr_spear_ge <- transform(crispr_spear_ge, Gene = reorder(Gene, GE_r))
crispr_spear_ge <- add_column(crispr_spear_ge, Drug = "CRISPR", .before = "Gene")
Drug_Gene <- with(crispr_spear_ge, paste0(Drug, "_", Gene))
crispr_spear_ge <- add_column(crispr_spear_ge, Drug_Gene = Drug_Gene, .before = "Drug")
saveRDS(crispr_spear_ge, "./data_munging/rds/crispr_ge_cor.rds")

# CCLE
ccle_filt <- filter(ccle, Drug %in% g2p_drugs & Gene %in% g2p_genes)
ccle_filt$Gene <- factor(ccle_filt$Gene, ordered = FALSE)
ccle_filt <- unfactorize(ccle_filt)
ccle_ge_res <- ccle_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(TPM) %>%
  summarize(GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$estimate,
...
ccle_cn_res <- ccle_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(CN) %>%
  summarize(CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$estimate,
...
ctrp_filt$Gene <- factor(ctrp_filt$Gene, ordered = FALSE)
ctrp_filt <- unfactorize(ctrp_filt)
ctrp_ge_res <- ctrp_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(TPM) %>%
  summarize(GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$estimate,
            GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$p.value,
            GE_p_log10 = -log10(GE_p))
...
ctrp_cn_res <- ctrp_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(CN) %>%
  summarize(CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$estimate,
            CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$p.value,
            CN_p_log10 = -log10(CN_p))
...
gdsc_filt$Gene <- factor(gdsc_filt$Gene, ordered = FALSE)
gdsc_filt <- unfactorize(gdsc_filt)
gdsc_ge_res <- gdsc_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(TPM) %>%
  summarize(GE_r = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$estimate,
            GE_p = cor.test(y = Score, x = TPM, method = "spearman", use = "na.or.complete")$p.value,
            GE_p_log10 = -log10(GE_p))
...
gdsc_cn_res <- gdsc_filt %>% group_by(Drug, Gene, Drug_Gene) %>% drop_na(CN) %>%
  summarize(CN_r = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$estimate,
            CN_p = cor.test(y = Score, x = CN, method = "spearman", use = "na.or.complete")$p.value,
            CN_p_log10 = -log10(CN_p))
...
# CRISPR
## CN
g2p_cn_crispr_4merge <- unique(select(g2p_amp, Gene, Direction_Curated))
crispr_cn_summ <- g2p_cn_crispr_4merge %>% count(Gene)
crispr_cn_summ <- merge(crispr_cn_summ, g2p_cn_crispr_4merge, by = "Gene")
crispr_cn_summ$Direction_Heatmap <- with(crispr_cn_summ, ifelse(n == 2, "both", ifelse(n == 1 & Direction_Curated == "resistance", "resistance", "sensitivity")))
crispr_cn_summ <- unique(select(crispr_cn_summ, Gene, Direction_Heatmap))
crispr_spear_cn <- readRDS("./data_munging/rds/crispr_cn_cor.rds")
crispr_cn_cor <- merge(crispr_spear_cn, crispr_cn_summ, by = "Gene", all.x = TRUE)
crispr_cn_cor$Metric <- NULL
## GE
g2p_ge_crispr_4merge <- unique(select(g2p_expr, Gene, Direction_Curated))
crispr_ge_summ <- g2p_ge_crispr_4merge %>% count(Gene)
crispr_ge_summ <- merge(crispr_ge_summ, g2p_ge_crispr_4merge, by = "Gene")
crispr_ge_summ$Direction_Heatmap <- with(crispr_ge_summ, ifelse(n == 2, "both", ifelse(n == 1 & Direction_Curated == "resistance", "resistance", "sensitivity")))
crispr_ge_summ <- unique(select(crispr_ge_summ, Gene, Direction_Heatmap))
crispr_spear_ge <- readRDS("./data_munging/rds/crispr_ge_cor.rds")
crispr_ge_cor <- merge(crispr_spear_ge, crispr_ge_summ, by = "Gene", all.x = TRUE)
crispr_ge_cor$Drug_Gene <- paste0(crispr_ge_cor$Drug, "_", crispr_ge_cor$Gene)
crispr_ge_cor$Metric <- NULL

all_cn_cor <- rbind(cn_cor, crispr_cn_cor)
all_ge_cor <- rbind(ge_cor, crispr_ge_cor)
```

## Plots

```{r crispr_spear_eda, warning=FALSE}
...
**10/28 FIX CRISPR PART OF corr_heatmap_function**
**11/11 Added CRISPR code to function, but R aborted due to misplaced bracket (fixed but did not run)**
...
MakeCorrHeatmap <- function(cor_res, row_names, col_title, color_range, legend_title, metric, radius) {
  if(toupper(metric) == "CN" | toupper(metric) == "COPY NUMBER") {
    if(col_title == "CRISPR") {
      ...
      signif_grid <- signif_grid[g2p_gene_order,]
      ...
      corr_grid <- corr_grid[g2p_gene_order,]
    }
    if(col_title != "CRISPR") {
      signif_grid <- dcast(cor_res, Gene ~ Drug, value.var = "CN_p", fun.aggregate = mean, fill = 0) %>% `rownames<-`(.[,1]) %>% select(-Gene)
      signif_grid <- signif_grid[g2p_gene_order, intersect(g2p_drug_order, colnames(signif_grid))]
  ...
  ht <- Heatmap(corr_grid,
                ...
                cell_fun = function(j, i, x, y, width, height, fill) {
                  ...
                  if(annot_grid[i, j] == "resistance") {
                    grid.circle(x = x - width*0.20, y = y + height*0.20, r = height*0.15, default.units = "npc", gp = gpar(fill = "red", col = "black"))
                  }
...
# Copy number
ccle_cn_cor <- unfactorize(filter(all_cn_cor, Dataset == "CCLE" & Gene %in% unique(g2p_amp$Gene)))
ccle_cn_ht <- MakeCorrHeatmap(cor_res = ccle_cn_cor, row_names = TRUE, col_title = "CCLE", color_range = c(-0.25, 0, 0.25), radius = 0.1, legend_title = "CCLE Spearman correlation", metric = "CN", crispr =  FALSE)
...
ctrp_cn_cor <- unfactorize(filter(all_cn_cor, Dataset == "CTRP" & Gene %in% unique(g2p_amp$Gene)))
ctrp_cn_ht <- MakeCorrHeatmap(cor_res = ctrp_cn_cor, row_names = TRUE, col_title = "CTRP", color_range = c(-0.25, 0, 0.25), radius = 0.1, legend_title = "CTRP Spearman correlation", metric = "CN", crispr =  FALSE)
...
gdsc_cn_cor <- unfactorize(filter(all_cn_cor, Dataset == "GDSC" & Gene %in% unique(g2p_amp$Gene)))
gdsc_cn_ht <- MakeCorrHeatmap(cor_res = gdsc_cn_cor, row_names = TRUE, col_title = "GDSC", color_range = c(-0.25, 0, 0.25), radius = 0.1, legend_title = "GDSC Spearman correlation", metric = "CN",  crispr = FALSE)
...
crispr_cn_cor <- unfactorize(filter(all_cn_cor, Dataset == "CRISPR" & Gene %in% unique(g2p_amp$Gene)))
crispr_cn_ht <- MakeCorrHeatmap(cor_res = crispr_cn_cor, row_names = TRUE, col_title = "CRISPR", color_range = c(-0.3, 0, 0.1), radius = 0.1, legend_title = "CRISPR Spearman correlation", metric = "CN")
png(filename = "./plots/manuscript/gdsc_cn_heatmap.png", width = 5, height = 3, units = "in", res = 500)
draw(gdsc_cn_ht, heatmap_legend_side = "top")
dev.off()

ccle_cn_ht2 <- MakeCorrHeatmap(cor_res = ccle_cn_cor, row_names = TRUE, col_title = "CCLE", color_range = c(-0.25, 0, 0.25), radius = 0.025, legend_title = "Copy num. Spearman correlation", metric = "CN", crispr =  FALSE)
ctrp_cn_ht2 <- MakeCorrHeatmap(cor_res = ctrp_cn_cor, row_names = FALSE, col_title = "CTRP", color_range = c(-0.25, 0, 0.25), radius = 0.025, legend_title = "Copy num. Spearman correlation", metric = "CN", crispr =  FALSE)
gdsc_cn_ht2 <- MakeCorrHeatmap(cor_res = gdsc_cn_cor, row_names = FALSE, col_title = "GDSC", color_range = c(-0.25, 0, 0.25), radius = 0.025, legend_title = "Copy num. Spearman correlation", metric = "CN", crispr =  FALSE)
png(filename = "./plots/manuscript/drug_cn_heatmap.png", width = 13, height = 3.5, units = "in", res = 500)
draw(ccle_cn_ht2 + ctrp_cn_ht2 + gdsc_cn_ht2, gap = unit(0.25, "in"), heatmap_legend_side = "top")
dev.off()

# Gene expression
ccle_ge_cor <- unfactorize(filter(all_ge_cor, Dataset == "CCLE" & Gene %in% unique(g2p_expr$Gene)))
ccle_ge_ht <- MakeCorrHeatmap(cor_res = ccle_ge_cor, row_names = TRUE, col_title = "CCLE", color_range = c(-0.5, 0, 0.5), radius = 0.2, legend_title = "CCLE Spearman correlation", metric = "GE", crispr =  FALSE)
...
ctrp_ge_cor <- unfactorize(filter(all_ge_cor, Dataset == "CTRP" & Gene %in% unique(g2p_expr$Gene)))
ctrp_ge_ht <- MakeCorrHeatmap(cor_res = ctrp_ge_cor, row_names = TRUE, col_title = "CTRP", color_range = c(-0.5, 0, 0.5), radius = 0.2, legend_title = "CTRP Spearman correlation", metric = "GE", crispr =  FALSE)
...
gdsc_ge_cor <- unfactorize(filter(all_ge_cor, Dataset == "GDSC" & Gene %in% unique(g2p_expr$Gene)))
gdsc_ge_ht <- MakeCorrHeatmap(cor_res = gdsc_ge_cor, row_names = TRUE, col_title = "GDSC", color_range = c(-0.5, 0, 0.5), radius = 0.2, legend_title = "GDSC Spearman correlation", metric = "GE", crispr =  FALSE)
...
ccle_ge_ht2 <- MakeCorrHeatmap(cor_res = ccle_ge_cor, row_names = TRUE, col_title = "CCLE", color_range = c(-0.5, 0, 0.5), radius = 0.2, legend_title = "Gene expr. Spearman correlation", metric = "GE", crispr =  FALSE)
ctrp_ge_ht2 <- MakeCorrHeatmap(cor_res = ctrp_ge_cor, row_names = FALSE, col_title = "CTRP", color_range = c(-0.5, 0, 0.5), radius = 0.2, legend_title = "Gene expr. Spearman correlation", metric = "GE", crispr =  FALSE)
gdsc_ge_ht2 <- MakeCorrHeatmap(cor_res = gdsc_ge_cor, row_names = FALSE, col_title = "GDSC", color_range = c(-0.5, 0, 0.5), radius = 0.2, legend_title = "Gene expr. Spearman correlation", metric = "GE", crispr =  FALSE)
png(filename = "./plots/manuscript/drug_ge_heatmap.png", width = 13, height = 3, units = "in", res = 500)
draw(ccle_ge_ht2 + ctrp_ge_ht2 + gdsc_ge_ht2, gap = unit(0.25, "in"), heatmap_legend_side = "top")
dev.off()
```

**10/28 NEED TO MAKE SPEAR CORR PLOTS WITH CRISPR PANEL**
