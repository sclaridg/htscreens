...
makeDrugBoxplots <- function(drug, dataset) {
  if(dataset == "CCLE" | dataset == "ccle") {
    ...
    sig <- ccle_signif_lvlA_ByDrug[[drug]]
    ...
    use_color <- as.character(use_data$Color_Nonsilent)
    names(use_color) <- use_data$Mutation_Status_Nonsilent
    sig <- ctrp_signif_lvlA_ByDrug[[drug]]
    ...
    use_color <- as.character(use_data$Color_Nonsilent)
    names(use_color) <- use_data$Mutation_Status_Nonsilent
    sig <- gdsc_signif_lvlA_ByDrug[[drug]]
  ...
  plot <- ggplot(data = use_data, aes(x = Mutation_Status_Nonsilent, y = AUC)) +
    facet_wrap(~ Hugo_Symbol, drop = FALSE, nrow = 2) +
               # ncol = length(genes_lvlA_filt)) +
    geom_boxplot(mapping = aes(fill = Mutation_Status_Nonsilent), position = position_dodge(0.85)) +
    scale_fill_manual(values = use_color) +
    guides(color = FALSE) +
    geom_text(data = label_text, mapping = aes(x = 1.5, y = max(use_data$AUC), label = p.signif.adj), nudge_y = 0.2) +
...
```{r, eval=FALSE}
ccle_signif_lvlA_ByDrug <- lapply(unique(ccle_data_lvlA$Drug), WilcoxonByDrug, dataset = ccle_data_lvlA, data_name = "CCLE")
names(ccle_signif_lvlA_ByDrug) <- unique(ccle_data_lvlA$Drug)
saveRDS(ccle_signif_lvlA_ByDrug, "./data_munging/rds/ccle_signif_lvlA_ByDrug.rds")
...
```{r}
ccle_signif_lvlA_ByDrug <- readRDS("./data_munging/rds/ccle_signif_lvlA_ByDrug.rds")
ccle_signif_lvlA_ByDrug_all <- rbindlist(ccle_signif_lvlA_ByDrug, use.names = TRUE)
ccle_signif_lvlA_ByDrug_all <- ccle_signif_lvlA_ByDrug_all[order(ccle_signif_lvlA_ByDrug_all$p.adj),]

knitr::kable(ccle_signif_lvlA_ByDrug_all[, c("Hugo_Symbol", "Drug", "p", "p.adj", "p.format", "p.signif", "p.signif.adj")], caption = "CCLE: by-drug Wilcoxon test results for Level A point mutations, (BH-adjusted p-values: * p <= 0.05, ** p <= 0.01, *** p <= 0.001, **** p <= 0.0001)") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```
...
```{r, eval=FALSE}
ctrp_signif_lvlA_ByDrug <- lapply(unique(ctrp_data_lvlA$Drug), WilcoxonByDrug, dataset = ctrp_data_lvlA, data_name = "CTRP")
names(ctrp_signif_lvlA_ByDrug) <- unique(ctrp_data_lvlA$Drug)
saveRDS(ctrp_signif_lvlA_ByDrug, "./data_munging/rds/ctrp_signif_lvlA_ByDrug.rds")

ctrp_signif_lvlA <- compare_means(AUC ~ Mutation_Status_Nonsilent, group.by = c("Drug", "Hugo_Symbol"), data = ctrp_data_lvlA, method = "wilcox.test", p.adjust.method = "BH")
...
```{r}
ctrp_signif_lvlA_ByDrug <- readRDS("./data_munging/rds/ctrp_signif_lvlA_ByDrug.rds")
ctrp_signif_lvlA_ByDrug_all <- rbindlist(ctrp_signif_lvlA_ByDrug, use.names = TRUE)
ctrp_signif_lvlA_ByDrug_all <- ctrp_signif_lvlA_ByDrug_all[order(ctrp_signif_lvlA_ByDrug_all$p.adj),]

knitr::kable(ctrp_signif_lvlA_ByDrug_all[, c("Hugo_Symbol", "Drug", "p", "p.adj", "p.format", "p.signif", "p.signif.adj")], caption = "CTRP: by-drug Wilcoxon test results for Level A point mutations, (BH-adjusted p-values: * p <= 0.05, ** p <= 0.01, *** p <= 0.001, **** p <= 0.0001)") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```
...
```{r, eval=FALSE}
gdsc_signif_lvlA_ByDrug <- lapply(unique(gdsc_data_lvlA$Drug), WilcoxonByDrug, dataset = gdsc_data_lvlA, data_name = "GDSC")
names(gdsc_signif_lvlA_ByDrug) <- unique(gdsc_data_lvlA$Drug)
saveRDS(gdsc_signif_lvlA_ByDrug, "./data_munging/rds/gdsc_signif_lvlA_ByDrug.rds")

gdsc_signif_lvlA <- compare_means(AUC ~ Mutation_Status_Nonsilent, group.by = c("Drug", "Hugo_Symbol"), data = gdsc_data_lvlA, method = "wilcox.test", p.adjust.method = "BH")
...
```{r}
gdsc_signif_lvlA_ByDrug <- readRDS("./data_munging/rds/gdsc_signif_lvlA_ByDrug.rds")
gdsc_signif_lvlA_ByDrug_all <- rbindlist(gdsc_signif_lvlA_ByDrug, use.names = TRUE)
gdsc_signif_lvlA_ByDrug_all <- gdsc_signif_lvlA_ByDrug_all[order(gdsc_signif_lvlA_ByDrug_all$p.adj),]

knitr::kable(gdsc_signif_lvlA_ByDrug_all[, c("Hugo_Symbol", "Drug", "p", "p.adj", "p.format", "p.signif", "p.signif.adj")], caption = "GDSC: by-drug Wilcoxon test results for Level A point mutations, (BH-adjusted p-values: * p <= 0.05, ** p <= 0.01, *** p <= 0.001, **** p <= 0.0001)") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```
...
ccle_ptmuts_lvlA_filt <- filter(ccle_data_lvlA, Hugo_Symbol %in% genes_lvlA_filt)
...
```{r fig.height=4, fig.width=20}
ctrp_ptmuts_lvlA_filt <- filter(ctrp_data_lvlA, Hugo_Symbol %in% genes_lvlA_filt)
...
pwalk(list(ctrp_lvlA_drugboxplot_paths, ctrp_lvlA_drugboxplots), ggsave, path = "./plots_18Q3/manuscript/ctrp_lvlA_boxplots", dpi = 450, width = 12, height = 7, units = "in")
```
...
```{r fig.height=4, fig.width=20}
gdsc_ptmuts_lvlA_filt <- filter(gdsc_data_lvlA, Hugo_Symbol %in% genes_lvlA_filt)
...
pwalk(list(gdsc_lvlA_drugboxplot_paths, gdsc_lvlA_drugboxplots), ggsave, path = "./plots_18Q3/manuscript/gdsc_lvlA_boxplots", dpi = 450, width = 12, height = 7, units = "in")
```
