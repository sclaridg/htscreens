...
MutPlotlyByLineage <- function(lineage_name) {
  ...
  if(nrow(use_data_ccle_summ) <= 1) {
    ...
    ccle_lin_p_ttest_res <- paste0("t(", round(ccle_lin_p_ttest$parameter, 1), ") = ", round(ccle_lin_p_ttest$statistic, 2), ", p = ", round(ccle_lin_p_ttest$p.value, 3))
  }
  ...
  if(nrow(use_data_ctrp_summ) <= 1) {
    ...
    ctrp_lin_p_ttest_res <- paste0("t(", round(ctrp_lin_p_ttest$parameter, 1), ") = ", round(ctrp_lin_p_ttest$statistic, 2), ", p = ", round(ctrp_lin_p_ttest$p.value, 3))
  }
  ...
  if(nrow(use_data_gdsc_summ) <= 1) {
    ...
    gdsc_lin_p_ttest_res <- paste0("t(", round(gdsc_lin_p_ttest$parameter, 1), ") = ", round(gdsc_lin_p_ttest$statistic, 2), ", p = ", round(gdsc_lin_p_ttest$p.value, 3))
  }
  ...
  saveWidgetFix(widget = plot, file = paste0("./plots/manuscript/plotlys_mut_by_lineage/", lineage_name, "_mut_plotly.html"))
  ...
  mut_plot <- cowplot::plot_grid(plot_box,
                     ...
                     nrow = 2,
                     rel_heights = c(1, 4))
  ggsave(paste0("./plots/manuscript/pngs_mut_by_lineage/", lineage_name, "_mut_plot.png"), mut_plot, device = "png", dpi = 450, width = 12, height = 6, units =   "in")
...
PearsonPlotlyByLineage <- function(lineage) {
  ...
  use_data <- filter(signif_lin_g2p_grid, group_general_lineage_name == lineage)
  use_data$Dataset <- factor(use_data$Dataset, levels = c("CCLE", "CTRP", "GDSC"))
  ...
  if(sum(!is.na(use_data$Pearson_GE_Abs)) > 0) {
    ...
    saveWidgetFix(widget = plot_ge, file = paste0("./plots/manuscript/plotlys_ge_by_lineage/", lineage, "_ge_plotly.html"))
    ...
    png_ge
    ggsave(paste0("./plots/manuscript/pngs_ge_by_lineage/", lineage, "_ge_plot.png"), png_ge, device = "png", dpi = 450, width = 12, height = 6, units = "in")
  ...
  if(sum(!is.na(use_data$Pearson_CN_Abs)) > 0) {
    ...
    plot_cn <- subplot(plot_cn_box, plot_cn_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>%
               layout(title = lineage, margin = list(t = 75, l = 75),
                      ...
                      yaxis = list(title = "In G2P?", domain = c(0.8, 1)),
                      yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))
    saveWidgetFix(widget = plot_cn, file = paste0("./plots/manuscript/plotlys_cn_by_lineage/", lineage, "_cn_plotly.html"))
    ...
    png_cn
    ggsave(paste0("./plots/manuscript/pngs_cn_by_lineage/", lineage, "_cn_plot.png"), png_cn, device = "png", dpi = 450, width = 12, height = 6, units = "in")
...
```{r}
length(unique(ccle_data_g2p_grid$Drug))
length(unique(ccle_data_g2p_grid$Drug_Gene))
```
...
```{r}
test_ccle <- ccle_data_g2p_grid %>% group_by(Hugo_Symbol, Drug) %>% tally()
```
...
ccle_signif_g2p_grid <- compare_means(AUC_zscore ~ Mutation_Status_Nonsilent, group.by = c("Hugo_Symbol", "Drug"), ref.group = "Wildtype", data = ccle_data_g2p_grid, method = "wilcox.test", p.adjust.method = "BH")
...
#### Summary: Drug screen test plots
...
ctrp_summ_res_mut_plot <- ggplot(data = subset(ctrp_res_mut, p < 0.05)) +
  ...
  geom_point(aes(x = corr, y = p_log10, color = Metric), alpha = 0.5) +
  geom_label_repel(mapping = aes(y = p_log10, x = corr, label = Drug_Gene, color = Metric), force = 10, size = 2.5, segment.size = 0.2, segment.colour = "grey50", box.padding = 1) +
  theme(legend.position = "none", panel.grid.major.x = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(5.5, 1.5, 5.5, 5.5), "pt"), strip.background.y = element_blank(), strip.text.y = element_blank()) +
  scale_color_manual(values = c("Mutation Status" = "darkorchid4")) +
  labs(y = "-log10(p-value)")
...
gdsc_summ_res_mut_plot <- ggplot(data = subset(gdsc_res_mut, p < 0.05)) +
  ...
  # scale_y_continuous(breaks = seq(0, 30, by = 5), labels = seq(0, 30, by = 5)) +
  geom_point(aes(x = corr, y = p_log10, color = Metric), alpha = 0.5) +
  geom_text_repel(mapping = aes(y = p_log10, x = corr, label = Drug_Gene, color = Metric), box.padding = 0.75, force = 15, size = 2.5, segment.size = 0.2, segment.colour = "grey50") +
  theme(legend.position = "none", panel.grid.major.x = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(5.5, 1.5, 5.5, 5.5), "pt"), strip.background.y = element_blank(), strip.text.y = element_blank()) +
  scale_color_manual(values = c("Mutation Status" = "darkorchid4")) +
  labs(y = "-log10(p-value)")
...
```{r}
levels(all_pearson_res$Dataset)
```
...
Test the 102 level-A G2P drug-gene-lineage combinations:
```{r, eval=FALSE}
...
saveRDS(signif_all_lin, "./data_munging/rds/signif_lin_g2p_dgl.rds")
```

```{r}
signif_lin_g2p_dgl <- readRDS("./data_munging/rds/signif_lin_g2p_dgl.rds")
...
# CCLE GE/CN, n = 9
ccle_ge <- select(subset(filter(signif_lin_g2p_dgl, Dataset == "CCLE"), Pearson_Pval_GE < 0.05), Drug_Gene_Lin, Pearson_Pval_GE, Pearson_Corr_GE)
ccle_cn <- select(subset(filter(signif_lin_g2p_dgl, Dataset == "CCLE"), Pearson_Pval_CN < 0.05), Drug_Gene_Lin, Pearson_Pval_CN, Pearson_Corr_CN)
ccle_Pear_n <- nrow(filter(signif_lin_g2p_dgl, Dataset == "CCLE"))
...
# CTRP GE/CN, n = 61
ctrp_ge <- select(subset(filter(signif_lin_g2p_dgl, Dataset == "CTRP"), Pearson_Pval_GE < 0.05), Drug_Gene_Lin, Pearson_Pval_GE, Pearson_Corr_GE)
ctrp_cn <- select(subset(filter(signif_lin_g2p_dgl, Dataset == "CTRP"), Pearson_Pval_CN < 0.05), Drug_Gene_Lin, Pearson_Pval_CN, Pearson_Corr_CN)
ctrp_Pear_n <- nrow(filter(signif_lin_g2p_dgl, Dataset == "CTRP"))
...
# GDSC GE/CN, n = 53
gdsc_ge <- select(subset(filter(signif_lin_g2p_dgl, Dataset == "GDSC"), Pearson_Pval_GE < 0.05), Drug_Gene_Lin, Pearson_Pval_GE, Pearson_Corr_GE)
gdsc_cn <- select(subset(filter(signif_lin_g2p_dgl, Dataset == "GDSC"), Pearson_Pval_CN < 0.05), Drug_Gene_Lin, Pearson_Pval_CN, Pearson_Corr_CN)
gdsc_Pear_n <- nrow(filter(signif_lin_g2p_dgl, Dataset == "GDSC"))
...
dgl_summ <- merge(merge(ccle_dgl_summ, ctrp_dgl_summ, by = c("Drug", "Gene", "Lineage"), all = TRUE), gdsc_dgl_summ, by = c("Drug", "Gene", "Lineage"), all = TRUE)
```
...
signif_all_lin$InG2P <- ifelse(signif_all_lin$n != 0, "Yes", "No")
signif_all_lin$Drug_Gene_Lin <- paste0(signif_all_lin$Drug_Gene, "_", signif_all_lin$group_general_lineage_name)
saveRDS(signif_all_lin, "./data_munging/rds/signif_lin_g2p_grid.rds")
```

```{r}
signif_lin_g2p_grid <- readRDS("./data_munging/rds/signif_lin_g2p_grid.rds")
```

```{r, results='hide', eval=FALSE}
druggenelin_pearson_ttest_res <- lapply(as.character(unique(signif_lin_g2p_grid$group_general_lineage_name)), PearsonPlotlyByLineage)
...
Example p-value and Pearson correlation coefficient derivations:
```{r}
...
```{r}
ttest_summ_res_mut <- g2p_druggenelin_p_ttest_res_list[, c("p_value", "Dataset", "group_general_lineage_name")]
...
ttest_summ_res_ge <- druggenelin_pearson_ttest_res_list[, c("p_value_GE", "Dataset", "group_general_lineage_name")]
...
ttest_summ_res_cn <- druggenelin_pearson_ttest_res_list[, c("p_value_CN", "Dataset", "group_general_lineage_name")]
...
ttest_mut_summ_plot <- ggplot(data = g2p_druggenelin_p_ttest_res_list, aes(x = p_value)) +
...
ttest_ge_summ_plot <- ggplot(data = druggenelin_pearson_ttest_res_list, aes(x = p_value_GE)) +
  geom_histogram(aes(fill = Dataset, color = Dataset), position = "stack", bins = 40, binwidth = 0.025, boundary = 0, closed = "left") +
...
ttest_cn_summ_plot <- ggplot(data = druggenelin_pearson_ttest_res_list, aes(x = p_value_CN)) +
  geom_histogram(aes(fill = Dataset, color = Dataset), position = "stack", bins = 40, binwidth = 0.025, boundary = 0, closed = "left") +
...
all_druggenelin_ge <- select(signif_lin_g2p_grid, Drug_Gene, group_general_lineage_name, n, Pearson_GE_Abs, InG2P, Dataset, Pearson_GE_Sign)
...
all_druggenelin_cn <- select(signif_lin_g2p_grid, Drug_Gene, group_general_lineage_name, n, Pearson_CN_Abs, InG2P, Dataset, Pearson_CN_Sign)
...
# ggsave("./plots/manuscript/all_druggenelin_summ_plot.png", all_druggenelin_summ_plot, device = "png", dpi = 450, width = 12, height = 40, units = "in")
```
