...
```{r}
PearsonByLineage <- function(lin, dataset) {
  ...
  if(nrow(use_data_filt) > 0) {
    pearson_res <- use_data_filt %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$p.value)
    pearson_res <- merge(g2p_druggene_p_tally, pearson_res, all.y = TRUE)
    pearson_res$group_general_lineage_name <- lin
    pearson_res$Pearson_Corr_Abs <- abs(pearson_res$Pearson_Corr)
    pearson_res$Pearson_Corr_Sign <- ifelse(sign(pearson_res$Pearson_Corr) == 1, "+",
                                            ifelse(sign(pearson_res$Pearson_Corr) == -1, "-", "0"))
    return(pearson_res)
  }
}
```
...
## Make G2P lineage gene expression plotly
```{r}
GexpPlotlyByLineage <- function(lineage) {
  use_data <- filter(signif_pearson_all_lin, group_general_lineage_name == lineage)
  use_data$Dataset <- factor(use_data$Dataset, levels = c("CCLE", "CTRP", "GDSC"))
  use_data$Pearson_Pvalue <- paste0(round(use_data$Pearson_Pval, 3), "\nDrug_Gene: ", use_data$Drug_Gene)
  ticker <- 0
  
  use_data_ccle <- filter(use_data, Dataset == "CCLE")
  use_data_ccle_summ <- use_data_ccle %>% count(InG2P)
  ccle_lin_ttest <- data.frame(statistic = NA,
                                 parameter =  NA,
                                 p.value = NA)
  if(nrow(use_data_ccle_summ) <= 1) {
    ccle_ttest_res <-  NA
  ...
  else if(use_data_ccle_summ[use_data_ccle_summ$InG2P == "Yes",]$nn < 2) { ccle_ttest_res <-  NA }
  else if(use_data_ccle_summ[use_data_ccle_summ$InG2P == "Yes",]$nn >= 2) {
    ccle_lin_ttest <- t.test(use_data_ccle$Pearson_Corr_Abs ~ use_data_ccle$InG2P)
    ccle_ttest_res <- paste0("t = ", round(ccle_lin_ttest$statistic, 3), ", df = ", round(ccle_lin_ttest$parameter, 3), ", p = ", round(ccle_lin_ttest$p.value, 3))
  }
  
  use_data_ctrp <- filter(use_data, Dataset == "CTRP")
  use_data_ctrp_summ <- use_data_ctrp %>% count(InG2P)
  ctrp_lin_ttest <- data.frame(statistic = NA,
                                 parameter =  NA,
                                 p.value = NA)
  if(nrow(use_data_ctrp_summ) <= 1) {
    ctrp_ttest_res <-  NA
  ...
  else if(use_data_ctrp_summ[use_data_ctrp_summ$InG2P == "Yes",]$nn < 2) { ctrp_ttest_res <-  NA }
  else if(use_data_ctrp_summ[use_data_ctrp_summ$InG2P == "Yes",]$nn >= 2) {
    ctrp_lin_ttest <- t.test(use_data_ctrp$Pearson_Corr_Abs ~ use_data_ctrp$InG2P)
    ctrp_ttest_res <- paste0("t = ", round(ctrp_lin_ttest$statistic, 3), ", df = ", round(ctrp_lin_ttest$parameter, 3), ", p = ", round(ctrp_lin_ttest$p.value, 3))
  }
  
  use_data_gdsc <- filter(use_data, Dataset == "GDSC")
  use_data_gdsc_summ <- use_data_gdsc %>% count(InG2P)
  gdsc_lin_ttest <- data.frame(statistic = NA,
                                 parameter =  NA,
                                 p.value = NA)
  if(nrow(use_data_gdsc_summ) <= 1) {
    gdsc_ttest_res <-  NA
  ...
  else if(use_data_gdsc_summ[use_data_gdsc_summ$InG2P == "Yes",]$nn < 2) { gdsc_ttest_res <-  NA }
  else if(use_data_gdsc_summ[use_data_gdsc_summ$InG2P == "Yes",]$nn >= 2) {
    gdsc_lin_ttest <- t.test(use_data_gdsc$Pearson_Corr_Abs ~ use_data_gdsc$InG2P)
    gdsc_ttest_res <- paste0("t = ", round(gdsc_lin_ttest$statistic, 3), ", df = ", round(gdsc_lin_ttest$parameter, 3), ", p = ", round(gdsc_lin_ttest$p.value, 3))
  }
  
  y_value <- ifelse(ticker == 3, 10, max(use_data$n) + 1)
  
  summ_text <- data.frame(Dataset = c("CCLE", "CTRP", "GDSC"),
                          Test_Results_p = c(ccle_ttest_res, ctrp_ttest_res, gdsc_ttest_res),
                          t_statistic = c(round(ccle_lin_ttest$statistic, 3), round(ctrp_lin_ttest$statistic, 3), round(gdsc_lin_ttest$statistic, 3)),
                          df = c(round(ccle_lin_ttest$parameter, 3), round(ctrp_lin_ttest$parameter, 3), round(gdsc_lin_ttest$parameter, 3)),
                          p_value = c(round(ccle_lin_ttest$p.value, 3), round(ctrp_lin_ttest$p.value, 3), round(gdsc_lin_ttest$p.value, 3)),
                          group_general_lineage_name = lineage)
  
  plot_point <- ggplot(data = use_data) +
    facet_wrap(~ Dataset, drop = FALSE) +
    scale_y_continuous(breaks = pretty_breaks()) +
    geom_point(mapping = aes(label = Pearson_Pvalue, color = InG2P, fill = InG2P, shape = Pearson_Corr_Sign, x = Pearson_Corr_Abs, y = n), alpha = 0.5, position = position_jitter(width = 0, height = 0.15)) +
    scale_shape_manual(values = c(25, 24), labels = c("Negative", "Positive")) +
    scale_color_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
    scale_fill_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
    geom_text(data = summ_text, mapping = aes(x = 0.5, y = y_value, label = Test_Results_p), nudge_y = 1) +
    labs(y = "Number of level A G2P associations") +
    theme(legend.position = "none", plot.margin = unit(c(0.05, 0.5, 0.5, 0.5), "cm"), strip.background = element_blank(), strip.text.x = element_blank())

  plot_box <- ggplot(data = use_data) +
    facet_wrap(~ Dataset, drop = FALSE) +
    geom_violin(mapping = aes(color = InG2P, y = Pearson_Corr_Abs, x = InG2P), alpha = 0.5) +
    geom_boxplot(mapping = aes(color = InG2P, y = Pearson_Corr_Abs, x = InG2P), alpha = 0.5, width = 0.25) +
    scale_color_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
    coord_flip() +
    labs(x = "Level A G2P\nassociation") +
    theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(0.5, 0.5, 0.05, 0.5), "cm"))

  plot <- subplot(plot_box, plot_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(title = lineage, margin = list(t = 75, l = 75), xaxis5 = list(title = "Pearson correlation coefficient between drug-gene AUC and gene expression"), yaxis = list(title = "Level A G2P\nassociation", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))

  saveWidgetFix(widget = plot, file = paste0("./plots_18Q3/manuscript/plotlys_pearson_by_lineage/", lineage, "_g2p_evidence_pearson_plotly.html"))
  return(summ_text)
}
```
...
## Drugscreen: Mutation status G2P evidence
...
# ggsave("./plots_18Q3/manuscript/g2p_druggene_all_p_summ_plot.png", g2p_druggene_all_p_summ_plot, device = "png", dpi = 450, width = 12, height = 5, units = "in")
```
...
plot <- subplot(plot_p_box, plot_p_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(margin = list(l = 75), xaxis5 = list(title = "Gene-drug association p-value between mutation groups"), yaxis = list(title = "Level A G2P\nassociation", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))

saveWidgetFix(widget = plot, file = "./plots_18Q3/manuscript/g2p_druggene_all_p_summ_plot.html")
```
...
## Drugscreen: Gene expression G2P evidence

### Lineage-agnostic

```{r}
ccle_pearson_res <- ccle_data_ptmuts %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$p.value)
...
ccle_pearson_ttest <- t.test(ccle_pearson_res$Pearson_Corr ~ ccle_pearson_res$InG2P)
ccle_pearson_ttest_res <- paste0("t = ", round(ccle_pearson_ttest$statistic, 3), ", df = ", round(ccle_pearson_ttest$parameter, 3), ", p = ", round(ccle_pearson_ttest$p.value, 3))

ctrp_pearson_res <- ctrp_data_ptmuts %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$p.value)
...
ctrp_pearson_ttest <- t.test(ctrp_pearson_res$Pearson_Corr ~ ctrp_pearson_res$InG2P)
ctrp_pearson_ttest_res <- paste0("t = ", round(ctrp_pearson_ttest$statistic, 3), ", df = ", round(ctrp_pearson_ttest$parameter, 3), ", p = ", round(ctrp_pearson_ttest$p.value, 3))

gdsc_pearson_res <- gdsc_data_ptmuts %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$p.value)
...
gdsc_pearson_ttest <- t.test(gdsc_pearson_res$Pearson_Corr ~ gdsc_pearson_res$InG2P)
gdsc_pearson_ttest_res <- paste0("t = ", round(gdsc_pearson_ttest$statistic, 3), ", df = ", round(gdsc_pearson_ttest$parameter, 3), ", p = ", round(gdsc_pearson_ttest$p.value, 3))

g2p_druggene_pearson <- rbind(ccle_pearson_res, ctrp_pearson_res, gdsc_pearson_res)
g2p_druggene_pearson$Pearson_Corr_Abs <- abs(g2p_druggene_pearson$Pearson_Corr)
g2p_druggene_pearson$Pearson_Corr_Sign <- ifelse(sign(g2p_druggene_pearson$Pearson_Corr) == 1, "+",
                                           ifelse(sign(g2p_druggene_pearson$Pearson_Corr) == -1, "-", "0"))
g2p_druggene_pearson_text <- data.frame(Dataset = c("CCLE", "CTRP", "GDSC"), Test_Results_PearCorr = c(ccle_pearson_ttest_res, ctrp_pearson_ttest_res, gdsc_pearson_ttest_res))
```

```{r fig.height=5.5, fig.width=12}
plot_pearson_point <- ggplot(data = g2p_druggene_pearson, aes(x = Pearson_Corr_Abs, y = n, label = Drug_Gene)) +
  facet_wrap(~ Dataset) +
  geom_point(mapping = aes(fill = InG2P, color = InG2P, shape = Pearson_Corr_Sign), alpha = 0.5, size = 1.25) +
  scale_color_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
  scale_fill_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
  scale_y_continuous(breaks = seq(0, 110, by = 10), labels = seq(0, 110, by = 10)) +
  ...
  geom_text(data = g2p_druggene_pearson_text, mapping = aes(x = 0.3, y = 110, label = Test_Results_PearCorr)) +
...
plot_pearson_box <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_violin(mapping = aes(color = InG2P, y = Pearson_Corr_Abs, x = InG2P), alpha = 0.5) +
  geom_boxplot(mapping = aes(color = InG2P, y = Pearson_Corr_Abs, x = InG2P), alpha = 0.5, width = 0.25) +
  scale_color_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
...
plot_pearson_point <- ggplot_gtable(ggplot_build(plot_pearson_point))
plot_pearson_box <- ggplot_gtable(ggplot_build(plot_pearson_box))
p_maxWidth <-  grid::unit.pmax(plot_pearson_point$widths, plot_pearson_box$widths)
plot_pearson_point$widths <- as.list(p_maxWidth)
plot_pearson_box$widths <- as.list(p_maxWidth)

g2p_druggene_pearson_plot <- cowplot::plot_grid(plot_pearson_box, plot_pearson_point, nrow = 2, rel_heights = c(1.5, 4))
g2p_druggene_pearson_plot
# ggsave("./plots_18Q3/manuscript/g2p_druggene_pearson_plot.png", g2p_druggene_pearson_plot, device = "png", dpi = 450, width = 12, height = 5.5, units = "in")
```

```{r, eval=FALSE}
plot_pearson_point <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_point(mapping = aes(label = Drug_Gene, fill = InG2P, color = InG2P, shape = Pearson_Corr_Sign, x = Pearson_Corr_Abs, y = n), alpha = 0.5, size = 1.25) +
  scale_color_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
  scale_y_continuous(breaks = seq(0, 110, by = 10), labels = seq(0, 110, by = 10)) +
  scale_x_continuous(breaks = pretty_breaks()) +
  scale_shape_manual(values = c(25, 24), labels = c("Negative", "Positive")) +
...
plot <- subplot(plot_pearson_box, plot_pearson_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(margin = list(l = 75), xaxis5 = list(title = "Pearson correlation coefficient between drug-gene AUC and gene expression"), yaxis = list(title = "Level A G2P\nassociation", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))

saveWidgetFix(widget = plot, file = "./plots_18Q3/manuscript/g2p_druggene_pearson_plot.html")
```
...
ccle_signif_pearson <- lapply(ccle_lineages4pearson, PearsonByLineage, dataset = ccle_data_g2p_grid)
ccle_signif_pearson_all_lin <- rbindlist(ccle_signif_pearson, use.names = TRUE)
ccle_signif_pearson_all_lin$Dataset <- "CCLE"
...
ctrp_signif_pearson <- lapply(ctrp_lineages4pearson, PearsonByLineage, dataset = ctrp_data_g2p_grid)
ctrp_signif_pearson_all_lin <- rbindlist(ctrp_signif_pearson, use.names = TRUE)
ctrp_signif_pearson_all_lin$Dataset <- "CTRP"
...
gdsc_signif_pearson <- lapply(gdsc_lineages4pearson, PearsonByLineage, dataset = gdsc_data_g2p_grid)
gdsc_signif_pearson_all_lin <- rbindlist(gdsc_signif_pearson, use.names = TRUE)
gdsc_signif_pearson_all_lin$Dataset <- "GDSC"

signif_pearson_all_lin <- rbind(ccle_signif_pearson_all_lin, ctrp_signif_pearson_all_lin, gdsc_signif_pearson_all_lin)
signif_pearson_all_lin$n <- ifelse(is.na(signif_pearson_all_lin$n), 0, signif_pearson_all_lin$n)
signif_pearson_all_lin$InG2P <- ifelse(signif_pearson_all_lin$n != 0, "Yes", "No")
```

```{r, results='hide', eval=FALSE}
g2p_druggenelin_pearson_ttest_res <- lapply(as.character(unique(signif_pearson_all_lin$group_general_lineage_name)), GexpPlotlyByLineage)
g2p_druggenelin_pearson_ttest_res_list <- rbindlist(g2p_druggenelin_pearson_ttest_res, use.names = TRUE)
g2p_druggenelin_pearson_ttest_res_list <- g2p_druggenelin_pearson_ttest_res_list[order(g2p_druggenelin_pearson_ttest_res_list$p_value),]
saveRDS(g2p_druggenelin_pearson_ttest_res_list, "./data_munging/rds/g2p_druggenelin_pearson_ttest_res_list.rds")
```

```{r}
g2p_druggenelin_pearson_ttest_res_list <- readRDS("./data_munging/rds/g2p_druggenelin_pearson_ttest_res_list.rds")

knitr::kable(g2p_druggenelin_pearson_ttest_res_list, caption = "GDSC: T-test results for level A G2P drug-gene/gene expression associations across lineages") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```
