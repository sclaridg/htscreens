...
make_G2Plineage_plotly <- function(lineage_name) {
  ...
  plot_p_point <- ggplot(data = use_data) +
  ...
  plot_p_box <- ggplot(data = use_data) +
    facet_wrap(~ Dataset, drop = FALSE) +
  ...
  plot <- subplot(plot_p_box, plot_p_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(title = lineage_name, margin = list(t = 75, l = 75), xaxis5 = list(title = "Gene-drug association p-value between mutation groups"), yaxis = list(title = "Level A G2P\nassociation", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))

  saveWidgetFix(widget = plot, file = paste0("./plots_18Q3/manuscript/plotlys_by_lineage/", lineage_name, "_g2p_evidence_plotly.html"))
}
```
...
plot_mad_box <- ggplot(data = g2p_druggene_all_summ) +
  ...
  geom_boxplot(mapping = aes(color = InG2P, y = MAD, x = InG2P), alpha = 0.75, width = 0.25) +
  coord_flip() +
  labs(x = "Level A G2P\nassociation") +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(0.5, 0.5, 0.05, 0.5), "cm"))
