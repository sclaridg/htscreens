...
PearsonByLineage <- function(lin, dataset, variable) {
  ...
  if(variable == "GE") {
    ...
    use_data <- filter(use_data, !is.na(Copy_Number))
    use_data_summ <- use_data %>% count(Drug_Gene)
    use_data_summ <- filter(use_data_summ, n >= 3) # cor.test requires at least 3 non-NA data points
    use_data_filt <- filter(use_data, Drug_Gene %in% as.character(unique(use_data_summ$Drug_Gene)))
    if(nrow(use_data_filt) > 0) {
      pearson_res <- use_data_filt %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr_CN = cor.test(y = AUC, x = Copy_Number, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval_CN = cor.test(y = AUC, x = Copy_Number, method = "pearson", use = "complete.obs")$p.value)
      pearson_res <- merge(g2p_druggene_p_tally, pearson_res, all.y = TRUE)
      pearson_res$group_general_lineage_name <- lin
...
ccle_data <- merge(ccle_data, ge_filt, by = c("Hugo_Symbol", "CCLE_Name", "Broad_ID"), all.x = TRUE)
ccle_data <- merge(ccle_data, cn_filt, by = c("Hugo_Symbol", "Broad_ID"), all.x = TRUE)
...
ctrp_data <- merge(ctrp_data, ge_filt, by = c("Hugo_Symbol", "CCLE_Name", "Broad_ID"), all.x = TRUE)
ctrp_data <- merge(ctrp_data, cn_filt, by = c("Hugo_Symbol", "Broad_ID"), all.x = TRUE)
...
gdsc_data <- merge(gdsc_data, ge_filt, by = c("Hugo_Symbol", "CCLE_Name", "Broad_ID"), all.x = TRUE)
gdsc_data <- merge(gdsc_data, cn_filt, by = c("Hugo_Symbol", "Broad_ID"), all.x = TRUE)
...
ccle_pearson_res <- ccle_data_g2p_grid %>% group_by(Drug_Gene) %>%
                    summarize(Pearson_Corr_GE = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate,
                              ...
                              Pearson_Corr_CN = cor.test(y = AUC, x = Copy_Number, method = "pearson", use = "complete.obs")$estimate,
                              Pearson_Pval_CN = cor.test(y = AUC, x = Copy_Number, method = "pearson", use = "complete.obs")$p.value)
