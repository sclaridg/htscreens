...
WilcoxonByDrug <- function(drug, dataset, data_name, grouping_var) {
  ...
  sig <- compare_means(AUC ~ Mutation_Status_Nonsilent, group.by = grouper, data = use_data, method = "wilcox.test", p.adjust.method = "BH")
  sig <- adj_signif(sig)
  sig <- sig[order(sig$p),]
  sig$Drug <- drug
...
PearsonByLineage <- function(lin, dataset, variable) {
  ...
  if(variable == "GE") {
    ...
    if(nrow(use_data_filt) > 0) {
      pearson_res <- use_data_filt %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr_GE = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval_GE = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$p.value)
    ...
    use_data_filt <- filter(use_data, Drug_Gene %in% as.character(unique(use_data_summ$Drug_Gene)))
    if(nrow(use_data_filt) > 0) {
      pearson_res <- use_data_filt %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr_CN = cor.test(y = AUC, x = CN, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval_CN = cor.test(y = AUC, x = CN, method = "pearson", use = "complete.obs")$p.value)
      pearson_res <- merge(g2p_druggene_p_tally, pearson_res, all.y = TRUE)
      pearson_res$group_general_lineage_name <- lin
...
## Make G2P lineage p-value plotly
```{r}
MutPlotlyByLineage <- function(lineage_name) {
  ...
  plot <- subplot(plot_box, plot_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(title = lineage_name, margin = list(t = 75, l = 75), xaxis5 = list(title = "Gene-drug association p-value between mutation groups"), yaxis = list(title = "Level A G2P\nassociation", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))

  saveWidgetFix(widget = plot, file = paste0("./plots_18Q3/manuscript/plotlys_p_by_lineage/", lineage_name, "_g2p_evidence_p_plotly.html"))
  return(summ_text)
}
```
...
PearsonPlotlyByLineage <- function(lineage) {
  ...
  if(sum(!is.na(use_data$Pearson_GE_Abs)) > 0) {
    plot_ge_point <- ggplot(data = use_data) +
      ...
      labs(y = "Number of level A G2P associations") +
      theme(legend.position = "none", plot.margin = unit(c(0.05, 0.5, 0.5, 0.5), "cm"), strip.background = element_blank(), strip.text.x = element_blank())
  ...
  else if(sum(!is.na(use_data$Pearson_GE_Abs)) == 0) { print("Gene expression plotly: None") }
  ...
  else if(sum(!is.na(use_data$Pearson_CN_Abs)) == 0) { print("Copy number plotly: None") }
...
CCLE RNAseq gene expression data (log2(TPM+1)):
```{r, eval=FALSE}
ge <- read.delim("./../htscreen_lineages_giant_files/CCLE_depMap_18Q4_TPM_v2_20181115.csv.gz", header = TRUE, sep = ",", check.names = FALSE)
...
# Melt
ge_melt <- melt(ge, id.vars = "DepMap_ID", measure.vars = colnames(ge)[2:ncol(ge)], variable.name = "Hugo_Symbol", value.name = "TPM")

saveRDS(ge_melt, "./data_munging/rds/ge_melt_18Q4.rds", compress = "xz")
```

```{r, eval=FALSE}
ge_melt <- readRDS("./data_munging/rds/ge_melt_18Q4.rds")
```
...
ge_filt <- filter(ge_melt, Hugo_Symbol %in% as.character(unique(crispr_data$Hugo_Symbol)) & DepMap_ID %in% as.character(unique(crispr_data$DepMap_ID)))
crispr_data <- merge(crispr_data, ge_filt, by = c("Hugo_Symbol", "DepMap_ID", "CCLE_Name"), all.x = TRUE)
crispr_data$RPKM_log2 <- log2(crispr_data$RPKM + 0.0001)
saveRDS(crispr_data, "./../htscreen_lineages_giant_files/crispr_data_18Q3.rds", compress = "xz")
```

```{r}
crispr_data <- readRDS("./../htscreen_lineages_giant_files/crispr_data_18Q3.rds")
...
ccle_data <- merge(ccle_data, ge_filt, by = c("Hugo_Symbol", "CCLE_Name", "DepMap_ID"), all.x = TRUE)
...
ctrp_data <- merge(ctrp_data, ge_filt, by = c("Hugo_Symbol", "CCLE_Name", "DepMap_ID"), all.x = TRUE)
...
gdsc_data <- merge(gdsc_data, ge_filt, by = c("Hugo_Symbol", "CCLE_Name", "DepMap_ID"), all.x = TRUE)
...
ccle_signif_g2p_grid <- compare_means(AUC ~ Mutation_Status_Nonsilent, group.by = c("Hugo_Symbol", "Drug"), data = ccle_data_g2p_grid, method = "wilcox.test", p.adjust.method = "BH")
...
# Grid comparisons of G2P drugs and genes
ctrp_signif_g2p_grid <- compare_means(AUC ~ Mutation_Status_Nonsilent, group.by = c("Hugo_Symbol", "Drug"), data = ctrp_data_g2p_grid, method = "wilcox.test", p.adjust.method = "BH")
...
# Grid comparisons of G2P drugs and genes
gdsc_signif_g2p_grid <- compare_means(AUC ~ Mutation_Status_Nonsilent, group.by = c("Hugo_Symbol", "Drug"), data = gdsc_data_g2p_grid, method = "wilcox.test", p.adjust.method = "BH")
...
plot_p_point <- ggplot(data = g2p_druggene_all_summ, aes(x = p, y = n, label = Drug_Gene)) +
  ...
  labs(x = "Gene-drug association p-value between mutation groups", y = "Number of level A G2P associations", color = "Does the drug-gene combination have level A G2P indication?") +
  theme(legend.position = "none", plot.margin = unit(c(0.05, 0.5, 0.5, 0.5), "cm"), strip.background = element_blank(), strip.text.x = element_blank())
...
plot_cn_point <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_point(mapping = aes(label = Drug_Gene, fill = InG2P, color = InG2P, shape = Pearson_GE_Sign, x = Pearson_GE_Abs, y = n), alpha = 0.5, size = 1.25) +
  scale_color_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
  scale_fill_manual(values = c("Yes" = "darkorchid4", "No" = "lightpink")) +
  scale_shape_manual(values = c(25, 24), labels = c("Negative", "Positive")) +
  scale_y_continuous(breaks = seq(0, 110, by = 10), labels = seq(0, 110, by = 10)) +
...
plot <- subplot(plot_cn_box, plot_cn_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(margin = list(l = 75), xaxis5 = list(title = "Pearson correlation coefficient between drug-gene AUC and gene expression"), yaxis = list(title = "In G2P?", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))
...
knitr::kable(g2p_druggenelin_p_ttest_res_list, caption = "GDSC: T-test results for level A G2P drug-gene associations across lineages") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```
