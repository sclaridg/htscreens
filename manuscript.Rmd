...
MutPlotlyByLineage <- function(lineage_name) {
  ...
  plot_box <- ggplot(data = use_data) +
    ...
    geom_boxplot(mapping = aes(color = InG2P, y = p, x = InG2P), alpha = 0.75, width = 0.5) +
    coord_flip() +
    labs(x = "Level A G2P\nassociation") +
...
GexpPlotlyByLineage <- function(lineage) {
  ...
  if(nrow(use_data_ccle_summ) <= 1) {
    ...
    ccle_lin_ttest <- t.test(use_data_ccle$Pearson_Corr ~ use_data_ccle$InG2P)
  ...
  if(nrow(use_data_ctrp_summ) <= 1) {
    ...
    ctrp_lin_ttest <- t.test(use_data_ctrp$Pearson_Corr ~ use_data_ctrp$InG2P)
  ...
  if(nrow(use_data_gdsc_summ) <= 1) {
    ...
    gdsc_lin_ttest <- t.test(use_data_gdsc$Pearson_Corr ~ use_data_gdsc$InG2P)
  ...
  y_value <- ifelse(ticker == 3, 10, max(use_data$n) + 1)
  x_value <- (max(use_data$Pearson_Corr) + min(use_data$Pearson_Corr)) / 2
  ...
  plot_point <- ggplot(data = use_data) +
    facet_wrap(~ Dataset, drop = FALSE) +
    scale_y_continuous(breaks = pretty_breaks()) +
    geom_point(mapping = aes(label = Pearson_Pvalue, color = InG2P, x = Pearson_Corr, y = n), alpha = 0.5, position = position_jitter(width = 0, height = 0.15)) +
    geom_text(data = summ_text, mapping = aes(x = x_value, y = y_value, label = Test_Results), nudge_y = 1) +
    labs(x = "Gene-drug association p-value between mutation groups", y = "Number of level A G2P associations") +
    theme(legend.position = "none", plot.margin = unit(c(0.05, 0.5, 0.5, 0.5), "cm"), strip.background = element_blank(), strip.text.x = element_blank())

  plot_box <- ggplot(data = use_data) +
    facet_wrap(~ Dataset, drop = FALSE) +
    geom_violin(mapping = aes(color = InG2P, y = Pearson_Corr, x = InG2P), alpha = 0.5) +
    geom_boxplot(mapping = aes(color = InG2P, y = Pearson_Corr, x = InG2P), alpha = 0.5, width = 0.5) +
    coord_flip() +
    labs(x = "Level A G2P\nassociation") +
    theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(0.5, 0.5, 0.05, 0.5), "cm"))
...
plot_pearson_point <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_point(mapping = aes(label = Drug_Gene, color = InG2P, x = Pearson_Corr, y = n), alpha = 0.5) +
  scale_y_continuous(breaks = seq(0, 110, by = 10), labels = seq(0, 110, by = 10)) +
  scale_x_continuous(breaks = pretty_breaks()) +
  geom_text(data = g2p_druggene_pearson_text, mapping = aes(x = -0.05, y = 110, label = Test_Results_PearCorr)) +
...
plot_pearson_box <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_violin(mapping = aes(color = InG2P, y = Pearson_Corr, x = InG2P), alpha = 0.5) +
  geom_boxplot(mapping = aes(color = InG2P, y = Pearson_Corr, x = InG2P), alpha = 0.5, width = 0.25) +
  coord_flip() +
  labs(x = "Level A G2P\nassociation") +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(0.5, 0.5, 0.05, 0.5), "cm"))
