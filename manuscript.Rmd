...
make_G2Plineage_plotly <- function(lineage_name) {
  ...
  else if(sum(use_data_ccle$n) == 0) {ccle_lin_p_ttest_res <-  NA}
  
  use_data_ctrp <- filter(use_data, Dataset == "CTRP")
  if(sum(use_data_ctrp$n) > 0) {
  ...
  else if(sum(use_data_ctrp$n) == 0) {ctrp_lin_p_ttest_res <-  NA}
  
  use_data_gdsc <- filter(use_data, Dataset == "GDSC")
  if(sum(use_data_gdsc$n) > 0) {
  ...
  else if(sum(use_data_gdsc$n) == 0) {gdsc_lin_p_ttest_res <-  NA}
  ...
  #   facet_wrap(~ Dataset, scales = "free_x") +
  #   geom_point(mapping = aes(label = Drug_Gene, color = InG2P, x = p, y = n), alpha = 0.5) +
  #   geom_text(data = g2p_druggene_all_summ_text, mapping = aes(x = 0.5, y = max(use_data$n), label = Test_Results_p)) +
  #   labs(x = "Gene-drug association p-value between mutation groups", y = "Number of level A G2P associations", color = "Does the gene-drug combination have level A G2P indication?") +
  #   theme(legend.position = "none", plot.margin = unit(c(0.05, 0.5, 0.5, 0.5), "cm"), strip.background = element_blank(), strip.text.x = element_blank())
  # 
  ...
  # plot <- subplot(plot_p_box, plot_p_point, nrows = 2, titleX = TRUE, titleY = TRUE) %>% layout(margin = list(l = 75), xaxis5 = list(title = "Gene-drug association p-value between mutation groups"), yaxis = list(title = "Level A G2P\nassociation", domain = c(0.8, 1)), yaxis2 = list(title = "Number of level A G2P associations", domain = c(0, 0.79)))
  # 
  # saveWidgetFix(widget = plot, file = paste0("./plots_18Q3/manuscript/lineage_plotly/", lineage_name, "_g2p_evidence_plotly.html"))
}
```
