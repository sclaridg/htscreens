...
PearsonPlotlyByLineage <- function(lineage) {
  ...
  if(nrow(use_data_ccle_ge_summ) <= 1) {
    ...
    ccle_ge_res <- paste0("t(", round(ccle_ge_ttest$parameter, 1), ") = ", round(ccle_ge_ttest$statistic, 2), ", p = ", round(ccle_ge_ttest$p.value, 3))
  }
  if(nrow(use_data_ccle_cn_summ) <= 1) {
    ...
    ccle_cn_res <- paste0("t(", round(ccle_cn_ttest$parameter, 1), ") = ", round(ccle_cn_ttest$statistic, 2), ", p = ", round(ccle_cn_ttest$p.value, 3))
  }
  ...
  if(nrow(use_data_ctrp_ge_summ) <= 1) {
    ...
    ctrp_ge_res <- paste0("t(", round(ctrp_ge_ttest$parameter, 1), ") = ", round(ctrp_ge_ttest$statistic, 2), ", p = ", round(ctrp_ge_ttest$p.value, 3))
  }
  if(nrow(use_data_ctrp_cn_summ) <= 1) {
    ...
    ctrp_cn_res <- paste0("t(", round(ctrp_cn_ttest$parameter, 1), ") = ", round(ctrp_cn_ttest$statistic, 2), ", p = ", round(ctrp_cn_ttest$p.value, 3))
  }
  ...
  if(nrow(use_data_gdsc_ge_summ) <= 1) {
    ...
    gdsc_ge_res <- paste0("t(", round(gdsc_ge_ttest$parameter, 1), ") = ", round(gdsc_ge_ttest$statistic, 2), ", p = ", round(gdsc_ge_ttest$p.value, 3))
  }
  if(nrow(use_data_gdsc_cn_summ) <= 1) {
    ...
    gdsc_cn_res <- paste0("t(", round(gdsc_cn_ttest$parameter, 1), ") = ", round(gdsc_cn_ttest$statistic, 2), ", p = ", round(gdsc_cn_ttest$p.value, 3))
  }
...
summ <- g2p_nonCID %>% group_by(Drug, DrugID) %>% tally() %>% summarize(n = n,
                                                                percent = n / 402 * 100)
summ
```
...
# ggsave("./plots/manuscript/crispr_data_signif_g2p_lineage_plot.png", crispr_data_signif_g2p_lineage_plot, width = 12, height = 15, units = "in")
```
...
g2p_drug_tally <- g2p %>% group_by(Drug) %>% tally() %>% drop_na()
range(g2p_drug_tally$n)
mean(g2p_drug_tally$n)
sd(g2p_drug_tally$n)

g2p_gene_tally <- g2p %>% group_by(Gene) %>% tally() %>% drop_na()
...
ccle_p_ttest_res <- paste0("t(", round(ccle_p_ttest$parameter, 1), ") = ", round(ccle_p_ttest$statistic, 2), ", p = ", round(ccle_p_ttest$p.value, 3))
...
ctrp_p_ttest_res <- paste0("t(", round(ctrp_p_ttest$parameter, 1), ") = ", round(ctrp_p_ttest$statistic, 2), ", p = ", round(ctrp_p_ttest$p.value, 3))
...
gdsc_p_ttest_res <- paste0("t(", round(gdsc_p_ttest$parameter, 1), ") = ", round(gdsc_p_ttest$statistic, 2), ", p = ", round(gdsc_p_ttest$p.value, 3))
...
ccle_pearson_ttest_res_GE <- paste0("t(", round(ccle_pearson_ttest_GE$parameter, 1), ") = ", round(ccle_pearson_ttest_GE$statistic, 2), ", p = ", round(ccle_pearson_ttest_GE$p.value, 3))
ccle_pearson_ttest_CN <- t.test(ccle_pearson_res$Pearson_Corr_CN ~ ccle_pearson_res$InG2P)
ccle_pearson_ttest_res_CN <- paste0("t(", round(ccle_pearson_ttest_CN$parameter, 1), ") = ", round(ccle_pearson_ttest_CN$statistic, 2), ", p = ", round(ccle_pearson_ttest_CN$p.value, 3))
...
ctrp_pearson_ttest_res_GE <- paste0("t(", round(ctrp_pearson_ttest_GE$parameter, 1), ") = ", round(ctrp_pearson_ttest_GE$statistic, 2), ", p = ", round(ctrp_pearson_ttest_GE$p.value, 3))
ctrp_pearson_ttest_CN <- t.test(ctrp_pearson_res$Pearson_Corr_CN ~ ctrp_pearson_res$InG2P)
ctrp_pearson_ttest_res_CN <- paste0("t(", round(ctrp_pearson_ttest_CN$parameter, 1), ") = ", round(ctrp_pearson_ttest_CN$statistic, 2), ", p = ", round(ctrp_pearson_ttest_CN$p.value, 3))
...
gdsc_pearson_ttest_res_GE <- paste0("t(", round(gdsc_pearson_ttest_GE$parameter, 1), ") = ", round(gdsc_pearson_ttest_GE$statistic, 2), ", p = ", round(gdsc_pearson_ttest_GE$p.value, 3))
gdsc_pearson_ttest_CN <- t.test(gdsc_pearson_res$Pearson_Corr_CN ~ gdsc_pearson_res$InG2P)
gdsc_pearson_ttest_res_CN <- paste0("t(", round(gdsc_pearson_ttest_CN$parameter, 1), ") = ", round(gdsc_pearson_ttest_CN$statistic, 2), ", p = ", round(gdsc_pearson_ttest_CN$p.value, 3))
...
g2p_druggene_all_summ_stats <- g2p_druggene_all_summ_stats %>% drop_na(p) %>% group_by(Dataset, InG2P) %>% summarize(p_mean = round(mean(p), 3), p_sd = round(sd(p), 3), n = sum(n_true), n_signif = sum(p < 0.05))
```
...
g2p_druggene_pearson_stats <- g2p_druggene_pearson_stats %>% group_by(Dataset, InG2P) %>% drop_na() %>% summarize(ge_mean = round(mean(Pearson_GE_Abs), 3),
          ge_sd = round(sd(Pearson_GE_Abs), 3),
          ge_n = sum(n_true),
          ge_n_signif = sum(Pearson_Pval_GE < 0.05),
          cn_mean = round(mean(Pearson_CN_Abs), 3),
          cn_sd = round(sd(Pearson_CN_Abs), 3),
          cn_n = sum(n_true),
...
ctrp_grid_summary_mut_corr_text <- paste0("W = ", ctrp_grid_summary_mut_corr$statistic, ", p = ", round(ctrp_grid_summary_mut_corr$p.value, 3))
...
ctrp_grid_summary_ge_corr_text <- paste0("r = ", round(ctrp_grid_summary_ge_corr$estimate, 3), ", t(", ctrp_grid_summary_ge_corr$parameter, ") = ", round(ctrp_grid_summary_ge_corr$statistic, 2), ", p = ", round(ctrp_grid_summary_ge_corr$p.value, 3))
...
ctrp_grid_summary_cn_corr_text <- paste0("r = ", round(ctrp_grid_summary_cn_corr$estimate, 3), ", t(", ctrp_grid_summary_cn_corr$parameter, ") = ", round(ctrp_grid_summary_cn_corr$statistic, 2), ", p = ", round(ctrp_grid_summary_cn_corr$p.value, 3))
...
ctrp_grid_summary_ge_plot <- ggplot(data = ctrp_grid_summary_data, aes(y = AUC_zscore, x = RPKM)) +
  ...
  annotate(geom = "text", label = ctrp_grid_summary_ge_corr_text, x = 1750, y = 1.3) +
...
ctrp_grid_summary_cn_plot <- ggplot(data = ctrp_grid_summary_data, aes(y = AUC_zscore, x = CN)) +
  geom_point(alpha = 0.25, size = 1.25, color = "gray4") +
  geom_smooth(method = "lm", se = FALSE, lwd = 0.5, lty = 2) +
  annotate(geom = "text", label = ctrp_grid_summary_cn_corr_text, x = 4, y = 1.3) +
