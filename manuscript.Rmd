...
MutPlotlyByLineage <- function(lineage_name) {
  ...
  g2p_druggenelin_all_summ_text <- data.frame(Dataset = c("CCLE", "CTRP", "GDSC"), Test_Results_p = c(ccle_lin_p_ttest_res, ctrp_lin_p_ttest_res, gdsc_lin_p_ttest_res))
  ...
  plot_point <- ggplot(data = use_data) +
    ...
    geom_text(data = g2p_druggenelin_all_summ_text, mapping = aes(x = 0.5, y = y_value, label = Test_Results_p), nudge_y = 0.1) +
...
GexpPlotlyByLineage <- function(lineage) {
  ...
  summ_text <- data.frame(Dataset = c("CCLE", "CTRP", "GDSC"), Test_Results = c(ccle_ttest_res, ctrp_ttest_res, gdsc_ttest_res))
  
  plot_point <- ggplot(data = use_data) +
    facet_wrap(~ Dataset, drop = FALSE) +
    scale_y_continuous(breaks = pretty_breaks()) +
    geom_point(mapping = aes(label = Pearson_Pvalue, color = InG2P, shape = Pearson_Corr_Sign, x = Pearson_Corr_Abs, y = n), alpha = 0.5, position = position_jitter(width = 0, height = 0.15)) +
    scale_shape_manual(values = c(45, 43, 42)) +
    geom_text(data = summ_text, mapping = aes(x = 0.5, y = y_value, label = Test_Results), nudge_y = 1) +
...
knitr::kable(ccle_signif_g2p_grid[, c("Hugo_Symbol", "Drug", "p", "p.adj", "p.format", "p.signif", "p.signif.adj")], caption = "CCLE: By-drug Wilcoxon test results for point mutations in level A G2P genes (* p <= 0.05, ** p <= 0.01, *** p <= 0.001, **** p <= 0.0001)") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>% scroll_box(width = "900px", height = "450px")
```

### G2P gene, drug, and lineage

```{r, eval=FALSE}
ccle_signif_g2p_lineage <- lapply(unique(ccle_data_ptmuts$Drug), WilcoxonByDrug, dataset = ccle_data_ptmuts, data_name = "ccle", grouping_var = "lineage")
...
ctrp_signif_g2p_ByDrug <- lapply(unique(ctrp_data_ptmuts$Drug), WilcoxonByDrug, dataset = ctrp_data_ptmuts, data_name = "CTRP", grouping_var = "gene")
names(ctrp_signif_g2p_ByDrug) <- unique(ctrp_data_ptmuts$Drug)
...
```{r, eval=FALSE}
ctrp_signif_g2p_lineage <- lapply(unique(ctrp_data_ptmuts$Drug), WilcoxonByDrug, dataset = ctrp_data_ptmuts, data_name = "ctrp", grouping_var = "lineage")
...
gdsc_signif_g2p_ByDrug <- lapply(unique(gdsc_data_ptmuts$Drug), WilcoxonByDrug, dataset = gdsc_data_ptmuts, data_name = "gdsc", grouping_var = "gene")
names(gdsc_signif_g2p_ByDrug) <- unique(gdsc_data_ptmuts$Drug)
...
```{r, eval=FALSE}
gdsc_signif_g2p_lineage <- lapply(unique(gdsc_data_ptmuts$Drug), WilcoxonByDrug, dataset = gdsc_data_ptmuts, data_name = "gdsc", grouping_var = "lineage")
...
ccle_druggene_p_summ <- ccle_signif_g2p_lineage
...
ctrp_druggene_p_summ <- ctrp_signif_g2p_lineage
...
gdsc_druggene_p_summ <- gdsc_signif_g2p_lineage
...
plot_mad_point <- ggplot(data = g2p_druggene_all_summ) +
  ...
  labs(x = "Gene-drug association difference in mean AUC between mutation groups", y = "Number of level A G2P associations", color = "Does the drug-gene combination have level A G2P indication?") +
  theme(legend.position = "none", plot.margin = unit(c(0.05, 0.5, 0.5, 0.5), "cm"), strip.background = element_blank(), strip.text.x = element_blank())

plot_mad_box <- ggplot(data = g2p_druggene_all_summ) +
  facet_wrap(~ Dataset, scales = "free_x") +
  geom_violin(mapping = aes(color = InG2P, y = MAD, x = InG2P), alpha = 0.5) +
  geom_boxplot(mapping = aes(color = InG2P, y = MAD, x = InG2P), alpha = 0.75, width = 0.5) +
  coord_flip() +
  labs(x = "Level A G2P\nassociation") +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(0.5, 0.5, 0.05, 0.5), "cm"))
...
# ggsave("./plots_18Q3/manuscript/g2p_druggene_all_mad_summ_plot.png", g2p_druggene_all_mad_summ_plot, device = "png", dpi = 450, width = 12, height = 5, units = "in")
...
```{r}
ccle_druggenelin_p_summ <- ccle_signif_g2p_lineage
ccle_druggenelin_p_summ$Drug_Gene <- paste0(ccle_druggenelin_p_summ$Drug, "_", ccle_druggenelin_p_summ$Hugo_Symbol)
ccle_druggenelin_summ <- merge(g2p_druggenelin_p_tally, ccle_druggenelin_p_summ[, c("Drug_Gene", "group_general_lineage_name", "p")], by = c("Drug_Gene", "group_general_lineage_name"), all.y = TRUE)
...
ctrp_druggenelin_p_summ <- ctrp_signif_g2p_lineage
ctrp_druggenelin_p_summ$Drug_Gene <- paste0(ctrp_druggenelin_p_summ$Drug, "_", ctrp_druggenelin_p_summ$Hugo_Symbol)
ctrp_druggenelin_summ <- merge(g2p_druggenelin_p_tally, ctrp_druggenelin_p_summ[, c("Drug_Gene", "group_general_lineage_name", "p")], by = c("Drug_Gene", "group_general_lineage_name"), all.y = TRUE)
...
gdsc_druggenelin_p_summ <- gdsc_signif_g2p_lineage
gdsc_druggenelin_p_summ$Drug_Gene <- paste0(gdsc_druggenelin_p_summ$Drug, "_", gdsc_druggenelin_p_summ$Hugo_Symbol)
gdsc_druggenelin_summ <- merge(g2p_druggenelin_p_tally, gdsc_druggenelin_p_summ[, c("Drug_Gene", "group_general_lineage_name", "p")], by = c("Drug_Gene", "group_general_lineage_name"), all.y = TRUE)
...
g2p_druggenelin_all_summ$InG2P <- factor(g2p_druggenelin_all_summ$InG2P)
...
lapply(as.character(unique(g2p_druggenelin_all_summ$group_general_lineage_name)), MutPlotlyByLineage)

# test <- filter(g2p_druggenelin_all_summ, Dataset == "CCLE")
# test_summ <- test %>% count(group_general_lineage_name, InG2P)
...
plot_pearson_point <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_point(mapping = aes(label = Drug_Gene, color = InG2P, shape = Pearson_Corr_Sign, x = Pearson_Corr_Abs, y = n)) +
  scale_y_continuous(breaks = seq(0, 110, by = 10), labels = seq(0, 110, by = 10)) +
...
# ggsave("./plots_18Q3/manuscript/g2p_druggene_pearson_plot.png", g2p_druggene_pearson_plot, device = "png", dpi = 450, width = 12, height = 5, units = "in")
```

```{r, eval=FALSE}
plot_pearson_point <- ggplot(data = g2p_druggene_pearson) +
  facet_wrap(~ Dataset) +
  geom_point(mapping = aes(label = Drug_Gene, color = InG2P, x = Pearson_Corr_Abs, shape = Pearson_Corr_Sign, y = n), alpha = 0.5) +
  scale_y_continuous(breaks = seq(0, 110, by = 10), labels = seq(0, 110, by = 10)) +
  scale_x_continuous(breaks = pretty_breaks()) +
  scale_shape_manual(values = c(45, 43, 42)) +
  geom_text(data = g2p_druggene_pearson_text, mapping = aes(x = 0.3, y = 110, label = Test_Results_PearCorr)) +
...
```{r, eval=FALSE}
ccle_lineages4pearson <- as.character(unique(ccle_data_ptmuts$group_general_lineage_name))[!is.na(as.character(unique(ccle_data_ptmuts$group_general_lineage_name)))]
ccle_signif_pearson <- lapply(ccle_lineages4pearson, PearsonByLineage, dataset = ccle_data_ptmuts)
ccle_signif_pearson_all_lin <- rbindlist(ccle_signif_pearson, use.names = TRUE)
ccle_signif_pearson_all_lin$Dataset <- "CCLE"

ctrp_lineages4pearson <- as.character(unique(ctrp_data_ptmuts$group_general_lineage_name))[!is.na(as.character(unique(ctrp_data_ptmuts$group_general_lineage_name)))]
ctrp_signif_pearson <- lapply(ctrp_lineages4pearson, PearsonByLineage, dataset = ctrp_data_ptmuts)
ctrp_signif_pearson_all_lin <- rbindlist(ctrp_signif_pearson, use.names = TRUE)
ctrp_signif_pearson_all_lin$Dataset <- "CTRP"

gdsc_lineages4pearson <- as.character(unique(gdsc_data_ptmuts$group_general_lineage_name))[!is.na(as.character(unique(gdsc_data_ptmuts$group_general_lineage_name)))]
gdsc_signif_pearson <- lapply(gdsc_lineages4pearson, PearsonByLineage, dataset = gdsc_data_ptmuts)
...
lapply(as.character(unique(signif_pearson_all_lin$group_general_lineage_name)), GexpPlotlyByLineage)
```
