...
## Make G2P lineage plotly
```{r}
make_G2Plineage_plotly <- function(lineage_name) {
...
lapply(as.character(unique(g2p_druggenelin_all_summ$group_general_lineage_name)), make_G2Plineage_plotly)
...
```{r}
drugscreen_ptmuts <- rbind(ccle_data_ptmuts, ctrp_data_ptmuts, gdsc_data_ptmuts)
```

```{r}
summ <- ccle_data_ptmuts %>% count(group_general_lineage_name, Drug_Gene)
```

```{r}
PearsonByLineage <- function(lineage, dataset) {
  use_data <- filter(dataset, group_general_lineage_name == lineage)
  if(nrow(use_data) == 0) {
    use_data <- NA
  }
  # use_data_summ <- use_data %>% count(Drug_Gene)
  # use_data_summ <- filter(use_data_summ, n >= 3)
  # use_data <- filter(use_data, Drug_Gene %in% as.character(unique(use_data_summ$Drug_Gene)))
  # 
  # pearson_res <- use_data %>% group_by(Drug_Gene) %>% summarize(Pearson_Corr = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$estimate, Pearson_Pval = cor.test(y = AUC, x = RPKM, method = "pearson", use = "complete.obs")$p.value)
  # pearson_res <- merge(g2p_druggene_p_tally, pearson_res, all.y = TRUE)
  # pearson_res$group_general_lineage_name <- lineage
  
  return(use_data)
}

# test <- PearsonByLineage("breast cancer", ccle_data_ptmuts)

# ccle_signif_pearson <- lapply(as.character(unique(ccle_data_ptmuts$group_general_lineage_name)), PearsonByLineage, dataset = ccle_data_ptmuts)
```
...
PearsonPlotlyByLineage <- function(lineage) {
  ...
  if(nrow(use_data_ccle_summ) <= 1) {
    ...
    ccle_lin_p_ttest <- t.test(use_data_ccle$p ~ use_data_ccle$InG2P)
    ccle_ttest_res <- paste0("t = ", round(ccle_lin_p_ttest$statistic, 3), ", df = ", round(ccle_lin_p_ttest$parameter, 3), ", p = ", round(ccle_lin_p_ttest$p.value, 3))
  }
  ...
  if(nrow(use_data_ctrp_summ) <= 1) {
    ...
    ctrp_lin_p_ttest <- t.test(use_data_ctrp$p ~ use_data_ctrp$InG2P)
    ctrp_ttest_res <- paste0("t = ", round(ctrp_lin_p_ttest$statistic, 3), ", df = ", round(ctrp_lin_p_ttest$parameter, 3), ", p = ", round(ctrp_lin_p_ttest$p.value, 3))
  }
  ...
  if(nrow(use_data_gdsc_summ) <= 1) {
    ...
    gdsc_lin_p_ttest <- t.test(use_data_gdsc$p ~ use_data_gdsc$InG2P)
    gdsc_ttest_res <- paste0("t = ", round(gdsc_lin_p_ttest$statistic, 3), ", df = ", round(gdsc_lin_p_ttest$parameter, 3), ", p = ", round(gdsc_lin_p_ttest$p.value, 3))
  }
  
  y_value <- ifelse(ticker == 3, 10, max(use_data$n) + 1)

  g2p_druggenelin_all_summ_text <- data.frame(Dataset = c("CCLE", "CTRP", "GDSC"), Test_Results_p = c(ccle_ttest_res, ctrp_ttest_res, gdsc_ttest_res))
  # use_data_summ <- use_data %>% count(InG2P)
  # if(nrow(use_data_ccle_summ) <= 1) { ttest_res <- NA }
  # else if(use_data_summ[use_data_summ$InG2P == "Yes",]$nn < 2) { ttest_res <- NA }
  # else if(use_data_summ[use_data_summ$InG2P == "Yes",]$nn >= 2) {
  #   ttest <- t.test(use_data$p ~ use_data$InG2P)
  #   ttest_res <- paste0("t = ", round(ttest$statistic, 3), ", df = ", round(ttest$parameter, 3), ", p = ", round(ttest$p.value, 3))
  # }
}
```
...
plot_pearson_box <- ggplot(data = g2p_druggene_pearson) +
  ...
  geom_boxplot(mapping = aes(color = InG2P, y = Pearson_Corr, x = InG2P), alpha = 0.75, width = 0.25) +
  coord_flip() +
  labs(x = "Level A G2P\nassociation") +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(0.5, 0.5, 0.05, 0.5), "cm"))
