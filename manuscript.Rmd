...
g2p_genes <- unique(sapply(strsplit(g2p_druggenes, "_", fixed = TRUE), function(x) x[2]))
g2p_drugs <- unique(sapply(strsplit(g2p_druggenes, "_", fixed = TRUE), function(x) x[1]))
g2p_druggenes
g2p_genes
g2p_drugs
...
g2p_cor_stats <- g2p_cor
g2p_cor_stats$n_true <- TRUE
g2p_cor_stats <- g2p_cor_stats %>% group_by(Dataset, InG2P) %>% drop_na() %>% summarize(
...
MakeCorrResSummaryTable <- function(df_list, dataset_name) {
  ...
  for(i in 1:length(df_list)) {
    corr_res <- df_list[[i]] %>% group_by(G2P_DGL) %>% summarize(Dataset = dataset_name,
                ...
                Pearson_Corr_GE = cor.test(y = AUC_zscore, x = RPKM, method = "pearson", use = "complete.obs")$estimate,
