...
```{r}
g2p <- read.delim("./data_munging/g2p_full_level_A_dataset_NEW_NOV_27.csv", sep = "\t", header = TRUE)
colnames(g2p)[colnames(g2p) == "Phenotype.ID"] <- "grouped_general_doid"
...
crispr_data_signif_g2p_lineage_plot <- ggplot(data = crispr_data_signif_g2p_lineage, aes(x = group_general_lineage_name, y = Score)) +
  facet_wrap(~ Hugo_Symbol, nrow = 6, scales = "free_y") +
...
gdsc_p_ttest_res <- paste0("t(", round(gdsc_p_ttest$parameter, 0), ") = ", round(gdsc_p_ttest$statistic, 2), ", p = ", round(gdsc_p_ttest$p.value, 2))
...
g2p_druggene_all_summ_stats <- g2p_druggene_all_summ_stats %>% drop_na(p) %>% group_by(Dataset, InG2P) %>% summarize(p_mean = round(mean(p), 2), p_sd = round(sd(p), 2), n = sum(n_true), n_signif = sum(p < 0.05))
```
